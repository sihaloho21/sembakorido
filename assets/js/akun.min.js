...
Untuk sementara, hubungi admin untuk reset PIN.`);const c=d.replace(/^0/,"62");window.open(`https://wa.me/628993370200?text=Halo, saya ingin reset PIN akun saya. Nomor WhatsApp: ${c}`,"_blank"),setTimeout(()=>{showLogin()},1e3)}catch(r){console.error("Forgot PIN error:",r),a.textContent="Terjadi kesalahan. Silakan coba lagi.",n.classList.remove("hidden")}finally{o.disabled=!1,o.innerHTML='<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg> Kirim Kode Verifikasi'}});function openEditProfile(){const e=getLoggedInUser();e&&(document.getElementById("edit-name").value=e.nama,document.getElementById("edit-old-pin").value="",document.getElementById("edit-new-pin").value="",document.getElementById("edit-confirm-pin").value="",document.getElementById("edit-error").classList.add("hidden"),document.getElementById("edit-profile-modal").classList.remove("hidden"))}function closeEditProfile(){document.getElementById("edit-profile-modal").classList.add("hidden")}document.getElementById("edit-profile-form").addEventListener("submit",async e=>{e.preventDefault();const t=getLoggedInUser();if(!t)return;const n=document.getElementById("edit-name").value.trim(),a=document.getElementById("edit-old-pin").value.trim(),o=document.getElementById("edit-new-pin").value.trim(),d=document.getElementById("edit-confirm-pin").value.trim(),r=document.getElementById("edit-error"),i=document.getElementById("edit-error-text"),s=document.getElementById("edit-save-btn");if(r.classList.add("hidden"),n.replace(/\s/g,"").length<4){i.textContent="Masukkan Nama Lengkap",r.classList.remove("hidden");return}if(a||o||d){if(!a||!o||!d){i.textContent="Lengkapi semua field PIN",r.classList.remove("hidden");return}if(o.length!==6||!/^\d{6}$/.test(o)){i.textContent="PIN baru harus 6 digit angka",r.classList.remove("hidden");return}if(o!==d){i.textContent="PIN baru tidak cocok",r.classList.remove("hidden");return}}s.disabled=!0,s.textContent="Menyimpan...";try{const c=CONFIG.getMainApiUrl(),m=await(await fetch(`${c}?sheet=users&id=${t.id}`)).json();if(!m||m.length===0)throw new Error("User not found");const p=m[0];if(a&&p.pin!==a){i.textContent="PIN lama salah",r.classList.remove("hidden"),s.disabled=!1,s.textContent="Simpan";return}const g={nama:n};o&&(g.pin=o);const w=await GASActions.update("users",t.id,g);if(!w.affected||w.affected<1)throw new Error("Failed to update");t.nama=n,saveLoggedInUser(t),document.getElementById("user-name").textContent=n,closeEditProfile(),alert("Profil berhasil diperbarui!")}catch(c){console.error("Edit profile error:",c),i.textContent="Terjadi kesalahan. Silakan coba lagi.",r.classList.remove("hidden")}finally{s.disabled=!1,s.textContent="Simpan"}});function showOrderDetailModal(e){document.getElementById("tracking-order-id").textContent=e.id||"N/A";const t=e.tanggal||e.tanggal_pesanan||e.timestamp||e.date||e.created_at;document.getElementById("tracking-order-date").textContent=formatDate(t);const n=normalizeStatus(e.status||"Menunggu"),a=getStatusBadge(n),o=document.getElementById("tracking-status-badge");o&&(o.innerHTML=a),document.getElementById("tracking-products").textContent=e.produk||e.items||e.product_name||"N/A",document.getElementById("tracking-total").textContent=formatCurrency(e.total||e.total_bayar||0);const d=createAnimatedTimeline(n);document.getElementById("tracking-timeline").innerHTML=d,document.getElementById("order-tracking-modal").classList.remove("hidden")}function createAnimatedTimeline(e){const t=[{name:"Menunggu",gif:"wait.gif",key:"Menunggu"},{name:"Diproses",gif:"grocery-basket.gif",key:"Diproses"},{name:"Dikirim",gif:"grocery.gif",key:"Dikirim"},{name:"Diterima",gif:"shipping.gif",key:"Diterima"}];let n=t.findIndex(a=>a.key===e);return e==="Dibatalkan"?`
    ...
    `,t}function closeLoyaltyModal(){document.getElementById("loyalty-modal").classList.add("hidden"),switchRewardTab("exchange")}async function openRewardModal(){const e=getLoggedInUser();if(!e)return;await loadModalPoints();const t=document.getElementById("loyalty-modal-points"),n=t?parseInt(t.textContent||"0"):0;setRewardContextFromAkun(e.whatsapp,n,e.nama),document.getElementById("loyalty-modal").classList.remove("hidden"),switchRewardTab("exchange"),fetchRewardItemsForAkun()}async function loadModalPoints(){const e=getLoggedInUser();if(!e)return;const t=parseInt(e.total_points||e.points||e.poin||0,10)||0;try{const n=CONFIG.getMainApiUrl(),a=buildSessionQuery(e);if(a){const i=await fetch(`${n}?action=public_user_points${a}&_t=${Date.now()}`);if(i.ok){const s=await i.json();if(s&&s.success){document.getElementById("loyalty-modal-points").textContent=String(parseInt(s.points||0,10)||0);return}}}if(!hasPublicSession(e)){document.getElementById("loyalty-modal-points").textContent=String(t);return}const o=await fetch(`${n}?sheet=user_points`);if(!o.ok){document.getElementById("loyalty-modal-points").textContent=String(t);return}const d=await o.json();if(d&&typeof d=="object"&&String(d.error||"").toLowerCase()==="unauthorized"){console.warn("\u26A0\uFE0F Unauthorized access to user_points (modal). Falling back to user profile points."),document.getElementById("loyalty-modal-points").textContent=String(t);return}const r=Array.isArray(d)?d.filter(i=>normalizePhoneTo08(i.phone)===normalizePhoneTo08(e.whatsapp)):[];if(r&&r.length>0){const i=r[0],s=parseInt(i.points||i.poin||0);document.getElementById("loyalty-modal-points").textContent=s}else document.getElementById("loyalty-modal-points").textContent=String(t)}catch(n){console.error("Error loading modal points:",n),document.getElementById("loyalty-modal-points").textContent=String(t)}}var escapeHtml=window.FrontendSanitize&&window.FrontendSanitize.escapeHtml||(e=>{const t=document.createElement("div");return t.textContent=e,t.innerHTML}),sanitizeUrl=window.FrontendSanitize&&window.FrontendSanitize.sanitizeUrl||(e=>String(e||"")),ensureImageFallbackHandler=window.FrontendSanitize&&window.FrontendSanitize.ensureImageFallbackHandler||(()=>{});ensureImageFallbackHandler();function hideRewardLoaders(){const e=document.getElementById("reward-items-loading"),t=document.getElementById("rewards-loading");e&&e.classList.add("hidden"),t&&t.classList.add("hidden")}async function fetchRewardItemsForAkun(){const e=document.getElementById("reward-items-loading"),t=document.getElementById("reward-items-empty"),n=document.getElementById("reward-items-list");if(n){e&&e.classList.remove("hidden"),t&&t.classList.add("hidden"),n.innerHTML="";try{const a=CONFIG.getMainApiUrl(),o=await fetch(`${a}?sheet=tukar_poin`);if(!o.ok)throw new Error("Failed to fetch reward items");const d=await o.json(),r=parseSheetResponse(d);renderRewardItemsListAkun(r)}catch(a){console.error("Error fetching reward items:",a),t&&t.classList.add("hidden"),n.innerHTML=`
        ...
        `}finally{hideRewardLoaders()}}}function renderRewardItemsListAkun(e){const t=document.getElementById("reward-items-empty"),n=document.getElementById("reward-items-list");if(n){if(!e||e.length===0){t&&t.classList.remove("hidden"),n.innerHTML="";return}t&&t.classList.add("hidden"),n.innerHTML=e.map(a=>{const o=escapeHtml((a.id||"").toString()),d=escapeHtml((a.nama||a.judul||"Hadiah").toString()),r=parseInt(a.poin||a.Poin||0),i=(a.gambar||"https://via.placeholder.com/80?text=Reward").toString(),s=sanitizeUrl(i,"https://via.placeholder.com/80?text=Reward"),l=escapeHtml((a.deskripsi||"").toString());return`
            <div class="border-2 border-gray-200 rounded-xl p-4 hover:border-amber-500 transition">
                <div class="flex gap-3 mb-3">
                    <!-- Left: Image -->
                    <div class="w-16 h-16 bg-gray-100 rounded-lg overflow-hidden flex-shrink-0">
                        <img src="${s}" alt="${d}" class="w-full h-full object-cover" data-fallback-src="https://via.placeholder.com/80?text=Reward">
                    </div>
                    
                    <!-- Middle: Title & Description -->
                    <div class="flex-1 min-w-0">
                        <p class="font-bold text-gray-800 text-sm mb-1">${d}</p>
