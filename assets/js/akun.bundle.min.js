window.FrontendSanitize={escapeHtml:function(e){return String(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")},sanitizeUrl:function(e,t=""){if(!e)return t;const n=String(e).trim();return/^(https?:\/\/|\/(?!\/)|\.\/|\.\.\/|#)/i.test(n)?n:t},ensureImageFallbackHandler:function(){window.__imageFallbackHandlerAdded||(window.__imageFallbackHandlerAdded=!0,document.addEventListener("error",e=>{const t=e.target;if(!t||t.tagName!=="IMG")return;const n=t.getAttribute("data-fallback-src"),a=t.getAttribute("data-fallback-action");n&&t.src!==n?t.src=n:a==="hide"&&(t.style.display="none")},!0))}},window.sanitizeUrl||(window.sanitizeUrl=window.FrontendSanitize.sanitizeUrl),window.ensureImageFallbackHandler||(window.ensureImageFallbackHandler=window.FrontendSanitize.ensureImageFallbackHandler),window.AdminSanitize||(window.AdminSanitize=window.FrontendSanitize);var escapeHtml=window.FrontendSanitize&&window.FrontendSanitize.escapeHtml||(e=>String(e||"")),sanitizeUrl=window.FrontendSanitize&&window.FrontendSanitize.sanitizeUrl||(e=>String(e||"")),ensureImageFallbackHandler=window.FrontendSanitize&&window.FrontendSanitize.ensureImageFallbackHandler||(()=>{});function getApiUrl(){const e=CONFIG.getMainApiUrl();return console.log("Using API URL:",e),e}ensureImageFallbackHandler();let API_URL=getApiUrl();function refreshApiUrl(){const e=API_URL;API_URL=getApiUrl(),console.log("Old API URL:",e),console.log("New API URL:",API_URL),e!==API_URL&&(console.log("\u{1F504} API URL changed, clearing cache and reloading..."),typeof ApiService<"u"&&ApiService.clearCache()),fetchProducts()}let cart=JSON.parse(localStorage.getItem("sembako_cart"))||[],allProducts=[],currentCategory="Semua",currentPage=1;const itemsPerPage=12;let filteredProducts=[],storeClosed=CONFIG.isStoreClosed(),selectedVariation=null,currentModalProduct=null;function normalizePhoneNumber(e){if(!e)return null;let t=String(e).replace(/[^0-9]/g,"");return t.startsWith("62")&&(t="0"+t.substring(2)),t.startsWith("8")&&!t.startsWith("08")&&(t="0"+t),!t.startsWith("08")||t.length<10||t.length>13?null:t}function createSlug(e){return e?e.toLowerCase().replace(/[^\w\s-]/g,"").trim().replace(/[-\s]+/g,"-"):""}function ensureProductId(e,t){const n=e.id||e.sku||e.slug||createSlug(e.nama)||"product";return e.id||e.sku?String(n):`${n}-${t}`}function findProductById(e){return e&&allProducts.find(t=>String(t.productId)===String(e))||null}async function fetchProducts(){try{const e=await ApiService.get("?sheet=products",{cacheDuration:3e5});console.log("Products received:",e),allProducts=e.map((t,n)=>{const a=parseInt(t.harga)||0,o=typeof calculateGajianPrice=="function"?calculateGajianPrice(a):{price:a,daysLeft:0,markupPercent:0};let r=t.kategori||"Bahan Pokok";t.kategori||(a>=15e4?r="Paket Lengkap":a>=5e4&&(r="Paket Hemat"));let s=[];if(t.variasi)try{s=JSON.parse(t.variasi)}catch(d){console.error("Error parsing variations for product:",t.id,d)}const i=t.slug||createSlug(t.nama),l=ensureProductId({...t,slug:i},n);return{...t,slug:i,productId:l,harga:a,hargaCoret:parseInt(t.harga_coret)||0,hargaGajian:o.price,stok:parseInt(t.stok)||0,category:r,deskripsi:t.deskripsi&&t.deskripsi.trim()!==""?t.deskripsi:"Kualitas Terjamin, Stok Selalu Baru, Harga Kompetitif",variations:s}}),renderCategoryFilters(),filterProducts(),updateCartUI(),checkStoreStatus(),startNotificationLoop()}catch(e){console.error("Error fetching products:",e);const t=document.getElementById("product-grid");t&&(t.innerHTML='<p class="text-center col-span-full text-red-500">Gagal memuat produk. Silakan coba lagi nanti.</p>')}}function renderCategoryFilters(){const e=document.getElementById("category-filters");if(!e||!allProducts||allProducts.length===0)return;console.log("\u{1F3F7}\uFE0F Rendering category filters...");const t=new Set;allProducts.forEach(o=>{o.category&&o.category.trim()!==""&&t.add(o.category.trim())});const n=Array.from(t).sort();console.log("\u{1F4CA} Categories found:",n);let a='<button type="button" data-action="set-category" data-category="Semua" class="filter-btn active snap-start flex-shrink-0 px-6 py-2 rounded-full border-2 border-green-500 bg-green-50 text-green-700 text-sm font-bold transition hover:border-green-600 hover:bg-green-100">Semua</button>';n.forEach(o=>{const r=escapeHtml(o);a+=`<button type="button" data-action="set-category" data-category="${r}" class="filter-btn snap-start flex-shrink-0 px-6 py-2 rounded-full border-2 border-gray-300 bg-white text-gray-700 text-sm font-bold transition hover:border-green-500 hover:bg-green-50">${r}</button>`}),e.innerHTML=a,console.log("\u2705 Category filters rendered:",n.length,"categories"),initCategoryCarouselScroll()}function initCategoryCarouselScroll(){const e=document.getElementById("category-filters");if(!e)return;e.addEventListener("wheel",o=>{o.deltaY!==0&&(o.preventDefault(),e.scrollLeft+=o.deltaY)},{passive:!1});let t,n,a=!1;e.addEventListener("mousedown",o=>{o.target.classList.contains("filter-btn")||(a=!0,e.style.cursor="grabbing",t=o.pageX-e.offsetLeft,n=e.scrollLeft)}),e.addEventListener("mouseleave",()=>{a=!1,e.style.cursor="grab"}),e.addEventListener("mouseup",()=>{a=!1,e.style.cursor="grab"}),e.addEventListener("mousemove",o=>{if(!a)return;o.preventDefault();const r=2*(o.pageX-e.offsetLeft-t);e.scrollLeft=n-r}),e.style.cursor="grab",updateCategoryArrowsVisibility(),e.addEventListener("scroll",updateCategoryArrowsVisibility)}function scrollCategoryCarousel(e){const t=document.getElementById("category-filters");t&&(e==="left"?t.scrollLeft-=300:e==="right"&&(t.scrollLeft+=300))}function updateCategoryArrowsVisibility(){const e=document.getElementById("category-filters"),t=document.getElementById("category-scroll-left"),n=document.getElementById("category-scroll-right");if(!e||!t||!n)return;const a=e.scrollLeft<=10,o=e.scrollLeft+e.clientWidth>=e.scrollWidth-10;a?t.classList.add("opacity-0","pointer-events-none"):t.classList.remove("opacity-0","pointer-events-none"),o?n.classList.add("opacity-0","pointer-events-none"):n.classList.remove("opacity-0","pointer-events-none")}function findProductBySlug(e){return e&&allProducts&&allProducts.length!==0?allProducts.find(t=>t.slug===e):null}function renderProducts(e){const t=document.getElementById("product-grid");if(!t)return;if(e.length===0)return t.innerHTML='<p class="text-center col-span-full text-gray-500 py-10">Tidak ada produk yang ditemukan.</p>',void(document.getElementById("pagination-container").innerHTML="");const n=Math.ceil(e.length/12);currentPage>n&&(currentPage=n||1);const a=12*(currentPage-1),o=a+12,r=e.slice(a,o);t.innerHTML="",r.forEach(s=>{let i="";i=s.stok>10?'<span class="bg-green-100 text-green-700 text-[10px] px-2 py-0.5 rounded-full font-bold">Stok Tersedia</span>':s.stok>5?`<span class="bg-yellow-100 text-yellow-700 text-[10px] px-2 py-0.5 rounded-full font-bold">Stok Menipis (${s.stok})</span>`:s.stok>0?`<span class="bg-orange-100 text-orange-700 text-[10px] px-2 py-0.5 rounded-full font-bold">Hanya sisa ${s.stok}</span>`:'<span class="bg-red-100 text-red-700 text-[10px] px-2 py-0.5 rounded-full font-bold">Stok Habis</span>';const l=(s.gambar?s.gambar.split(","):[])[0]||"https://placehold.co/300x200?text=Produk",d=sanitizeUrl(l,"https://placehold.co/300x200?text=Produk"),u=calculateRewardPoints(s.harga,s.nama);let h="",p=!1;if(s.grosir)try{const y=JSON.parse(s.grosir);Array.isArray(y)&&y.length>0&&(p=!0,h=`
            ...
            <div class="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition duration-300 relative" data-product-id="${g}">
                ...
                <div class="lazy-image-wrapper">
                    ...
                    <img src="${d}" alt="${escapeHtml(s.nama)}" data-action="show-detail" data-product-id="${g}" class="w-full h-48 object-cover cursor-pointer hover:opacity-90 transition-opacity ${s.stok===0?"grayscale opacity-60":""}" loading="lazy" data-fallback-src="https://placehold.co/300x200?text=Produk" onload="this.classList.add('loaded'); this.previousElementSibling.style.display='none';">
                </div>
                <div class="p-6">
                    ...
                    `:`
                    <button data-action="add-to-cart" data-product-id="${g}" ${s.stok===0?"disabled":""} class="w-full bg-green-600 hover:bg-green-700 disabled:bg-gray-300 text-white font-bold py-3 rounded-xl transition flex items-center justify-center gap-2 mb-3">
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        ...
                        <button data-action="direct-order" data-product-id="${g}" ${s.stok===0?"disabled":""} class="bg-green-100 hover:bg-green-200 text-green-700 font-bold py-2 rounded-lg text-sm transition">Beli Sekarang</button>
                    </div>
    ...
    `).join(""),a.classList.remove("hidden")}function closeSearchSuggestions(){[document.getElementById("search-suggestions"),document.getElementById("search-suggestions-header")].forEach(e=>{e&&(e.classList.add("hidden"),e.innerHTML="")})}function getSearchSuggestions(e,t=6){const n=tokenize(e);if(n.length===0)return[];const a=allProducts.map(r=>{const s=[...tokenize(r.nama),...tokenize(r.deskripsi||"")];let i=0;return n.forEach(l=>{s.forEach(d=>{d===l?i=Math.max(i,3):d.includes(l)?i=Math.max(i,2):l.length>2&&d.length>2&&levenshtein(d,l)<=1&&(i=Math.max(i,1))})}),{name:r.nama,score:i}}).filter(r=>r.score>0),o=new Map;return a.sort((r,s)=>s.score-r.score).forEach(r=>{o.has(r.name)||o.set(r.name,r)}),Array.from(o.values()).slice(0,t).map(r=>r.name)}function levenshtein(e,t){const n=e.length,a=t.length;if(n===0)return a;if(a===0)return n;const o=Array.from({length:n+1},()=>new Array(a+1).fill(0));for(let r=0;r<=n;r+=1)o[r][0]=r;for(let r=0;r<=a;r+=1)o[0][r]=r;for(let r=1;r<=n;r+=1)for(let s=1;s<=a;s+=1){const i=e[r-1]===t[s-1]?0:1;o[r][s]=Math.min(o[r-1][s]+1,o[r][s-1]+1,o[r-1][s-1]+i)}return o[n][a]}function renderPagination(e){const t=document.getElementById("pagination-container");if(!t)return;const n=Math.ceil(e/12);if(n<=1)return void(t.innerHTML="");let a="";a+=`
    ...
    `,n.appendChild(a),document.body.appendChild(n);const r=a.querySelector('[data-action="dismiss-notification"]');if(r&&r.addEventListener("click",()=>n.remove()),!document.getElementById("success-notification-styles")){const s=document.createElement("style");s.id="success-notification-styles",s.textContent=`
        ...
        `,document.head.appendChild(s)}n.addEventListener("click",s=>{s.target===n&&(n.style.animation="fadeOut 0.3s ease-out",setTimeout(()=>n.remove(),300))})}function saveCart(){localStorage.setItem("sembako_cart",JSON.stringify(cart))}function updateCartUI(){const e=cart.reduce((r,s)=>r+s.qty,0),t=document.getElementById("cart-count");t&&(e>0?(t.innerText=e,t.classList.remove("hidden")):t.classList.add("hidden"));const n=document.getElementById("cart-items"),a=document.getElementById("cart-footer"),o=document.getElementById("cart-empty");if(n)if(cart.length===0)n.innerHTML="",a&&a.classList.add("hidden"),o&&o.classList.remove("hidden");else{o&&o.classList.add("hidden"),a&&a.classList.remove("hidden");let r=0;n.innerHTML=cart.map((d,u)=>{const h=calculateTieredPrice(d.harga,d.qty,d.grosir),p=h<d.harga,c=h*d.qty;r+=c;let m=(d.gambar?d.gambar.split(","):[])[0]||"https://placehold.co/100x100?text=Produk";return d.selectedVariation&&d.selectedVariation.gambar&&(m=d.selectedVariation.gambar),`
            ...
            `}).join("");const s=document.getElementById("cart-total");s&&(s.innerText=`Rp ${r.toLocaleString("id-ID")}`);const i=document.getElementById("order-summary-total");i&&(i.innerText=`Rp ${r.toLocaleString("id-ID")}`);const l=document.getElementById("sticky-order-total");l&&(l.innerText=`Rp ${r.toLocaleString("id-ID")}`)}}function updateQty(e,t){cart[e].qty+=t,cart[e].qty<1&&cart.splice(e,1),saveCart(),updateCartUI()}function removeItem(e){cart.splice(e,1),saveCart(),updateCartUI()}function openCartModal(){const e=document.getElementById("cart-modal");e&&(e.classList.remove("hidden"),document.body.classList.add("modal-active"))}function closeCartModal(){const e=document.getElementById("cart-modal");e&&(e.classList.add("hidden"),document.body.classList.remove("modal-active"))}function updateModalQty(e){const t=document.getElementById("modal-qty");if(!t)return;let n=parseInt(t.value)||1;n+=e,n<1&&(n=1),t.value=n,t.oninput({target:t})}function showDetail(e){closeWishlistModal(),console.log("showDetail called for product:",e.nama);const t=document.getElementById("detail-modal");if(!t)return;currentModalProduct=e,t.dataset.productId=e.productId||"",selectedVariation=null;const n=document.getElementById("modal-qty");n&&(n.value=1);const a=document.getElementById("modal-product-name"),o=document.getElementById("modal-product-image"),r=(document.getElementById("modal-cash-price"),document.getElementById("modal-gajian-price"),document.getElementById("modal-price-date")),s=document.getElementById("modal-items-list"),i=document.getElementById("modal-badges"),l=(document.getElementById("savings-highlight"),document.getElementById("savings-amount"),document.getElementById("modal-variation-container"));a&&(a.innerText=e.nama),n&&(n.oninput=u=>{const h=parseInt(u.target.value)||1,p=selectedVariation?selectedVariation.harga:e.harga,c=selectedVariation?selectedVariation.grosir:e.grosir,m=selectedVariation?selectedVariation.harga_coret||0:e.hargaCoret||0;typeof updateTieredPricingUI=="function"&&updateTieredPricingUI({...e,harga:p,grosir:c},h);const g=typeof calculateTieredPrice=="function"?calculateTieredPrice(p,h,c):p;updateModalPrices(g*h,(typeof calculateGajianPrice=="function"?calculateGajianPrice(g):{price:g}).price*h,m*h)}),l?e.variations&&e.variations.length>0?(l.classList.remove("hidden"),document.getElementById("modal-variation-list").innerHTML=e.variations.map((u,h)=>`
            ...
            `).join(""),selectVariation(e.variations[0],0)):(l.classList.add("hidden"),updateModalPrices(e.harga,e.hargaGajian,e.hargaCoret)):updateModalPrices(e.harga,e.hargaGajian,e.hargaCoret),r&&(r.innerText=`Harga Per Tgl ${new Date().toLocaleDateString("id-ID",{day:"2-digit",month:"2-digit",year:"numeric"}).replace(/\//g,"-")}`),i&&(i.innerHTML=`
        ...
        `);const d=e.gambar?e.gambar.split(","):[];if(typeof initializeSlider=="function"?initializeSlider(d):o&&(o.src=d.length>0?d[0]:"https://placehold.co/300x200?text=Produk",o.onerror=function(){this.src="https://placehold.co/300x200?text=Produk"}),s){const u=String(e.deskripsi||"").replace(/\r\n?/g,`
`).replace(/\\r\\n/g,`
`).replace(/\\n/g,`
`).split(`
`).map(p=>p.trim()).filter(p=>p!==""&&!/^isi paket\s*:\s*$/i.test(p)).map(p=>p.replace(/^[\-•]\s*/,"").trim()).filter(p=>p!==""),h=["\u{1F35C}","\u{1F372}","\u{1F4E6}","\u2615","\u{1F35A}","\u{1F373}","\u{1F9C2}"];s.innerHTML=u.map((p,c)=>`
        ...
        `).join("")}typeof updateTieredPricingUI=="function"&&updateTieredPricingUI(e,1),t.classList.remove("hidden"),document.body.classList.add("modal-active")}function selectVariation(e,t){if(selectedVariation=e,document.querySelectorAll(".variation-btn").forEach((a,o)=>{o===t?(a.classList.add("border-green-500","bg-green-50"),a.classList.remove("border-gray-200")):(a.classList.remove("border-green-500","bg-green-50"),a.classList.add("border-gray-200"))}),e.gambar&&typeof initializeSlider=="function"){const a=e.gambar.split(",");initializeSlider(a)}const n=document.getElementById("modal-qty");if(n)n.oninput({target:n});else{const a=typeof calculateGajianPrice=="function"?calculateGajianPrice(e.harga):{price:e.harga,daysLeft:0,markupPercent:0};updateModalPrices(e.harga,a.price,e.harga_coret||0)}}function updateModalPrices(e,t,n){const a=document.getElementById("modal-cash-price"),o=document.getElementById("modal-gajian-price"),r=document.getElementById("savings-highlight"),s=document.getElementById("savings-amount");a&&(a.innerText=`Rp ${e.toLocaleString("id-ID")}`),o&&(o.innerText=`Rp ${t.toLocaleString("id-ID")}`),n>e?(r&&r.classList.remove("hidden"),s&&(s.innerText=`Rp ${(n-e).toLocaleString("id-ID")}`)):r&&r.classList.add("hidden")}function closeDetailModal(){const e=document.getElementById("detail-modal");e&&(e.classList.add("hidden"),document.body.classList.remove("modal-active")),selectedVariation=null}function directOrder(e){storeClosed?showStoreWarning(()=>{proceedDirectOrder(e)}):proceedDirectOrder(e)}function proceedDirectOrder(e){if(e.variations&&e.variations.length>0&&!selectedVariation)return void showDetail(e);const t={...e};if(selectedVariation){t.selectedVariation=selectedVariation,t.harga=selectedVariation.harga,t.sku=selectedVariation.sku,t.stok=selectedVariation.stok;const o=typeof calculateGajianPrice=="function"?calculateGajianPrice(selectedVariation.harga):{price:selectedVariation.harga,daysLeft:0,markupPercent:0};t.hargaGajian=o.price}const n=document.getElementById("modal-qty"),a=n&&parseInt(n.value)||1;cart=[{...t,qty:a}],saveCart(),updateCartUI(),openOrderModal(),selectedVariation=null}function directOrderFromModal(){const e=document.getElementById("detail-modal"),t=findProductById(e?e.dataset.productId:null);t&&(directOrder(t),closeDetailModal())}function checkStoreStatus(){storeClosed=CONFIG.isStoreClosed();const e=document.getElementById("store-closed-banner"),t=document.getElementById("main-header");storeClosed?(e&&e.classList.remove("hidden"),t&&(t.style.top="36px"),sessionStorage.getItem("store_closed_modal_shown")||setTimeout(()=>{document.getElementById("store-closed-modal").classList.remove("hidden"),sessionStorage.setItem("store_closed_modal_shown","true")},1e3)):(e&&e.classList.add("hidden"),t&&(t.style.top="0"))}function closeStoreClosedModal(){document.getElementById("store-closed-modal").classList.add("hidden")}function showStoreWarning(e){const t=document.getElementById("store-warning-modal"),n=document.getElementById("confirm-store-warning");t.classList.remove("hidden"),n.onclick=()=>{t.classList.add("hidden"),e&&e()}}function closeStoreWarningModal(){document.getElementById("store-warning-modal").classList.add("hidden")}function openOrderModal(){if(cart.length===0)return;closeCartModal(),prefillCustomerInfo();const e=document.getElementById("order-summary"),t=document.querySelector('input[name="pay-method"]:checked'),n=t&&t.value==="Bayar Gajian";if(e){let o=0;e.innerHTML=cart.map(r=>{const s=n?r.hargaGajian:r.harga,i=calculateTieredPrice(s,r.qty,r.grosir),l=i<s,d=calculateRewardPoints(r.harga,r.nama)*r.qty;return o+=d,`
                <div class="flex justify-between items-center py-1">
                    ...
                    <div class="flex flex-col items-end">
                        ...
                        <span class="font-bold">Rp ${(i*r.qty).toLocaleString("id-ID")}</span>
                    </div>
                </div>
            `}).join(""),e.innerHTML+=`
...
Mohon segera diproses ya, terima kasih!`,M=`https://wa.me/628993370200?text=${encodeURIComponent(C)}`,L={id:w,pelanggan:t,produk:g.slice(0,-3),qty:c,total:p,status:"Pending",tanggal:new Date().toLocaleString("id-ID"),phone:normalizePhone(n),poin:v,point_processed:"No"};try{await logOrderToGAS(L),console.log("\u2705 Order logged to spreadsheet successfully"),cart=[],saveCart(),updateCartUI(),closeOrderModal(),showSuccessNotification(w,M)}catch(b){console.error("\u274C Error logging order:",b),alert("Gagal menyimpan pesanan. Silakan coba lagi atau hubungi admin.")}finally{e&&setTimeout(()=>{e.disabled=!1,e.innerHTML=`
            ...
            <div class="bg-white p-4 rounded-2xl border-2 border-gray-100 hover:border-green-500 transition-all group shadow-sm">
                <div class="flex gap-4">
                    <div class="w-20 h-20 bg-gray-100 rounded-xl overflow-hidden flex-shrink-0">
                        <img src="${i}" alt="${d}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" data-fallback-src="https://via.placeholder.com/100?text=Reward">
                    </div>
    ...
    `}function saveLoggedInUser(e){const t=normalizePhoneTo08(e.whatsapp||e.phone||"");localStorage.setItem("gosembako_user",JSON.stringify({id:e.id,nama:e.nama,whatsapp:t||e.whatsapp,tanggal_daftar:e.tanggal_daftar}))}function getLoggedInUser(){const e=localStorage.getItem("gosembako_user");if(!e)return null;const t=JSON.parse(e);return t.whatsapp=normalizePhoneTo08(t.whatsapp||t.phone||"")||t.whatsapp,t}function logout(){confirm("Apakah Anda yakin ingin keluar?")&&(localStorage.removeItem("gosembako_user"),window.location.reload())}async function copyReferralCode(){const e=document.getElementById("referral-code-display"),t=toReferralCodeValue(e?e.value:"");if(!t||t==="-"){setReferralStatus("Kode referral belum tersedia.","warning");return}try{if(navigator.clipboard&&navigator.clipboard.writeText)await navigator.clipboard.writeText(t);else{const n=document.createElement("textarea");n.value=t,document.body.appendChild(n),n.select(),document.execCommand("copy"),document.body.removeChild(n)}setReferralStatus("Kode referral berhasil disalin.","success")}catch(n){console.error("Copy referral failed:",n),setReferralStatus("Gagal menyalin kode referral.","error")}}async function applyReferralCode(){const e=getLoggedInUser();if(!e){setReferralStatus("Silakan login ulang.","error");return}const t=document.getElementById("referral-input"),n=document.getElementById("apply-referral-btn"),a=toReferralCodeValue(t?t.value:"");if(!a){setReferralStatus("Masukkan kode referral terlebih dahulu.","warning");return}const o=toReferralCodeValue(document.getElementById("referral-code-display")?.value);if(o&&a===o){setReferralStatus("Kode referral sendiri tidak bisa dipakai.","error");return}if(t&&t.disabled){setReferralStatus("Kode referral sudah pernah diterapkan.","warning");return}const r=n?n.textContent:"";try{n&&(n.disabled=!0,n.textContent="Memproses...");const s=await GASActions.post({action:"attach_referral",sheet:"users",data:{referee_phone:normalizePhoneTo08(e.whatsapp),ref_code:a}});if(!s||s.success===!1){const i=s&&(s.message||s.error||s.error_code)||"Gagal menerapkan referral";setReferralStatus(i,"error"),n&&(n.disabled=!1,n.textContent=r||"Terapkan");return}t&&(t.value=a,t.disabled=!0),n&&(n.disabled=!0,n.textContent="Terapkan"),setReferralStatus("Kode referral berhasil diterapkan.","success"),await loadReferralData(e)}catch(s){console.error("applyReferralCode error:",s),setReferralStatus(s.message||"Gagal menerapkan kode referral.","error"),n&&(n.disabled=!1,n.textContent=r||"Terapkan")}}async function loadLoyaltyPoints(e){try{const t=CONFIG.getMainApiUrl(),n=normalizePhoneTo08(e.whatsapp);if(!n){console.warn("\u26A0\uFE0F Invalid phone number for loyalty points lookup"),document.getElementById("loyalty-points").textContent="0";return}console.log(`\u{1F50D} Loading loyalty points for phone: ${n}`);const a=await fetch(`${t}?sheet=user_points`);if(!a.ok){console.error("\u274C Failed to fetch loyalty points"),document.getElementById("loyalty-points").textContent="0";return}const o=await a.json();console.log("\u{1F4E5} Points data received:",o);let r=Array.isArray(o)?o:o.result||[];if(!Array.isArray(r)){console.warn("\u26A0\uFE0F Unexpected points data format"),document.getElementById("loyalty-points").textContent="0";return}const s=phoneLookupVariants(n);let i=null;for(const l of s)if(i=r.find(d=>normalizePhoneTo08(d.phone||d.whatsapp||"")===normalizePhoneTo08(l)),i){console.log(`\u2705 Found points record for ${l}:`,i);break}if(i){const l=parseInt(i.points||i.poin||0);console.log(`\u2705 User points: ${l}`),document.getElementById("loyalty-points").textContent=l,setRewardContextFromAkun(e.whatsapp,l,e.nama)}else console.log("\u26A0\uFE0F No points record found for user"),document.getElementById("loyalty-points").textContent="0",setRewardContextFromAkun(e.whatsapp,0,e.nama)}catch(t){console.error("\u274C Error loading loyalty points:",t),document.getElementById("loyalty-points").textContent="0"}}async function loadOrderHistory(e){const t=document.getElementById("order-loading"),n=document.getElementById("order-empty"),a=document.getElementById("order-list"),o=document.getElementById("total-accepted-spend");t.classList.remove("hidden"),n.classList.add("hidden"),a.innerHTML="",o&&(o.textContent="Rp 0");try{const r=CONFIG.getMainApiUrl();let s=await fetch(`${r}?sheet=orders`);if(!s.ok)throw new Error("Gagal memuat riwayat pesanan");let i=await s.json(),l=Array.isArray(i)?i.filter(d=>normalizePhoneTo08(d.phone)===normalizePhoneTo08(e.whatsapp)):[];if(t.classList.add("hidden"),!l||l.length===0){n.classList.remove("hidden");return}if(l=l.filter(d=>{const u=normalizePhoneTo08(d.phone||d.whatsapp||""),h=normalizePhoneTo08(e.whatsapp);return u===h}),l.length===0){n.classList.remove("hidden");return}if(l.sort((d,u)=>{const h=new Date(d.tanggal_pesanan||d.timestamp||0);return new Date(u.tanggal_pesanan||u.timestamp||0)-h}),window.allOrders=l,window.currentPage=1,window.ordersPerPage=10,o){const d=new Set(["terima","diterima","selesai"]),u=l.reduce((h,p)=>{const c=String(p.status||"").toLowerCase().trim();return d.has(c)?h+parseCurrencyValue(p.total||p.total_bayar||0):h},0);o.textContent=formatCurrency(u)}displayOrderPage(1)}catch(r){console.error("Error loading order history:",r),t.classList.add("hidden"),a.innerHTML=`
        ...
        <div class="border-t border-gray-100 pt-2 space-y-1.5">
            <div class="text-sm">
                ...
                <p class="font-semibold text-gray-800 lg:hidden">${escapeHtml(i)}</p>
...
Untuk sementara, hubungi admin untuk reset PIN.`);const d=r.replace(/^0/,"62");window.open(`https://wa.me/628993370200?text=Halo, saya ingin reset PIN akun saya. Nomor WhatsApp: ${d}`,"_blank"),setTimeout(()=>{showLogin()},1e3)}catch(s){console.error("Forgot PIN error:",s),a.textContent="Terjadi kesalahan. Silakan coba lagi.",n.classList.remove("hidden")}finally{o.disabled=!1,o.innerHTML='<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg> Kirim Kode Verifikasi'}});function openEditProfile(){const e=getLoggedInUser();e&&(document.getElementById("edit-name").value=e.nama,document.getElementById("edit-old-pin").value="",document.getElementById("edit-new-pin").value="",document.getElementById("edit-confirm-pin").value="",document.getElementById("edit-error").classList.add("hidden"),document.getElementById("edit-profile-modal").classList.remove("hidden"))}function closeEditProfile(){document.getElementById("edit-profile-modal").classList.add("hidden")}document.getElementById("edit-profile-form").addEventListener("submit",async e=>{e.preventDefault();const t=getLoggedInUser();if(!t)return;const n=document.getElementById("edit-name").value.trim(),a=document.getElementById("edit-old-pin").value.trim(),o=document.getElementById("edit-new-pin").value.trim(),r=document.getElementById("edit-confirm-pin").value.trim(),s=document.getElementById("edit-error"),i=document.getElementById("edit-error-text"),l=document.getElementById("edit-save-btn");if(s.classList.add("hidden"),n.replace(/\s/g,"").length<4){i.textContent="Masukkan Nama Lengkap",s.classList.remove("hidden");return}if(a||o||r){if(!a||!o||!r){i.textContent="Lengkapi semua field PIN",s.classList.remove("hidden");return}if(o.length!==6||!/^\d{6}$/.test(o)){i.textContent="PIN baru harus 6 digit angka",s.classList.remove("hidden");return}if(o!==r){i.textContent="PIN baru tidak cocok",s.classList.remove("hidden");return}}l.disabled=!0,l.textContent="Menyimpan...";try{const d=CONFIG.getMainApiUrl(),u=await(await fetch(`${d}?sheet=users&id=${t.id}`)).json();if(!u||u.length===0)throw new Error("User not found");const h=u[0];if(a&&h.pin!==a){i.textContent="PIN lama salah",s.classList.remove("hidden"),l.disabled=!1,l.textContent="Simpan";return}const p={nama:n};o&&(p.pin=o);const c=await GASActions.update("users",t.id,p);if(!c.affected||c.affected<1)throw new Error("Failed to update");t.nama=n,saveLoggedInUser(t),document.getElementById("user-name").textContent=n,closeEditProfile(),alert("Profil berhasil diperbarui!")}catch(d){console.error("Edit profile error:",d),i.textContent="Terjadi kesalahan. Silakan coba lagi.",s.classList.remove("hidden")}finally{l.disabled=!1,l.textContent="Simpan"}});function showOrderDetailModal(e){document.getElementById("tracking-order-id").textContent=e.id||"N/A";const t=e.tanggal||e.tanggal_pesanan||e.timestamp||e.date||e.created_at;document.getElementById("tracking-order-date").textContent=formatDate(t);const n=normalizeStatus(e.status||"Menunggu"),a=getStatusBadge(n),o=document.getElementById("tracking-status-badge");o&&(o.innerHTML=a),document.getElementById("tracking-products").textContent=e.produk||e.items||e.product_name||"N/A",document.getElementById("tracking-total").textContent=formatCurrency(e.total||e.total_bayar||0);const r=createAnimatedTimeline(n);document.getElementById("tracking-timeline").innerHTML=r,document.getElementById("order-tracking-modal").classList.remove("hidden")}function createAnimatedTimeline(e){const t=[{name:"Menunggu",gif:"wait.gif",key:"Menunggu"},{name:"Diproses",gif:"grocery-basket.gif",key:"Diproses"},{name:"Dikirim",gif:"grocery.gif",key:"Dikirim"},{name:"Diterima",gif:"shipping.gif",key:"Diterima"}];let n=t.findIndex(a=>a.key===e);return e==="Dibatalkan"?`
        ...
        <div class="flex items-center justify-between gap-2 py-2">
            ${t.map((a,o)=>{const r=o<=n,s=o===n,i=o===t.length-1,l=`./assets/images/${a.gif}`,d=sanitizeUrl(l,"./assets/images/wait.gif");return`
                    <div class="flex flex-col items-center flex-1">
                        <div class="${r?"bg-green-50":"bg-gray-100"} w-12 h-12 rounded-full flex items-center justify-center ${s?"ring-2 ring-green-500 ring-offset-2":""}">
                            <img src="${d}" alt="${a.name}" class="w-8 h-8 object-contain ${r?"":"opacity-40"}" data-fallback-src="${l}">
                        </div>
            ...
            <div class="border-2 border-gray-200 rounded-xl p-4 hover:border-amber-500 transition">
                <div class="flex gap-3 mb-3">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-xs text-gray-500 line-clamp-2">${d}</p>
                    </div>
