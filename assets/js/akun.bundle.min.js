                ...
                <div class="flex items-center gap-4 bg-gray-50 p-3 rounded-xl">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="flex flex-col">
                            ...
                            <p class="text-green-600 font-bold text-xs">Rp ${h.toLocaleString("id-ID")} ${m?'<span class="bg-green-100 text-green-700 text-[8px] px-1 rounded ml-1">Grosir</span>':""}</p>
                        </div>
...
`).replace(/\\n/g,`
`).split(`
`).map(m=>m.trim()).filter(m=>m!==""&&!/^isi paket\s*:\s*$/i.test(m)).map(m=>m.replace(/^[\-•]\s*/,"").trim()).filter(m=>m!==""),h=["\u{1F35C}","\u{1F372}","\u{1F4E6}","\u2615","\u{1F35A}","\u{1F373}","\u{1F9C2}"];s.innerHTML=g.map((m,c)=>`
...
Saldo poin saat ini: ${s.toFixed(1)}`;if(!confirm(d))return;let l=prompt("Silakan masukkan nama Anda untuk klaim ini:",sessionStorage.getItem("reward_customer_name")||"");if(l===null)return;l=l.trim()||"Pelanggan",showToast("Sedang memproses penukaran...");const i=`RW-${Date.now()}-${Math.floor(1e5*Math.random())}`,g=await GASActions.post({action:"claim_reward",sheet:"tukar_poin",data:{reward_id:e,phone:t,customer_name:l,request_id:i}}),h=g.claim_id||"CLM-"+Date.now().toString().slice(-6),m=parseFloat(g.points_used||o)||0,c=parseFloat(g.balance_after),u=Number.isNaN(c)?Math.max(0,s-m):c;sessionStorage.setItem("user_points",u);const p=document.querySelector("#points-display h4");p&&(p.innerHTML=`${escapeHtml(u.toFixed(1))} <span class="text-sm font-bold">Poin</span>`);const f=`*KLAIM REWARD POIN BERHASIL*
...
Mohon segera diproses. Terima kasih!`,f=`https://wa.me/628993370200?text=${encodeURIComponent(p)}`;window.claimWhatsAppUrl=f,pendingRewardData={id:null,nama:null,poin:null,gambar:null,deskripsi:null},document.getElementById("name-input-modal").classList.add("hidden"),document.getElementById("confirm-tukar-modal").classList.add("hidden"),setTimeout(()=>{showClaimSuccessModal(g)},300)}catch(a){console.error("Error processing claim:",a),showToast(buildClaimRewardErrorMessage(a))}}function buildClaimRewardErrorMessage(e){const t=String(e&&e.message||e||"").toLowerCase();return t.includes("reward_stock_empty")?"Stok reward sedang habis.":t.includes("reward_daily_quota_reached")?"Quota harian reward sudah habis.":t.includes("points_insufficient")?"Poin Anda tidak cukup untuk reward ini.":t.includes("user_not_found")?"Data poin pengguna tidak ditemukan.":t.includes("rate_limited")?"Terlalu banyak percobaan klaim, coba lagi sebentar.":"Gagal memproses penukaran. Silakan coba lagi."}const WISHLIST_KEY="gos_wishlist";function getWishlist(){const e=localStorage.getItem(WISHLIST_KEY);return e?JSON.parse(e):[]}function saveWishlist(e){localStorage.setItem(WISHLIST_KEY,JSON.stringify(e)),updateWishlistCount()}function toggleWishlist(e){let t=getWishlist();const n=t.indexOf(e);n>-1?(t.splice(n,1),showToast("Produk dihapus dari Wishlist.")):(t.push(e),showToast("Produk ditambahkan ke Wishlist!")),saveWishlist(t),updateProductWishlistIcon(e);const a=document.getElementById("wishlist-modal");a&&!a.classList.contains("hidden")&&renderWishlistItems()}function isProductInWishlist(e){return getWishlist().includes(e)}function updateWishlistCount(){const e=getWishlist().length,t=document.getElementById("wishlist-count");t&&(t.textContent=e,e>0?t.classList.remove("hidden"):t.classList.add("hidden"))}function updateProductWishlistIcon(e){const t=document.getElementById(`wishlist-btn-${e}`);if(!t)return;const n=isProductInWishlist(e)?'<svg class="w-5 h-5 text-red-500 fill-current" viewBox="0 0 24 24"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg>':'<svg class="w-5 h-5 text-gray-400 hover:text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/></svg>';t.innerHTML=n}function openWishlistModal(){const e=document.getElementById("wishlist-modal");e&&(e.classList.remove("hidden"),document.body.classList.add("modal-active"),renderWishlistItems())}function closeWishlistModal(){const e=document.getElementById("wishlist-modal");e&&(e.classList.add("hidden"),document.body.classList.remove("modal-active"))}async function renderWishlistItems(){const e=getWishlist(),t=document.getElementById("wishlist-items-container");if(console.log("\u{1F50D} Rendering wishlist items:",{wishlistIds:e,containerFound:!!t}),!t)return;if(e.length===0)return void(t.innerHTML=`
    ...
    `}function selectSidebarCategory(e){setCategory(e),closeCategorySidebar();const t=document.getElementById("products");t&&t.scrollIntoView({behavior:"smooth",block:"start"})}function updateMobileCartCount(){const e=document.getElementById("mobile-cart-count"),t=document.getElementById("cart-count");if(e&&t){const n=cart.length;n>0?(e.textContent=n,e.classList.remove("hidden")):e.classList.add("hidden")}}document.addEventListener("DOMContentLoaded",updateWishlistCount),window.addEventListener("hashchange",handleDeepLink,!1),document.addEventListener("DOMContentLoaded",()=>{setTimeout(handleDeepLink,1e3)});const originalUpdateCartUI=updateCartUI;updateCartUI=function(){typeof originalUpdateCartUI=="function"&&originalUpdateCartUI(),updateMobileCartCount()},document.addEventListener("DOMContentLoaded",function(){updateMobileCartCount()});const normalizePhoneTo08=e=>{const t=String(e??"").replace(/[^0-9]/g,"");if(!t)return"";let n=t;return n.startsWith("62")&&(n=n.slice(2)),n.startsWith("0")&&(n=n.slice(1)),n.startsWith("8")?"0"+n:""},phoneLookupVariants=e=>{const t=normalizePhoneTo08(e);if(!t)return[];const n=t.slice(1);return[t,`+62${n}`,`62${n}`,n]},displayPhone=e=>normalizePhoneTo08(e)||e||"",parseSheetResponse=e=>Array.isArray(e)?e:e&&Array.isArray(e.result)?e.result:e&&e.result?Array.isArray(e.result)?e.result:[e.result]:[],isUnauthorizedApiResponse=e=>!!(e&&typeof e=="object"&&!Array.isArray(e)&&String(e.error||"").toLowerCase()==="unauthorized");let referralProfileCache=null;function toReferralCodeValue(e){return String(e||"").trim().toUpperCase()}function setReferralStatus(e,t){const n=document.getElementById("referral-status");n&&(n.textContent=e||"",n.className="text-xs",t==="success"?n.classList.add("text-green-600"):t==="error"?n.classList.add("text-red-600"):t==="warning"?n.classList.add("text-amber-600"):n.classList.add("text-gray-500"))}function generateReferralCode(e,t,n){const a=String(e||"USER").toUpperCase().replace(/[^A-Z0-9]/g,"").slice(0,6)||"USER",o=normalizePhoneTo08(t).replace(/\D/g,"").slice(-4)||Math.floor(1e3+Math.random()*9e3).toString();let r=`${a}${o}`.slice(0,24);if(!n.has(r))return r;for(let s=1;s<=2e3;s+=1){const d=`${a}${o}${s}`.slice(0,24);if(!n.has(d))return d}return`${a}${Date.now().toString().slice(-6)}`.slice(0,24)}async function fetchUsersList(){const e=CONFIG.getMainApiUrl(),t=await fetch(`${e}?sheet=users&_t=${Date.now()}`);if(!t.ok)throw new Error("Gagal memuat data users");return parseSheetResponse(await t.json())}async function ensureUserReferralCode(e,t){const n=t.find(s=>String(s.id)===String(e.id))||t.find(s=>normalizePhoneTo08(s.whatsapp||s.phone||"")===normalizePhoneTo08(e.whatsapp));if(!n)return{user:{id:e.id||"",nama:e.nama||"User",whatsapp:normalizePhoneTo08(e.whatsapp||e.phone||""),kode_referral:"",referred_by:"",referral_count:0,referral_points_total:0},code:"",missingProfile:!0};const a=toReferralCodeValue(n.kode_referral);if(a)return{user:n,code:a};const o=new Set(t.map(s=>toReferralCodeValue(s.kode_referral)).filter(Boolean)),r=generateReferralCode(n.nama,n.whatsapp||n.phone,o);return await GASActions.update("users",n.id,{kode_referral:r}),n.kode_referral=r,{user:n,code:r}}function applyReferralDataToUI(e){const t=document.getElementById("referral-code-display"),n=document.getElementById("referral-input"),a=document.getElementById("apply-referral-btn"),o=document.getElementById("referral-count"),r=document.getElementById("referral-points-total");t&&(t.value=toReferralCodeValue(e.kode_referral)||"-"),o&&(o.textContent=String(parseInt(e.referral_count||0,10)||0)),r&&(r.textContent=String(parseInt(e.referral_points_total||0,10)||0));const s=toReferralCodeValue(e.referred_by)!=="";n&&(n.value=s?toReferralCodeValue(e.referred_by):"",n.disabled=s),a&&(a.disabled=s),s?setReferralStatus(`Referral aktif dengan kode: ${toReferralCodeValue(e.referred_by)}`,"success"):setReferralStatus("Kode referral hanya bisa dipakai satu kali.","info")}function buildReferralProfileFallback(e){return{id:e&&e.id?e.id:"",nama:e&&e.nama?e.nama:"User",whatsapp:normalizePhoneTo08(e&&(e.whatsapp||e.phone)||""),kode_referral:toReferralCodeValue(e&&e.kode_referral?e.kode_referral:""),referred_by:toReferralCodeValue(e&&e.referred_by?e.referred_by:""),referral_count:parseInt(e&&e.referral_count||0,10)||0,referral_points_total:parseInt(e&&e.referral_points_total||0,10)||0}}function getReferralStatusBadge(e){const t=String(e||"").toLowerCase();return t==="approved"?'<span class="px-2 py-0.5 rounded-full bg-green-100 text-green-700 font-bold">Approved</span>':t==="rejected"||t==="void"?'<span class="px-2 py-0.5 rounded-full bg-red-100 text-red-700 font-bold">Rejected</span>':'<span class="px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 font-bold">Pending</span>'}async function loadReferralHistory(e){const t=document.getElementById("referral-history-list"),n=document.getElementById("referral-history-badge");if(!(!t||!n))try{const a=CONFIG.getMainApiUrl(),o=normalizePhoneTo08(e.whatsapp),r=await fetch(`${a}?sheet=referrals&phone=${encodeURIComponent(o)}&_t=${Date.now()}`);if(!r.ok)throw new Error("Gagal memuat riwayat referral");const s=parseSheetResponse(await r.json()).filter(d=>{const l=normalizePhoneTo08(d.referrer_phone||""),i=normalizePhoneTo08(d.referee_phone||"");return l===o||i===o}).sort((d,l)=>new Date(l.created_at||0).getTime()-new Date(d.created_at||0).getTime()).slice(0,5);if(n.textContent=`${s.length} data`,s.length===0){t.innerHTML='<p class="text-gray-400">Belum ada riwayat referral.</p>';return}t.innerHTML=s.map(d=>{const l=normalizePhoneTo08(d.referrer_phone||"")===o?"Anda mengajak":"Anda diajak",i=normalizePhoneTo08(l==="Anda mengajak"?d.referee_phone||"":d.referrer_phone||""),g=l==="Anda mengajak"?parseInt(d.reward_referrer_points||0,10)||0:parseInt(d.reward_referee_points||0,10)||0;return`
            ...
            `}).join("")}catch(a){console.error("Error loading referral history:",a),n.textContent="error",t.innerHTML='<p class="text-red-500">Gagal memuat riwayat referral.</p>'}}async function loadReferralData(e){try{const t=await fetchUsersList(),n=await ensureUserReferralCode(e,t);if(referralProfileCache=n.user,applyReferralDataToUI(n.user),n.missingProfile){applyReferralDataToUI(buildReferralProfileFallback(e)),setReferralStatus("Kode referral hanya bisa dipakai satu kali.","info");return}await loadReferralHistory(e)}catch(t){console.error("Error loading referral data:",t),applyReferralDataToUI(buildReferralProfileFallback(e)),setReferralStatus("Kode referral hanya bisa dipakai satu kali.","info")}}document.addEventListener("DOMContentLoaded",()=>{const e=getLoggedInUser(),t=document.getElementById("referral-input");t&&t.addEventListener("keydown",n=>{n.key==="Enter"&&(n.preventDefault(),applyReferralCode())}),e?showDashboard(e):showLogin()});function showLogin(){document.getElementById("login-section").classList.remove("hidden"),document.getElementById("register-section").classList.add("hidden"),document.getElementById("forgot-pin-section").classList.add("hidden"),document.getElementById("dashboard-section").classList.add("hidden")}function showDashboard(e){document.getElementById("login-section").classList.add("hidden"),document.getElementById("register-section").classList.add("hidden"),document.getElementById("forgot-pin-section").classList.add("hidden"),document.getElementById("dashboard-section").classList.remove("hidden"),document.getElementById("user-name").textContent=e.nama,document.getElementById("user-whatsapp").textContent=displayPhone(e.whatsapp),loadLoyaltyPoints(e),loadReferralData(e),loadOrderHistory(e)}document.getElementById("login-form").addEventListener("submit",async e=>{e.preventDefault();const t=document.getElementById("login-whatsapp").value.trim(),n=document.getElementById("login-pin").value.trim(),a=document.getElementById("login-btn"),o=document.getElementById("login-error"),r=document.getElementById("login-error-text");if(!t||!n){showError("Mohon lengkapi semua field");return}const s=normalizePhoneTo08(t);if(!s){showError("Gunakan format 08xxxxxxxxxx");return}if(s.length<10||s.length>13){showError("Panjang nomor tidak valid");return}if(n.length!==6){showError("PIN harus 6 digit");return}a.disabled=!0,a.innerHTML=`
        <svg class="animate-spin h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
        </svg>
        <span>Memproses...</span>
    `,o.classList.add("hidden");try{const d=CONFIG.getMainApiUrl(),l="&_t="+Date.now();let i=null;const g=`${d}?action=public_login&phone=${encodeURIComponent(s)}&pin=${encodeURIComponent(n)}${l}`,h=await fetch(g);if(h.ok){const m=await h.json();if(m&&m.success&&m.user)i=m.user;else{const c=String(m&&m.error?m.error:"").toLowerCase();if(c==="login_failed"){showError("Nomor WhatsApp atau PIN salah"),resetLoginButton();return}if(c==="account_inactive"){showError("Akun Anda tidak aktif. Hubungi admin."),resetLoginButton();return}if(c==="rate_limited"){showError(m&&m.message||"Terlalu banyak percobaan login, coba lagi."),resetLoginButton();return}if(!(c==="invalid sheet"||c==="invalid action")){showError(m&&m.message||"Login gagal. Silakan coba lagi."),resetLoginButton();return}}}if(!i){const m=phoneLookupVariants(s),c=await fetch(`${d}?sheet=users${l}`);if(!c.ok){showError("Gagal menghubungi server. Silakan coba lagi."),resetLoginButton();return}const u=await c.json();if(isUnauthorizedApiResponse(u)){showError("Layanan login membutuhkan endpoint public_login. Hubungi admin."),resetLoginButton();return}const p=parseSheetResponse(u);for(const f of m){const y=p.find(x=>normalizePhoneTo08(x.whatsapp||x.phone||"")===normalizePhoneTo08(f));if(y){i=y;break}}if(!i){showError("Nomor WhatsApp tidak terdaftar"),resetLoginButton();return}if((i.pin||"").toString()!==n){showError("PIN salah. Silakan coba lagi."),resetLoginButton();return}}if(i.status&&i.status.toLowerCase()!=="aktif"){showError("Akun Anda tidak aktif. Hubungi admin."),resetLoginButton();return}i.whatsapp=normalizePhoneTo08(i.whatsapp||i.phone||s),saveLoggedInUser(i),showDashboard(i),document.getElementById("dashboard-section").scrollIntoView({behavior:"smooth",block:"start"})}catch(d){console.error("Login error:",d),showError("Terjadi kesalahan. Silakan coba lagi."),resetLoginButton()}});function showError(e){const t=document.getElementById("login-error"),n=document.getElementById("login-error-text");n.textContent=e,t.classList.remove("hidden")}function resetLoginButton(){const e=document.getElementById("login-btn");e.disabled=!1,e.innerHTML=`
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1"></path>
        </svg>
        Masuk
    `}function saveLoggedInUser(e){const t=normalizePhoneTo08(e.whatsapp||e.phone||"");localStorage.setItem("gosembako_user",JSON.stringify({id:e.id,nama:e.nama,whatsapp:t||e.whatsapp,tanggal_daftar:e.tanggal_daftar,status:e.status||"",total_points:parseInt(e.total_points||e.points||e.poin||0,10)||0,kode_referral:toReferralCodeValue(e.kode_referral||""),referred_by:toReferralCodeValue(e.referred_by||""),referral_count:parseInt(e.referral_count||0,10)||0,referral_points_total:parseInt(e.referral_points_total||0,10)||0}))}function getLoggedInUser(){const e=localStorage.getItem("gosembako_user");if(!e)return null;const t=JSON.parse(e);return t.whatsapp=normalizePhoneTo08(t.whatsapp||t.phone||"")||t.whatsapp,t}function logout(){confirm("Apakah Anda yakin ingin keluar?")&&(localStorage.removeItem("gosembako_user"),window.location.reload())}async function copyReferralCode(){const e=document.getElementById("referral-code-display"),t=toReferralCodeValue(e?e.value:"");if(!t||t==="-"){setReferralStatus("Kode referral belum tersedia.","warning");return}try{if(navigator.clipboard&&navigator.clipboard.writeText)await navigator.clipboard.writeText(t);else{const n=document.createElement("textarea");n.value=t,document.body.appendChild(n),n.select(),document.execCommand("copy"),document.body.removeChild(n)}setReferralStatus("Kode referral berhasil disalin.","success")}catch(n){console.error("Copy referral failed:",n),setReferralStatus("Gagal menyalin kode referral.","error")}}async function applyReferralCode(){const e=getLoggedInUser();if(!e){setReferralStatus("Silakan login ulang.","error");return}const t=document.getElementById("referral-input"),n=document.getElementById("apply-referral-btn"),a=toReferralCodeValue(t?t.value:"");if(!a){setReferralStatus("Masukkan kode referral terlebih dahulu.","warning");return}const o=toReferralCodeValue(document.getElementById("referral-code-display")?.value);if(o&&a===o){setReferralStatus("Kode referral sendiri tidak bisa dipakai.","error");return}if(t&&t.disabled){setReferralStatus("Kode referral sudah pernah diterapkan.","warning");return}const r=n?n.textContent:"";try{n&&(n.disabled=!0,n.textContent="Memproses...");const s=await GASActions.post({action:"attach_referral",sheet:"users",data:{referee_phone:normalizePhoneTo08(e.whatsapp),ref_code:a}});if(!s||s.success===!1){const d=s&&(s.message||s.error||s.error_code)||"Gagal menerapkan referral";setReferralStatus(d,"error"),n&&(n.disabled=!1,n.textContent=r||"Terapkan");return}t&&(t.value=a,t.disabled=!0),n&&(n.disabled=!0,n.textContent="Terapkan"),setReferralStatus("Kode referral berhasil diterapkan.","success"),await loadReferralData(e)}catch(s){console.error("applyReferralCode error:",s),setReferralStatus(s.message||"Gagal menerapkan kode referral.","error"),n&&(n.disabled=!1,n.textContent=r||"Terapkan")}}async function loadLoyaltyPoints(e){try{const t=CONFIG.getMainApiUrl(),n=normalizePhoneTo08(e.whatsapp),a=parseInt(e.total_points||e.points||e.poin||0,10)||0;if(!n){console.warn("\u26A0\uFE0F Invalid phone number for loyalty points lookup"),document.getElementById("loyalty-points").textContent=String(a),setRewardContextFromAkun(e.whatsapp,a,e.nama);return}console.log(`\u{1F50D} Loading loyalty points for phone: ${n}`);const o=await fetch(`${t}?sheet=user_points`);if(!o.ok){console.error("\u274C Failed to fetch loyalty points"),document.getElementById("loyalty-points").textContent="0";return}const r=await o.json();if(console.log("\u{1F4E5} Points data received:",r),r&&typeof r=="object"&&String(r.error||"").toLowerCase()==="unauthorized"){console.warn("\u26A0\uFE0F Unauthorized access to user_points. Falling back to user profile points."),document.getElementById("loyalty-points").textContent=String(a),setRewardContextFromAkun(e.whatsapp,a,e.nama);return}let s=Array.isArray(r)?r:r.result||[];if(!Array.isArray(s)){console.warn("\u26A0\uFE0F Unexpected points data format"),document.getElementById("loyalty-points").textContent="0";return}const d=phoneLookupVariants(n);let l=null;for(const i of d)if(l=s.find(g=>normalizePhoneTo08(g.phone||g.whatsapp||"")===normalizePhoneTo08(i)),l){console.log(`\u2705 Found points record for ${i}:`,l);break}if(l){const i=parseInt(l.points||l.poin||0);console.log(`\u2705 User points: ${i}`),document.getElementById("loyalty-points").textContent=i,setRewardContextFromAkun(e.whatsapp,i,e.nama)}else console.log("\u26A0\uFE0F No points record found for user"),document.getElementById("loyalty-points").textContent=String(a),setRewardContextFromAkun(e.whatsapp,a,e.nama)}catch(t){console.error("\u274C Error loading loyalty points:",t);const n=parseInt(e.total_points||e.points||e.poin||0,10)||0;document.getElementById("loyalty-points").textContent=String(n),setRewardContextFromAkun(e.whatsapp,n,e.nama)}}async function loadOrderHistory(e){const t=document.getElementById("order-loading"),n=document.getElementById("order-empty"),a=document.getElementById("order-list"),o=document.getElementById("total-accepted-spend");t.classList.remove("hidden"),n.classList.add("hidden"),a.innerHTML="",o&&(o.textContent="Rp 0");try{const r=CONFIG.getMainApiUrl();let s=await fetch(`${r}?sheet=orders`);if(!s.ok)throw new Error("Gagal memuat riwayat pesanan");let d=await s.json(),l=Array.isArray(d)?d.filter(i=>normalizePhoneTo08(i.phone)===normalizePhoneTo08(e.whatsapp)):[];if(t.classList.add("hidden"),!l||l.length===0){n.classList.remove("hidden");return}if(l=l.filter(i=>{const g=normalizePhoneTo08(i.phone||i.whatsapp||""),h=normalizePhoneTo08(e.whatsapp);return g===h}),l.length===0){n.classList.remove("hidden");return}if(l.sort((i,g)=>{const h=new Date(i.tanggal_pesanan||i.timestamp||0);return new Date(g.tanggal_pesanan||g.timestamp||0)-h}),window.allOrders=l,window.currentPage=1,window.ordersPerPage=10,o){const i=new Set(["terima","diterima","selesai"]),g=l.reduce((h,m)=>{const c=String(m.status||"").toLowerCase().trim();return i.has(c)?h+parseCurrencyValue(m.total||m.total_bayar||0):h},0);o.textContent=formatCurrency(g)}displayOrderPage(1)}catch(r){console.error("Error loading order history:",r),t.classList.add("hidden"),a.innerHTML=`
        ...
        <div class="border-t border-gray-100 pt-2 space-y-1.5">
            <div class="text-sm">
                ...
                <p class="font-semibold text-gray-800 hidden lg:block">${escapeHtml(s)}</p>
            </div>
...
Untuk sementara, hubungi admin untuk reset PIN.`);const i=r.replace(/^0/,"62");window.open(`https://wa.me/628993370200?text=Halo, saya ingin reset PIN akun saya. Nomor WhatsApp: ${i}`,"_blank"),setTimeout(()=>{showLogin()},1e3)}catch(s){console.error("Forgot PIN error:",s),a.textContent="Terjadi kesalahan. Silakan coba lagi.",n.classList.remove("hidden")}finally{o.disabled=!1,o.innerHTML='<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg> Kirim Kode Verifikasi'}});function openEditProfile(){const e=getLoggedInUser();e&&(document.getElementById("edit-name").value=e.nama,document.getElementById("edit-old-pin").value="",document.getElementById("edit-new-pin").value="",document.getElementById("edit-confirm-pin").value="",document.getElementById("edit-error").classList.add("hidden"),document.getElementById("edit-profile-modal").classList.remove("hidden"))}function closeEditProfile(){document.getElementById("edit-profile-modal").classList.add("hidden")}document.getElementById("edit-profile-form").addEventListener("submit",async e=>{e.preventDefault();const t=getLoggedInUser();if(!t)return;const n=document.getElementById("edit-name").value.trim(),a=document.getElementById("edit-old-pin").value.trim(),o=document.getElementById("edit-new-pin").value.trim(),r=document.getElementById("edit-confirm-pin").value.trim(),s=document.getElementById("edit-error"),d=document.getElementById("edit-error-text"),l=document.getElementById("edit-save-btn");if(s.classList.add("hidden"),n.replace(/\s/g,"").length<4){d.textContent="Masukkan Nama Lengkap",s.classList.remove("hidden");return}if(a||o||r){if(!a||!o||!r){d.textContent="Lengkapi semua field PIN",s.classList.remove("hidden");return}if(o.length!==6||!/^\d{6}$/.test(o)){d.textContent="PIN baru harus 6 digit angka",s.classList.remove("hidden");return}if(o!==r){d.textContent="PIN baru tidak cocok",s.classList.remove("hidden");return}}l.disabled=!0,l.textContent="Menyimpan...";try{const i=CONFIG.getMainApiUrl(),g=await(await fetch(`${i}?sheet=users&id=${t.id}`)).json();if(!g||g.length===0)throw new Error("User not found");const h=g[0];if(a&&h.pin!==a){d.textContent="PIN lama salah",s.classList.remove("hidden"),l.disabled=!1,l.textContent="Simpan";return}const m={nama:n};o&&(m.pin=o);const c=await GASActions.update("users",t.id,m);if(!c.affected||c.affected<1)throw new Error("Failed to update");t.nama=n,saveLoggedInUser(t),document.getElementById("user-name").textContent=n,closeEditProfile(),alert("Profil berhasil diperbarui!")}catch(i){console.error("Edit profile error:",i),d.textContent="Terjadi kesalahan. Silakan coba lagi.",s.classList.remove("hidden")}finally{l.disabled=!1,l.textContent="Simpan"}});function showOrderDetailModal(e){document.getElementById("tracking-order-id").textContent=e.id||"N/A";const t=e.tanggal||e.tanggal_pesanan||e.timestamp||e.date||e.created_at;document.getElementById("tracking-order-date").textContent=formatDate(t);const n=normalizeStatus(e.status||"Menunggu"),a=getStatusBadge(n),o=document.getElementById("tracking-status-badge");o&&(o.innerHTML=a),document.getElementById("tracking-products").textContent=e.produk||e.items||e.product_name||"N/A",document.getElementById("tracking-total").textContent=formatCurrency(e.total||e.total_bayar||0);const r=createAnimatedTimeline(n);document.getElementById("tracking-timeline").innerHTML=r,document.getElementById("order-tracking-modal").classList.remove("hidden")}function createAnimatedTimeline(e){const t=[{name:"Menunggu",gif:"wait.gif",key:"Menunggu"},{name:"Diproses",gif:"grocery-basket.gif",key:"Diproses"},{name:"Dikirim",gif:"grocery.gif",key:"Dikirim"},{name:"Diterima",gif:"shipping.gif",key:"Diterima"}];let n=t.findIndex(a=>a.key===e);return e==="Dibatalkan"?`
        ...
        <div class="flex items-center justify-between gap-2 py-2">
            ${t.map((a,o)=>{const r=o<=n,s=o===n,d=o===t.length-1,l=`./assets/images/${a.gif}`,i=sanitizeUrl(l,"./assets/images/wait.gif");return`
                    <div class="flex flex-col items-center flex-1">
                        <div class="${r?"bg-green-50":"bg-gray-100"} w-12 h-12 rounded-full flex items-center justify-center ${s?"ring-2 ring-green-500 ring-offset-2":""}">
                            <img src="${i}" alt="${a.name}" class="w-8 h-8 object-contain ${r?"":"opacity-40"}" data-fallback-src="${l}">
                        </div>
