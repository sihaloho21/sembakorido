window.FrontendSanitize={escapeHtml:function(e){return String(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")},sanitizeUrl:function(e,t=""){if(!e)return t;const n=String(e).trim();return/^(https?:\/\/|\/(?!\/)|\.\/|\.\.\/|#)/i.test(n)?n:t},ensureImageFallbackHandler:function(){window.__imageFallbackHandlerAdded||(window.__imageFallbackHandlerAdded=!0,document.addEventListener("error",e=>{const t=e.target;if(!t||t.tagName!=="IMG")return;const n=t.getAttribute("data-fallback-src"),a=t.getAttribute("data-fallback-action");n&&t.src!==n?t.src=n:a==="hide"&&(t.style.display="none")},!0))}},window.sanitizeUrl||(window.sanitizeUrl=window.FrontendSanitize.sanitizeUrl),window.ensureImageFallbackHandler||(window.ensureImageFallbackHandler=window.FrontendSanitize.ensureImageFallbackHandler),window.AdminSanitize||(window.AdminSanitize=window.FrontendSanitize);var escapeHtml=window.FrontendSanitize&&window.FrontendSanitize.escapeHtml||(e=>String(e||"")),sanitizeUrl=window.FrontendSanitize&&window.FrontendSanitize.sanitizeUrl||(e=>String(e||"")),ensureImageFallbackHandler=window.FrontendSanitize&&window.FrontendSanitize.ensureImageFallbackHandler||(()=>{});function getApiUrl(){const e=CONFIG.getMainApiUrl();return console.log("Using API URL:",e),e}ensureImageFallbackHandler();let API_URL=getApiUrl();function refreshApiUrl(){const e=API_URL;API_URL=getApiUrl(),console.log("Old API URL:",e),console.log("New API URL:",API_URL),e!==API_URL&&(console.log("\u{1F504} API URL changed, clearing cache and reloading..."),typeof ApiService<"u"&&ApiService.clearCache()),fetchProducts()}let cart=JSON.parse(localStorage.getItem("sembako_cart"))||[],allProducts=[],currentCategory="Semua",currentPage=1;const itemsPerPage=12;let filteredProducts=[],storeClosed=CONFIG.isStoreClosed(),selectedVariation=null,currentModalProduct=null;function normalizePhoneNumber(e){if(!e)return null;let t=String(e).replace(/[^0-9]/g,"");return t.startsWith("62")&&(t="0"+t.substring(2)),t.startsWith("8")&&!t.startsWith("08")&&(t="0"+t),!t.startsWith("08")||t.length<10||t.length>13?null:t}function createSlug(e){return e?e.toLowerCase().replace(/[^\w\s-]/g,"").trim().replace(/[-\s]+/g,"-"):""}function ensureProductId(e,t){const n=e.id||e.sku||e.slug||createSlug(e.nama)||"product";return e.id||e.sku?String(n):`${n}-${t}`}function findProductById(e){return e&&allProducts.find(t=>String(t.productId)===String(e))||null}async function fetchProducts(){try{const e=await ApiService.get("?sheet=products",{cacheDuration:3e5});console.log("Products received:",e),allProducts=e.map((t,n)=>{const a=parseInt(t.harga)||0,o=typeof calculateGajianPrice=="function"?calculateGajianPrice(a):{price:a,daysLeft:0,markupPercent:0};let r=t.kategori||"Bahan Pokok";t.kategori||(a>=15e4?r="Paket Lengkap":a>=5e4&&(r="Paket Hemat"));let s=[];if(t.variasi)try{s=JSON.parse(t.variasi)}catch(d){console.error("Error parsing variations for product:",t.id,d)}const l=t.slug||createSlug(t.nama),i=ensureProductId({...t,slug:l},n);return{...t,slug:l,productId:i,harga:a,hargaCoret:parseInt(t.harga_coret)||0,hargaGajian:o.price,stok:parseInt(t.stok)||0,category:r,deskripsi:t.deskripsi&&t.deskripsi.trim()!==""?t.deskripsi:"Kualitas Terjamin, Stok Selalu Baru, Harga Kompetitif",variations:s}}),renderCategoryFilters(),filterProducts(),updateCartUI(),checkStoreStatus(),startNotificationLoop()}catch(e){console.error("Error fetching products:",e);const t=document.getElementById("product-grid");t&&(t.innerHTML='<p class="text-center col-span-full text-red-500">Gagal memuat produk. Silakan coba lagi nanti.</p>')}}function renderCategoryFilters(){const e=document.getElementById("category-filters");if(!e||!allProducts||allProducts.length===0)return;console.log("\u{1F3F7}\uFE0F Rendering category filters...");const t=new Set;allProducts.forEach(o=>{o.category&&o.category.trim()!==""&&t.add(o.category.trim())});const n=Array.from(t).sort();console.log("\u{1F4CA} Categories found:",n);let a='<button type="button" data-action="set-category" data-category="Semua" class="filter-btn active snap-start flex-shrink-0 px-6 py-2 rounded-full border-2 border-green-500 bg-green-50 text-green-700 text-sm font-bold transition hover:border-green-600 hover:bg-green-100">Semua</button>';n.forEach(o=>{const r=escapeHtml(o);a+=`<button type="button" data-action="set-category" data-category="${r}" class="filter-btn snap-start flex-shrink-0 px-6 py-2 rounded-full border-2 border-gray-300 bg-white text-gray-700 text-sm font-bold transition hover:border-green-500 hover:bg-green-50">${r}</button>`}),e.innerHTML=a,console.log("\u2705 Category filters rendered:",n.length,"categories"),initCategoryCarouselScroll()}function initCategoryCarouselScroll(){const e=document.getElementById("category-filters");if(!e)return;e.addEventListener("wheel",o=>{o.deltaY!==0&&(o.preventDefault(),e.scrollLeft+=o.deltaY)},{passive:!1});let t,n,a=!1;e.addEventListener("mousedown",o=>{o.target.classList.contains("filter-btn")||(a=!0,e.style.cursor="grabbing",t=o.pageX-e.offsetLeft,n=e.scrollLeft)}),e.addEventListener("mouseleave",()=>{a=!1,e.style.cursor="grab"}),e.addEventListener("mouseup",()=>{a=!1,e.style.cursor="grab"}),e.addEventListener("mousemove",o=>{if(!a)return;o.preventDefault();const r=2*(o.pageX-e.offsetLeft-t);e.scrollLeft=n-r}),e.style.cursor="grab",updateCategoryArrowsVisibility(),e.addEventListener("scroll",updateCategoryArrowsVisibility)}function scrollCategoryCarousel(e){const t=document.getElementById("category-filters");t&&(e==="left"?t.scrollLeft-=300:e==="right"&&(t.scrollLeft+=300))}function updateCategoryArrowsVisibility(){const e=document.getElementById("category-filters"),t=document.getElementById("category-scroll-left"),n=document.getElementById("category-scroll-right");if(!e||!t||!n)return;const a=e.scrollLeft<=10,o=e.scrollLeft+e.clientWidth>=e.scrollWidth-10;a?t.classList.add("opacity-0","pointer-events-none"):t.classList.remove("opacity-0","pointer-events-none"),o?n.classList.add("opacity-0","pointer-events-none"):n.classList.remove("opacity-0","pointer-events-none")}function findProductBySlug(e){return e&&allProducts&&allProducts.length!==0?allProducts.find(t=>t.slug===e):null}function renderProducts(e){const t=document.getElementById("product-grid");if(!t)return;if(e.length===0)return t.innerHTML='<p class="text-center col-span-full text-gray-500 py-10">Tidak ada produk yang ditemukan.</p>',void(document.getElementById("pagination-container").innerHTML="");const n=Math.ceil(e.length/12);currentPage>n&&(currentPage=n||1);const a=12*(currentPage-1),o=a+12,r=e.slice(a,o);t.innerHTML="",r.forEach(s=>{let l="";l=s.stok>10?'<span class="bg-green-100 text-green-700 text-[10px] px-2 py-0.5 rounded-full font-bold">Stok Tersedia</span>':s.stok>5?`<span class="bg-yellow-100 text-yellow-700 text-[10px] px-2 py-0.5 rounded-full font-bold">Stok Menipis (${s.stok})</span>`:s.stok>0?`<span class="bg-orange-100 text-orange-700 text-[10px] px-2 py-0.5 rounded-full font-bold">Hanya sisa ${s.stok}</span>`:'<span class="bg-red-100 text-red-700 text-[10px] px-2 py-0.5 rounded-full font-bold">Stok Habis</span>';const i=(s.gambar?s.gambar.split(","):[])[0]||"https://placehold.co/300x200?text=Produk",d=sanitizeUrl(i,"https://placehold.co/300x200?text=Produk"),g=calculateRewardPoints(s.harga,s.nama);let u="",p=!1;if(s.grosir)try{const y=JSON.parse(s.grosir);Array.isArray(y)&&y.length>0&&(p=!0,u=`
                    ...
                    `)}catch(y){console.error("Error parsing grosir data for product:",s.id,y)}let c="";if(s.hargaCoret>s.harga){const y=Math.round((s.hargaCoret-s.harga)/s.hargaCoret*100);c=`
                <div class="flex items-center gap-1 mb-0.5">
                    <span class="text-[10px] text-gray-400 line-through">Rp ${s.hargaCoret.toLocaleString("id-ID")}</span>
                    <span class="bg-red-500 text-white text-[8px] px-1.5 py-0.5 rounded font-bold">-${y}%</span>
                </div>
            `}const m=s.variations&&s.variations.length>0,h=s.productId,f=isProductInWishlist(h)?'<svg class="w-5 h-5 text-red-500 fill-current" viewBox="0 0 24 24"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg>':'<svg class="w-5 h-5 text-gray-400 hover:text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/></svg>';t.innerHTML+=`
            <div class="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition duration-300 relative" data-product-id="${h}">
                <!-- Wishlist Heart Button -->
                <button id="wishlist-btn-${h}" data-action="toggle-wishlist" data-product-id="${h}" class="absolute top-3 right-3 z-20 p-2 bg-white/90 hover:bg-white rounded-full shadow-md transition active:scale-95">
                    ${f}
                </button>
                ...
                <div class="lazy-image-wrapper">
                    ...
                    <img src="${d}" alt="${escapeHtml(s.nama)}" data-action="show-detail" data-product-id="${h}" class="w-full h-48 object-cover cursor-pointer hover:opacity-90 transition-opacity ${s.stok===0?"grayscale opacity-60":""}" loading="lazy" data-fallback-src="https://placehold.co/300x200?text=Produk" onload="this.classList.add('loaded'); this.previousElementSibling.style.display='none';">
                </div>
                <div class="p-6">
                    <div class="flex justify-between items-start mb-2">
                        <h4 class="text-lg font-bold text-gray-800">${escapeHtml(s.nama)}</h4>
                        ${l}
                    </div>
                    <div class="flex justify-between items-center mb-4">
                        <button data-action="share-product" data-product-id="${h}" class="text-green-600 hover:text-green-700 flex items-center gap-1 text-xs font-medium">
                            <span>Share</span>
                    ...
                    ${u}
                    ${m?`
                    <button data-action="show-detail" data-product-id="${h}" class="w-full bg-amber-500 hover:bg-amber-600 text-white font-bold py-3 rounded-xl transition flex items-center justify-center gap-2 mb-3">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path></svg>
                        Pilih Variasi
                    </button>
                    `:`
                    <button data-action="add-to-cart" data-product-id="${h}" ${s.stok===0?"disabled":""} class="w-full bg-green-600 hover:bg-green-700 disabled:bg-gray-300 text-white font-bold py-3 rounded-xl transition flex items-center justify-center gap-2 mb-3">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
                        Tambah ke Keranjang
                    </button>
                    `}
                    <div class="grid grid-cols-2 gap-2">
                        <button data-action="show-detail" data-product-id="${h}" class="bg-gray-100 hover:bg-gray-200 text-gray-600 font-bold py-2 rounded-lg text-sm transition">Rincian</button>
                        <button data-action="direct-order" data-product-id="${h}" ${s.stok===0?"disabled":""} class="bg-green-100 hover:bg-green-200 text-green-700 font-bold py-2 rounded-lg text-sm transition">Beli Sekarang</button>
                    </div>

                </div>
            </div>
        `})}function filterProducts(){const e=document.getElementById("search-input"),t=e?normalizeSearch(e.value):"",n=document.getElementById("sort-select"),a=n?n.value:"default";filteredProducts=allProducts.filter(o=>{const r=matchesQuery(o,t),s=currentCategory==="Semua"||o.category===currentCategory;return r&&s}),filteredProducts=sortProducts(filteredProducts,a),currentPage=1,renderProducts(filteredProducts),renderPagination(filteredProducts.length),renderSearchSuggestions(t)}function sortProducts(e,t){const n=[...e];return t==="price-asc"?(n.sort((a,o)=>(a.harga||0)-(o.harga||0)),n):t==="price-desc"?(n.sort((a,o)=>(o.harga||0)-(a.harga||0)),n):(t==="promo"&&n.sort((a,o)=>promoScore(o)-promoScore(a)),n)}function promoScore(e){const t=e.harga||0,n=e.hargaCoret||e.harga_coret||0;return!t||!n||n<=t?0:Math.round((n-t)/n*100)}function normalizeSearch(e){return String(e||"").toLowerCase().replace(/[^a-z0-9\\s]/g," ").replace(/\\s+/g," ").trim()}function tokenize(e){return normalizeSearch(e).split(" ").filter(Boolean)}function matchesQuery(e,t){if(!t)return!0;const n=`${normalizeSearch(e.nama)} ${normalizeSearch(e.deskripsi||"")}`.trim(),a=t.replace(/\s+/g,""),o=n.replace(/\s+/g,"");return n.includes(t)||a&&o.includes(a)?!0:tokenize(t).every(r=>n.includes(r))}function renderSearchSuggestions(e){const t=document.getElementById("search-suggestions"),n=document.getElementById("search-suggestions-header"),a=n&&window.innerWidth<768?n:t;if(!a)return;if(!e)return a.classList.add("hidden"),void(a.innerHTML="");const o=getSearchSuggestions(e,6);if(o.length===0)return a.classList.add("hidden"),void(a.innerHTML="");a.innerHTML=o.map(r=>`
    ...
    `).join(""),a.classList.remove("hidden")}function closeSearchSuggestions(){[document.getElementById("search-suggestions"),document.getElementById("search-suggestions-header")].forEach(e=>{e&&(e.classList.add("hidden"),e.innerHTML="")})}function getSearchSuggestions(e,t=6){const n=tokenize(e);if(n.length===0)return[];const a=allProducts.map(r=>{const s=[...tokenize(r.nama),...tokenize(r.deskripsi||"")];let l=0;return n.forEach(i=>{s.forEach(d=>{d===i?l=Math.max(l,3):d.includes(i)?l=Math.max(l,2):i.length>2&&d.length>2&&levenshtein(d,i)<=1&&(l=Math.max(l,1))})}),{name:r.nama,score:l}}).filter(r=>r.score>0),o=new Map;return a.sort((r,s)=>s.score-r.score).forEach(r=>{o.has(r.name)||o.set(r.name,r)}),Array.from(o.values()).slice(0,t).map(r=>r.name)}function levenshtein(e,t){const n=e.length,a=t.length;if(n===0)return a;if(a===0)return n;const o=Array.from({length:n+1},()=>new Array(a+1).fill(0));for(let r=0;r<=n;r+=1)o[r][0]=r;for(let r=0;r<=a;r+=1)o[0][r]=r;for(let r=1;r<=n;r+=1)for(let s=1;s<=a;s+=1){const l=e[r-1]===t[s-1]?0:1;o[r][s]=Math.min(o[r-1][s]+1,o[r][s-1]+1,o[r-1][s-1]+l)}return o[n][a]}function renderPagination(e){const t=document.getElementById("pagination-container");if(!t)return;const n=Math.ceil(e/12);if(n<=1)return void(t.innerHTML="");let a="";a+=`
    ...
    `,t.innerHTML=a}function changePage(e){const t=Math.ceil(filteredProducts.length/12);e<1||e>t||(currentPage=e,renderProducts(filteredProducts),renderPagination(filteredProducts.length),document.getElementById("katalog").scrollIntoView({behavior:"smooth"}))}function setCategory(e){currentCategory=e,document.querySelectorAll(".filter-btn").forEach(t=>{t.innerText===e?(t.classList.add("active"),t.classList.remove("border-gray-300","bg-white","text-gray-700"),t.classList.add("border-green-500","bg-green-50","text-green-700"),t.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})):(t.classList.remove("active"),t.classList.remove("border-green-500","bg-green-50","text-green-700"),t.classList.add("border-gray-300","bg-white","text-gray-700"))}),filterProducts()}function addToCart(e,t,n=1){storeClosed?showStoreWarning(()=>{proceedAddToCart(e,t,n)}):proceedAddToCart(e,t,n)}function proceedAddToCart(e,t,n=1){if(e.variations&&e.variations.length>0&&!selectedVariation)return void showDetail(e);const a={...e};if(selectedVariation){a.selectedVariation=selectedVariation,a.harga=selectedVariation.harga,a.sku=selectedVariation.sku,a.stok=selectedVariation.stok;const r=typeof calculateGajianPrice=="function"?calculateGajianPrice(selectedVariation.harga):{price:selectedVariation.harga,daysLeft:0,markupPercent:0};a.hargaGajian=r.price}const o=cart.find(r=>{const s=r.id===a.id,l=!r.selectedVariation&&!a.selectedVariation||r.selectedVariation&&a.selectedVariation&&r.selectedVariation.sku===a.selectedVariation.sku;return s&&l});if(o?o.qty+=n:cart.push({...a,qty:n}),saveCart(),updateCartUI(),selectedVariation=null,t&&t.currentTarget){const r=(t.currentTarget.closest(".bg-white")||document.getElementById("detail-modal")).querySelector("img"),s=document.querySelector("header button");if(r&&s){const l=r.getBoundingClientRect(),i=s.getBoundingClientRect(),d=document.createElement("img");d.src=r.src,d.className="fly-item",d.style.top=`${l.top}px`,d.style.left=`${l.left}px`,d.style.width=`${l.width}px`,d.style.height=`${l.height}px`,d.style.borderRadius="12px",document.body.appendChild(d),requestAnimationFrame(()=>{d.style.top=`${i.top+i.height/2}px`,d.style.left=`${i.left+i.width/2}px`,d.style.width="20px",d.style.height="20px",d.style.opacity="0.5",d.style.borderRadius="50%"}),setTimeout(()=>{d.remove(),s.classList.add("cart-pop"),setTimeout(()=>s.classList.remove("cart-pop"),400)},800)}}else{const r=document.querySelector("header button");r&&(r.classList.add("cart-pop"),setTimeout(()=>r.classList.remove("cart-pop"),400))}showToast(`${a.nama}${a.selectedVariation?" ("+a.selectedVariation.nama+")":""} ditambahkan ke keranjang`)}function showToast(e){let t=document.querySelector(".toast-container");t||(t=document.createElement("div"),t.className="toast-container",t.setAttribute("role","status"),t.setAttribute("aria-live","polite"),t.setAttribute("aria-atomic","true"),document.body.appendChild(t));const n=document.createElement("div");n.className="toast",n.setAttribute("role","status"),n.setAttribute("aria-live","polite"),n.setAttribute("aria-atomic","true"),n.innerHTML=`
    ...
    `,n.appendChild(a),document.body.appendChild(n);const r=a.querySelector('[data-action="dismiss-notification"]');if(r&&r.addEventListener("click",()=>n.remove()),!document.getElementById("success-notification-styles")){const s=document.createElement("style");s.id="success-notification-styles",s.textContent=`
            @keyframes fadeIn {
                from { opacity: 0; }
                to { opacity: 1; }
            }
        ...
        `,document.head.appendChild(s)}n.addEventListener("click",s=>{s.target===n&&(n.style.animation="fadeOut 0.3s ease-out",setTimeout(()=>n.remove(),300))})}function saveCart(){localStorage.setItem("sembako_cart",JSON.stringify(cart))}function updateCartUI(){const e=cart.reduce((r,s)=>r+s.qty,0),t=document.getElementById("cart-count");t&&(e>0?(t.innerText=e,t.classList.remove("hidden")):t.classList.add("hidden"));const n=document.getElementById("cart-items"),a=document.getElementById("cart-footer"),o=document.getElementById("cart-empty");if(n)if(cart.length===0)n.innerHTML="",a&&a.classList.add("hidden"),o&&o.classList.remove("hidden");else{o&&o.classList.add("hidden"),a&&a.classList.remove("hidden");let r=0;n.innerHTML=cart.map((d,g)=>{const u=calculateTieredPrice(d.harga,d.qty,d.grosir),p=u<d.harga,c=u*d.qty;r+=c;let m=(d.gambar?d.gambar.split(","):[])[0]||"https://placehold.co/100x100?text=Produk";return d.selectedVariation&&d.selectedVariation.gambar&&(m=d.selectedVariation.gambar),`
                <div class="flex items-center gap-4 bg-gray-50 p-3 rounded-xl">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="flex flex-col">
                            ...
                            <p class="text-green-600 font-bold text-xs">Rp ${u.toLocaleString("id-ID")} ${p?'<span class="bg-green-100 text-green-700 text-[8px] px-1 rounded ml-1">Grosir</span>':""}</p>
                        </div>
            ...
            `}).join("");const s=document.getElementById("cart-total");s&&(s.innerText=`Rp ${r.toLocaleString("id-ID")}`);const l=document.getElementById("order-summary-total");l&&(l.innerText=`Rp ${r.toLocaleString("id-ID")}`);const i=document.getElementById("sticky-order-total");i&&(i.innerText=`Rp ${r.toLocaleString("id-ID")}`)}}function updateQty(e,t){cart[e].qty+=t,cart[e].qty<1&&cart.splice(e,1),saveCart(),updateCartUI()}function removeItem(e){cart.splice(e,1),saveCart(),updateCartUI()}function openCartModal(){const e=document.getElementById("cart-modal");e&&(e.classList.remove("hidden"),document.body.classList.add("modal-active"))}function closeCartModal(){const e=document.getElementById("cart-modal");e&&(e.classList.add("hidden"),document.body.classList.remove("modal-active"))}function updateModalQty(e){const t=document.getElementById("modal-qty");if(!t)return;let n=parseInt(t.value)||1;n+=e,n<1&&(n=1),t.value=n,t.oninput({target:t})}function showDetail(e){closeWishlistModal(),console.log("showDetail called for product:",e.nama);const t=document.getElementById("detail-modal");if(!t)return;currentModalProduct=e,t.dataset.productId=e.productId||"",selectedVariation=null;const n=document.getElementById("modal-qty");n&&(n.value=1);const a=document.getElementById("modal-product-name"),o=document.getElementById("modal-product-image"),r=(document.getElementById("modal-cash-price"),document.getElementById("modal-gajian-price"),document.getElementById("modal-price-date")),s=document.getElementById("modal-items-list"),l=document.getElementById("modal-badges"),i=(document.getElementById("savings-highlight"),document.getElementById("savings-amount"),document.getElementById("modal-variation-container"));a&&(a.innerText=e.nama),n&&(n.oninput=g=>{const u=parseInt(g.target.value)||1,p=selectedVariation?selectedVariation.harga:e.harga,c=selectedVariation?selectedVariation.grosir:e.grosir,m=selectedVariation?selectedVariation.harga_coret||0:e.hargaCoret||0;typeof updateTieredPricingUI=="function"&&updateTieredPricingUI({...e,harga:p,grosir:c},u);const h=typeof calculateTieredPrice=="function"?calculateTieredPrice(p,u,c):p;updateModalPrices(h*u,(typeof calculateGajianPrice=="function"?calculateGajianPrice(h):{price:h}).price*u,m*u)}),i?e.variations&&e.variations.length>0?(i.classList.remove("hidden"),document.getElementById("modal-variation-list").innerHTML=e.variations.map((g,u)=>`
	                <button type="button" data-action="select-variation" data-index="${u}" class="variation-btn border-2 border-gray-200 rounded-xl p-3 text-left transition hover:border-green-500 focus:outline-none">
            ...
            `).join(""),selectVariation(e.variations[0],0)):(i.classList.add("hidden"),updateModalPrices(e.harga,e.hargaGajian,e.hargaCoret)):updateModalPrices(e.harga,e.hargaGajian,e.hargaCoret),r&&(r.innerText=`Harga Per Tgl ${new Date().toLocaleDateString("id-ID",{day:"2-digit",month:"2-digit",year:"numeric"}).replace(/\//g,"-")}`),l&&(l.innerHTML=`
                ...
                <div class="flex justify-between items-center py-1">
                    <div class="flex flex-col">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            ${i?'<span class="bg-green-100 text-green-700 text-[8px] px-1 rounded font-bold">Harga Grosir</span>':""}
                        </div>
                    </div>
                    <div class="flex flex-col items-end">
                        ${i?`<span class="text-[10px] text-gray-400 line-through">Rp ${(s*r.qty).toLocaleString("id-ID")}</span>`:""}
                        <span class="font-bold">Rp ${(l*r.qty).toLocaleString("id-ID")}</span>
                    </div>
                </div>
            `}).join(""),e.innerHTML+=`
...
Mohon segera diproses ya, terima kasih!`,M=`https://wa.me/628993370200?text=${encodeURIComponent(C)}`,L={id:w,pelanggan:t,produk:h.slice(0,-3),qty:c,total:p,status:"Pending",tanggal:new Date().toLocaleString("id-ID"),phone:normalizePhone(n),poin:x,point_processed:"No"};try{await logOrderToGAS(L),console.log("\u2705 Order logged to spreadsheet successfully"),cart=[],saveCart(),updateCartUI(),closeOrderModal(),showSuccessNotification(w,M)}catch(b){console.error("\u274C Error logging order:",b),alert("Gagal menyimpan pesanan. Silakan coba lagi atau hubungi admin.")}finally{e&&setTimeout(()=>{e.disabled=!1,e.innerHTML=`
        ...
        `}finally{hideGlobalRewardLoader()}}function renderRewardItems(e){const t=document.getElementById("reward-items-list");t&&(e&&e.length!==0?t.innerHTML=e.map(n=>{const a=n.id||"",o=n.nama||n.judul||"Hadiah",r=n.poin||0,s=n.gambar||"https://via.placeholder.com/100?text=Reward",l=sanitizeUrl(s,"https://via.placeholder.com/100?text=Reward"),i=n.deskripsi||"",d=escapeHtml(o);return`
            <div class="bg-white p-4 rounded-2xl border-2 border-gray-100 hover:border-green-500 transition-all group shadow-sm">
                <div class="flex gap-4">
                    <div class="w-20 h-20 bg-gray-100 rounded-xl overflow-hidden flex-shrink-0">
                        <img src="${l}" alt="${d}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" data-fallback-src="https://via.placeholder.com/100?text=Reward">
                    </div>
        ...
        `)}function closeRewardModal(){const e=document.getElementById("reward-modal");e&&(e.classList.add("hidden"),document.body.classList.remove("modal-active"))}function checkUserPoints(){const e=document.getElementById("reward-phone"),t=e?e.value.trim():"";if(!t)return void alert("Mohon masukkan nomor WhatsApp.");const n=normalizePhone(t),a=event.target,o=a?a.innerText:"Cek Poin";a&&a.tagName==="BUTTON"&&(a.innerText="Mencari...",a.disabled=!0),ApiService.get("?sheet=user_points",{cache:!1}).then(r=>{const s=r.find(d=>normalizePhone(d.phone||d.whatsapp||"")===n),l=document.getElementById("points-display"),i=document.querySelector("#points-display h4");if(s){const d=(s.points||s.poin||"0").toString().replace(",","."),g=parseFloat(d)||0;i.innerHTML=`${escapeHtml(g.toFixed(1))} <span class="text-sm font-bold">Poin</span>`,sessionStorage.setItem("user_points",g),sessionStorage.setItem("reward_phone",n),sessionStorage.setItem("reward_customer_name",s.nama||s.pelanggan||n),showToast(`Ditemukan ${g.toFixed(1)} poin untuk nomor ini!`)}else i.innerHTML=`${escapeHtml("0.0")} <span class="text-sm font-bold">Poin</span>`,sessionStorage.setItem("user_points",0),sessionStorage.setItem("reward_phone",n),sessionStorage.setItem("reward_customer_name",n),showToast("Nomor tidak ditemukan atau belum memiliki poin.");l.classList.remove("hidden")}).catch(r=>{console.error("Error checking points:",r),alert("Gagal mengecek poin. Silakan coba lagi.")}).finally(()=>{a&&a.tagName==="BUTTON"&&(a.innerText=o,a.disabled=!1)})}async function claimReward(e){const t=sessionStorage.getItem("reward_phone");if(t)try{const n=await ApiService.get(`?sheet=tukar_poin&action=search&id=${e}`,{cache:!1});if(!n||n.length===0)return void alert("Data hadiah tidak ditemukan.");const a=n[0],o=parseFloat(a.poin)||0,r=a.nama||a.judul||"Hadiah",s=parseFloat(sessionStorage.getItem("user_points"))||0,l=`Tukar ${o} poin Anda dengan "${r}"?
Saldo poin saat ini: ${s.toFixed(1)}`;if(!confirm(l))return;let i=prompt("Silakan masukkan nama Anda untuk klaim ini:",sessionStorage.getItem("reward_customer_name")||"");if(i===null)return;i=i.trim()||"Pelanggan",showToast("Sedang memproses penukaran...");const d=`RW-${Date.now()}-${Math.floor(1e5*Math.random())}`,g=await GASActions.post({action:"claim_reward",sheet:"tukar_poin",data:{reward_id:e,phone:t,customer_name:i,request_id:d}}),u=g.claim_id||"CLM-"+Date.now().toString().slice(-6),p=parseFloat(g.points_used||o)||0,c=parseFloat(g.balance_after),m=Number.isNaN(c)?Math.max(0,s-p):c;sessionStorage.setItem("user_points",m);const h=document.querySelector("#points-display h4");h&&(h.innerHTML=`${escapeHtml(m.toFixed(1))} <span class="text-sm font-bold">Poin</span>`);const f=`*KLAIM REWARD POIN BERHASIL*
...
Mohon segera diproses. Terima kasih!`,y=`https://wa.me/628993370200?text=${encodeURIComponent(f)}`;showToast("Penukaran poin berhasil!"),setTimeout(()=>{window.open(y,"_blank")},1500)}catch(n){console.error("Error in claimReward:",n);const a=buildClaimRewardErrorMessage(n);alert(a)}else alert("Mohon cek poin Anda terlebih dahulu.")}document.addEventListener("DOMContentLoaded",()=>{fetchProducts(),fetchTukarPoin();const e=document.getElementById("product-grid");e&&e.addEventListener("click",c=>{const m=c.target.closest("[data-action]");if(!m||m.disabled)return;const h=m.getAttribute("data-action"),f=m.getAttribute("data-product-id")||m.closest("[data-product-id]")?.getAttribute("data-product-id"),y=findProductById(f);switch(h){case"toggle-wishlist":f&&toggleWishlist(f);break;case"show-detail":y&&showDetail(y);break;case"add-to-cart":y&&addToCart(y,c);break;case"direct-order":y&&directOrder(y);break;case"share-product":y&&shareProduct(y.nama)}});const t=document.getElementById("category-filters");t&&t.addEventListener("click",c=>{const m=c.target.closest('[data-action="set-category"]');if(!m)return;const h=m.getAttribute("data-category");h&&setCategory(h)});const n=document.getElementById("pagination-container");n&&n.addEventListener("click",c=>{const m=c.target.closest('[data-action="change-page"]');if(!m||m.disabled)return;const h=parseInt(m.getAttribute("data-page"),10);Number.isNaN(h)||changePage(h)});const a=document.getElementById("modal-add-cart");a&&a.addEventListener("click",c=>{const m=document.getElementById("detail-modal"),h=findProductById(m?m.dataset.productId:null);if(h){const f=document.getElementById("modal-qty");addToCart(h,c,f?parseInt(f.value):1),closeDetailModal()}});const o=document.getElementById("order-modal");o&&(o.querySelectorAll('input[name="ship-method"], input[name="pay-method"], #customer-name, #customer-phone').forEach(c=>{const m=c.type==="radio"?"change":"input";c.addEventListener(m,updateOrderCTAState)}),updateOrderCTAState());const r=document.getElementById("detail-modal");r&&r.addEventListener("click",c=>{const m=c.target.closest('[data-action="select-variation"]');if(!m||m.disabled)return;const h=parseInt(m.getAttribute("data-index"),10);if(!currentModalProduct||!currentModalProduct.variations||Number.isNaN(h))return;const f=currentModalProduct.variations[h];f&&selectVariation(f,h)});const s=document.getElementById("wishlist-items-container");s&&s.addEventListener("click",c=>{const m=c.target.closest("[data-action]");if(!m||m.disabled)return;const h=m.getAttribute("data-action"),f=m.getAttribute("data-product-id"),y=findProductById(f);h==="wishlist-toggle"&&f?toggleWishlist(f):h==="wishlist-buy"&&y&&showDetail(y)});const l=document.getElementById("cart-items");l&&l.addEventListener("click",c=>{const m=c.target.closest("[data-action]");if(!m||m.disabled)return;const h=m.getAttribute("data-action"),f=parseInt(m.getAttribute("data-index"),10),y=parseInt(m.getAttribute("data-delta"),10);h!=="update-cart-qty"||Number.isNaN(f)||Number.isNaN(y)||updateQty(f,y),h!=="remove-cart-item"||Number.isNaN(f)||removeItem(f)});const i=document.getElementById("search-suggestions");i&&i.addEventListener("click",c=>{const m=c.target.closest('[data-action="search-suggestion"]');if(!m)return;const h=m.getAttribute("data-value")||"",f=document.getElementById("search-input");f&&(f.value=h,filterProducts()),closeSearchSuggestions()});const d=document.getElementById("search-suggestions-header");d&&d.addEventListener("click",c=>{const m=c.target.closest('[data-action="search-suggestion"]');if(!m)return;const h=m.getAttribute("data-value")||"",f=document.getElementById("search-input-header"),y=document.getElementById("search-input");f&&(f.value=h),y&&(y.value=h),filterProducts(),closeSearchSuggestions()});const g=document.getElementById("search-input");g&&g.addEventListener("keydown",c=>{c.key==="Escape"&&closeSearchSuggestions(),c.key==="Enter"&&(closeSearchSuggestions(),c.preventDefault(),g.blur())});const u=document.getElementById("search-input-header");u&&u.addEventListener("keydown",c=>{c.key==="Escape"&&closeSearchSuggestions(),c.key==="Enter"&&(closeSearchSuggestions(),c.preventDefault(),u.blur())}),document.addEventListener("click",c=>{c.target.closest("#search-input")||c.target.closest("#search-suggestions")||c.target.closest("#search-input-header")||c.target.closest("#search-suggestions-header")||closeSearchSuggestions()});const p=document.getElementById("search-input-header");p&&(p.addEventListener("input",c=>{const m=c.target.value||"",h=document.getElementById("search-input");h&&(h.value=m),filterProducts()}),p.addEventListener("focus",()=>{const c=document.getElementById("main-header");c&&c.classList.add("header-search-active")}),p.addEventListener("blur",()=>{const c=document.getElementById("main-header");c&&setTimeout(()=>c.classList.remove("header-search-active"),120)}))});let pendingRewardData={id:null,nama:null,poin:null,gambar:null,deskripsi:null};async function showConfirmTukarModal(e){const t=parseFloat(sessionStorage.getItem("user_points"))||0;if(sessionStorage.getItem("reward_phone"))try{const n=await ApiService.get(`?sheet=tukar_poin&action=search&id=${e}`,{cache:!1});if(!n||n.length===0)return void showToast("Data hadiah tidak ditemukan.");const a=n[0],o=parseFloat(a.poin)||0,r=a.nama||a.judul||"Hadiah";if(t<o)return void showToast(`Poin Anda tidak cukup. Dibutuhkan ${o} poin, saldo Anda ${t.toFixed(1)} poin.`);pendingRewardData={id:e,nama:r,poin:o,gambar:a.gambar||"",deskripsi:a.deskripsi||""},document.getElementById("confirm-reward-name").textContent=r,document.getElementById("confirm-reward-points").textContent=o,document.getElementById("confirm-remaining-points").textContent=(t-o).toFixed(1),document.getElementById("confirm-tukar-modal").classList.remove("hidden"),document.body.classList.add("modal-active")}catch(n){console.error("Error showing confirm modal:",n),showToast("Terjadi kesalahan saat memproses permintaan Anda.")}else showToast("Mohon cek poin Anda terlebih dahulu.")}function cancelTukarModal(){document.getElementById("confirm-tukar-modal").classList.add("hidden"),document.body.classList.remove("modal-active"),pendingRewardData={id:null,nama:null,poin:null,gambar:null,deskripsi:null}}function proceedToNameInput(){document.getElementById("confirm-tukar-modal").classList.add("hidden"),document.getElementById("name-input-modal").classList.remove("hidden"),setTimeout(()=>{document.getElementById("claim-name-input").focus()},100)}function backToConfirmModal(){document.getElementById("name-input-modal").classList.add("hidden"),document.getElementById("confirm-tukar-modal").classList.remove("hidden"),document.getElementById("claim-name-input").value=""}async function submitNameAndClaim(){const e=document.getElementById("claim-name-input").value.trim();e?e.length<3?showToast("Nama harus minimal 3 karakter."):(document.getElementById("name-input-modal").classList.add("hidden"),document.body.classList.remove("modal-active"),await processClaimReward(pendingRewardData.id,e)):showToast("Mohon masukkan nama Anda terlebih dahulu.")}async function processClaimReward(e,t){const n=sessionStorage.getItem("reward_phone");try{const a=await ApiService.get(`?sheet=tukar_poin&action=search&id=${e}`,{cache:!1});if(!a||a.length===0)return void showToast("Data hadiah tidak ditemukan.");const o=a[0],r=parseFloat(o.poin)||0,s=o.nama||o.judul||"Hadiah",l=parseFloat(sessionStorage.getItem("user_points"))||0;showToast("Sedang memproses penukaran...");const i=`RW-${Date.now()}-${Math.floor(1e5*Math.random())}`,d=await GASActions.post({action:"claim_reward",sheet:"tukar_poin",data:{reward_id:e,phone:n,customer_name:t,request_id:i}}),g=d.claim_id||"CLM-"+Date.now().toString().slice(-6),u=parseFloat(d.points_used||r)||0,p=parseFloat(d.balance_after),c=Number.isNaN(p)?Math.max(0,l-u):p;sessionStorage.setItem("user_points",c);const m=document.querySelector("#points-display h4");m&&(m.innerHTML=`${escapeHtml(c.toFixed(1))} <span class="text-sm font-bold">Poin</span>`);const h=`*KLAIM REWARD POIN BERHASIL*
...
Mohon segera diproses. Terima kasih!`,f=`https://wa.me/628993370200?text=${encodeURIComponent(h)}`;window.claimWhatsAppUrl=f,pendingRewardData={id:null,nama:null,poin:null,gambar:null,deskripsi:null},document.getElementById("name-input-modal").classList.add("hidden"),document.getElementById("confirm-tukar-modal").classList.add("hidden"),setTimeout(()=>{showClaimSuccessModal(g)},300)}catch(a){console.error("Error processing claim:",a),showToast(buildClaimRewardErrorMessage(a))}}function buildClaimRewardErrorMessage(e){const t=String(e&&e.message||e||"").toLowerCase();return t.includes("reward_stock_empty")?"Stok reward sedang habis.":t.includes("reward_daily_quota_reached")?"Quota harian reward sudah habis.":t.includes("points_insufficient")?"Poin Anda tidak cukup untuk reward ini.":t.includes("user_not_found")?"Data poin pengguna tidak ditemukan.":t.includes("rate_limited")?"Terlalu banyak percobaan klaim, coba lagi sebentar.":"Gagal memproses penukaran. Silakan coba lagi."}const WISHLIST_KEY="gos_wishlist";function getWishlist(){const e=localStorage.getItem(WISHLIST_KEY);return e?JSON.parse(e):[]}function saveWishlist(e){localStorage.setItem(WISHLIST_KEY,JSON.stringify(e)),updateWishlistCount()}function toggleWishlist(e){let t=getWishlist();const n=t.indexOf(e);n>-1?(t.splice(n,1),showToast("Produk dihapus dari Wishlist.")):(t.push(e),showToast("Produk ditambahkan ke Wishlist!")),saveWishlist(t),updateProductWishlistIcon(e);const a=document.getElementById("wishlist-modal");a&&!a.classList.contains("hidden")&&renderWishlistItems()}function isProductInWishlist(e){return getWishlist().includes(e)}function updateWishlistCount(){const e=getWishlist().length,t=document.getElementById("wishlist-count");t&&(t.textContent=e,e>0?t.classList.remove("hidden"):t.classList.add("hidden"))}function updateProductWishlistIcon(e){const t=document.getElementById(`wishlist-btn-${e}`);if(!t)return;const n=isProductInWishlist(e)?'<svg class="w-5 h-5 text-red-500 fill-current" viewBox="0 0 24 24"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg>':'<svg class="w-5 h-5 text-gray-400 hover:text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/></svg>';t.innerHTML=n}function openWishlistModal(){const e=document.getElementById("wishlist-modal");e&&(e.classList.remove("hidden"),document.body.classList.add("modal-active"),renderWishlistItems())}function closeWishlistModal(){const e=document.getElementById("wishlist-modal");e&&(e.classList.add("hidden"),document.body.classList.remove("modal-active"))}async function renderWishlistItems(){const e=getWishlist(),t=document.getElementById("wishlist-items-container");if(console.log("\u{1F50D} Rendering wishlist items:",{wishlistIds:e,containerFound:!!t}),!t)return;if(e.length===0)return void(t.innerHTML=`
        ...
        `);if(!allProducts||allProducts.length===0){console.log("\u{1F4E6} Fetching products for wishlist...");try{await fetchProducts(),console.log("\u2705 Products fetched:",allProducts.length)}catch(o){return console.error("\u274C Failed to fetch products:",o),void(t.innerHTML='<p class="text-center text-red-500 py-4">Gagal memuat produk. Silakan refresh halaman.</p>')}}const n=new Set(e.map(o=>String(o).trim()));console.log("\u{1F50D} Wishlist Set:",Array.from(n));const a=allProducts.filter(o=>{const r=String(o.productId||"").trim(),s=n.has(r);return s&&console.log("\u2705 Found wishlist product:",r,o.nama),s});console.log("\u{1F4CA} Wishlist products found:",a.length),a.length!==0?(t.innerHTML=a.map(o=>{const r=o.productId,s=parseFloat(o.harga_tunai||o.harga||0),l=o.gambar?String(o.gambar).split(",")[0].trim():"";return`
            <div class="flex items-center justify-between p-4 border-b border-gray-100 hover:bg-gray-50 transition">
                <div class="flex items-center gap-3 flex-1">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-xs text-green-600 font-bold mt-1">Rp ${s.toLocaleString("id-ID")}</p>
                    </div>
                </div>
    ...
    `}function selectSidebarCategory(e){setCategory(e),closeCategorySidebar();const t=document.getElementById("products");t&&t.scrollIntoView({behavior:"smooth",block:"start"})}function updateMobileCartCount(){const e=document.getElementById("mobile-cart-count"),t=document.getElementById("cart-count");if(e&&t){const n=cart.length;n>0?(e.textContent=n,e.classList.remove("hidden")):e.classList.add("hidden")}}document.addEventListener("DOMContentLoaded",updateWishlistCount),window.addEventListener("hashchange",handleDeepLink,!1),document.addEventListener("DOMContentLoaded",()=>{setTimeout(handleDeepLink,1e3)});const originalUpdateCartUI=updateCartUI;updateCartUI=function(){typeof originalUpdateCartUI=="function"&&originalUpdateCartUI(),updateMobileCartCount()},document.addEventListener("DOMContentLoaded",function(){updateMobileCartCount()});const normalizePhoneTo08=e=>{const t=String(e??"").replace(/[^0-9]/g,"");if(!t)return"";let n=t;return n.startsWith("62")&&(n=n.slice(2)),n.startsWith("0")&&(n=n.slice(1)),n.startsWith("8")?"0"+n:""},phoneLookupVariants=e=>{const t=normalizePhoneTo08(e);if(!t)return[];const n=t.slice(1);return[t,`+62${n}`,`62${n}`,n]},displayPhone=e=>normalizePhoneTo08(e)||e||"",parseSheetResponse=e=>Array.isArray(e)?e:e&&Array.isArray(e.result)?e.result:e&&e.result?Array.isArray(e.result)?e.result:[e.result]:[],isUnauthorizedApiResponse=e=>!!(e&&typeof e=="object"&&!Array.isArray(e)&&String(e.error||"").toLowerCase()==="unauthorized"),buildSessionQuery=e=>{const t=String(e&&e.session_token||"").trim();return t?`&session_token=${encodeURIComponent(t)}`:""},hasPublicSession=e=>String(e&&e.session_token||"").trim()!=="";let referralProfileCache=null,pendingReferralCodeFromUrl="";function toReferralCodeValue(e){return String(e||"").trim().toUpperCase()}function getReferralCodeFromUrl(){try{const e=new URLSearchParams(window.location.search||"");return toReferralCodeValue(e.get("ref")||e.get("referral")||e.get("kode_referral")||"")}catch(e){return console.warn("Failed parsing referral code from URL:",e),""}}function buildReferralShareUrl(e){const t=toReferralCodeValue(e);if(!t||t==="-")return"";const n=new URL(`${window.location.origin}${window.location.pathname}`);return n.searchParams.set("ref",t),n.toString()}function updateRegisterReferralInfo(){const e=document.getElementById("register-referral-info"),t=document.getElementById("register-referral-info-text");e&&t&&(pendingReferralCodeFromUrl?(t.textContent=`Referral dari link aktif: ${pendingReferralCodeFromUrl}`,e.classList.remove("hidden")):e.classList.add("hidden"))}function setReferralStatus(e,t){const n=document.getElementById("referral-status");n&&(n.textContent=e||"",n.className="text-xs",t==="success"?n.classList.add("text-green-600"):t==="error"?n.classList.add("text-red-600"):t==="warning"?n.classList.add("text-amber-600"):n.classList.add("text-gray-500"))}function generateReferralCode(e,t,n){const a=String(e||"USER").toUpperCase().replace(/[^A-Z0-9]/g,"").slice(0,6)||"USER",o=normalizePhoneTo08(t).replace(/\D/g,"").slice(-4)||Math.floor(1e3+9e3*Math.random()).toString();let r=`${a}${o}`.slice(0,24);if(!n.has(r))return r;for(let s=1;s<=2e3;s+=1){const l=`${a}${o}${s}`.slice(0,24);if(!n.has(l))return l}return`${a}${Date.now().toString().slice(-6)}`.slice(0,24)}async function fetchUsersList(){const e=CONFIG.getMainApiUrl(),t=await fetch(`${e}?sheet=users&_t=${Date.now()}`);if(!t.ok)throw new Error("Gagal memuat data users");return parseSheetResponse(await t.json())}async function ensureUserReferralCode(e,t){const n=t.find(s=>String(s.id)===String(e.id))||t.find(s=>normalizePhoneTo08(s.whatsapp||s.phone||"")===normalizePhoneTo08(e.whatsapp));if(!n)return{user:{id:e.id||"",nama:e.nama||"User",whatsapp:normalizePhoneTo08(e.whatsapp||e.phone||""),kode_referral:"",referred_by:"",referral_count:0,referral_points_total:0},code:"",missingProfile:!0};const a=toReferralCodeValue(n.kode_referral);if(a)return{user:n,code:a};const o=new Set(t.map(s=>toReferralCodeValue(s.kode_referral)).filter(Boolean)),r=generateReferralCode(n.nama,n.whatsapp||n.phone,o);return await GASActions.update("users",n.id,{kode_referral:r}),n.kode_referral=r,{user:n,code:r}}function applyReferralDataToUI(e){const t=document.getElementById("referral-code-display"),n=document.getElementById("referral-input"),a=document.getElementById("apply-referral-btn"),o=document.getElementById("referral-count"),r=document.getElementById("referral-points-total");t&&(t.value=toReferralCodeValue(e.kode_referral)||"-"),o&&(o.textContent=String(parseInt(e.referral_count||0,10)||0)),r&&(r.textContent=String(parseInt(e.referral_points_total||0,10)||0));const s=toReferralCodeValue(e.referred_by)!=="";n&&(n.value=s?toReferralCodeValue(e.referred_by):"",n.disabled=s),a&&(a.disabled=s),s?setReferralStatus(`Referral aktif dengan kode: ${toReferralCodeValue(e.referred_by)}`,"success"):setReferralStatus("Kode referral hanya bisa dipakai satu kali.","info")}function buildReferralProfileFallback(e){return{id:e&&e.id?e.id:"",nama:e&&e.nama?e.nama:"User",whatsapp:normalizePhoneTo08(e&&(e.whatsapp||e.phone)||""),kode_referral:toReferralCodeValue(e&&e.kode_referral?e.kode_referral:""),referred_by:toReferralCodeValue(e&&e.referred_by?e.referred_by:""),referral_count:parseInt(e&&e.referral_count||0,10)||0,referral_points_total:parseInt(e&&e.referral_points_total||0,10)||0}}function getReferralStatusBadge(e){const t=String(e||"").toLowerCase();return t==="approved"?'<span class="px-2 py-0.5 rounded-full bg-green-100 text-green-700 font-bold">Approved</span>':t==="rejected"||t==="void"?'<span class="px-2 py-0.5 rounded-full bg-red-100 text-red-700 font-bold">Rejected</span>':'<span class="px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 font-bold">Pending</span>'}async function loadReferralHistory(e){const t=document.getElementById("referral-history-list"),n=document.getElementById("referral-history-badge");if(t&&n)try{const a=CONFIG.getMainApiUrl(),o=normalizePhoneTo08(e.whatsapp);let r=[];const s=buildSessionQuery(e);if(s){const i=await fetch(`${a}?action=public_referral_history${s}&_t=${Date.now()}`);if(i.ok){const d=await i.json();d&&d.success&&Array.isArray(d.referrals)&&(r=d.referrals)}}if(!r.length){const i=await fetch(`${a}?sheet=referrals&phone=${encodeURIComponent(o)}&_t=${Date.now()}`);if(!i.ok)throw new Error("Gagal memuat riwayat referral");r=parseSheetResponse(await i.json())}const l=r.filter(i=>{const d=normalizePhoneTo08(i.referrer_phone||""),g=normalizePhoneTo08(i.referee_phone||"");return d===o||g===o}).sort((i,d)=>new Date(d.created_at||0).getTime()-new Date(i.created_at||0).getTime()).slice(0,5);if(n.textContent=`${l.length} data`,l.length===0)return void(t.innerHTML='<p class="text-gray-400">Belum ada riwayat referral.</p>');t.innerHTML=l.map(i=>{const d=normalizePhoneTo08(i.referrer_phone||"")===o?"Anda mengajak":"Anda diajak",g=normalizePhoneTo08(d==="Anda mengajak"?i.referee_phone||"":i.referrer_phone||""),u=d==="Anda mengajak"?parseInt(i.reward_referrer_points||0,10)||0:parseInt(i.reward_referee_points||0,10)||0;return`
                <div class="border border-gray-200 rounded-lg p-2 bg-gray-50">
                    ...
                    <p class="text-gray-600">No: ${escapeHtml(g||"-")}</p>
        ...
        Masuk
    `}function saveLoggedInUser(e){const t=normalizePhoneTo08(e.whatsapp||e.phone||"");localStorage.setItem("gosembako_user",JSON.stringify({id:e.id,nama:e.nama,whatsapp:t||e.whatsapp,tanggal_daftar:e.tanggal_daftar,status:e.status||"",total_points:parseInt(e.total_points||e.points||e.poin||0,10)||0,kode_referral:toReferralCodeValue(e.kode_referral||""),referred_by:toReferralCodeValue(e.referred_by||""),referral_count:parseInt(e.referral_count||0,10)||0,referral_points_total:parseInt(e.referral_points_total||0,10)||0,session_token:String(e.session_token||"").trim()}))}function getLoggedInUser(){const e=localStorage.getItem("gosembako_user");if(!e)return null;const t=JSON.parse(e);return t.whatsapp=normalizePhoneTo08(t.whatsapp||t.phone||"")||t.whatsapp,t.session_token=String(t.session_token||"").trim(),t}function logout(){confirm("Apakah Anda yakin ingin keluar?")&&(localStorage.removeItem("gosembako_user"),window.location.reload())}async function copyReferralCode(){const e=document.getElementById("referral-code-display"),t=toReferralCodeValue(e?e.value:"");if(!t||t==="-")return void setReferralStatus("Kode referral belum tersedia.","warning");const n=buildReferralShareUrl(t);if(n)try{if(navigator.clipboard&&navigator.clipboard.writeText)await navigator.clipboard.writeText(n);else{const a=document.createElement("textarea");a.value=n,document.body.appendChild(a),a.select(),document.execCommand("copy"),document.body.removeChild(a)}setReferralStatus("Link referral berhasil disalin.","success")}catch(a){console.error("Copy referral failed:",a),setReferralStatus("Gagal menyalin link referral.","error")}else setReferralStatus("Link referral tidak tersedia.","warning")}async function applyReferralCode(){const e=getLoggedInUser();if(!e)return void setReferralStatus("Silakan login ulang.","error");const t=document.getElementById("referral-input"),n=document.getElementById("apply-referral-btn"),a=toReferralCodeValue(t?t.value:"");if(!a)return void setReferralStatus("Masukkan kode referral terlebih dahulu.","warning");const o=toReferralCodeValue(document.getElementById("referral-code-display")?.value);if(o&&a===o)return void setReferralStatus("Kode referral sendiri tidak bisa dipakai.","error");if(t&&t.disabled)return void setReferralStatus("Kode referral sudah pernah diterapkan.","warning");const r=n?n.textContent:"";try{n&&(n.disabled=!0,n.textContent="Memproses...");const s=await GASActions.post({action:"attach_referral",sheet:"users",data:{referee_phone:normalizePhoneTo08(e.whatsapp),ref_code:a}});if(!s||s.success===!1)return setReferralStatus(s&&(s.message||s.error||s.error_code)||"Gagal menerapkan referral","error"),void(n&&(n.disabled=!1,n.textContent=r||"Terapkan"));t&&(t.value=a,t.disabled=!0),n&&(n.disabled=!0,n.textContent="Terapkan"),setReferralStatus("Kode referral berhasil diterapkan.","success"),await loadReferralData(e)}catch(s){console.error("applyReferralCode error:",s),setReferralStatus(s.message||"Gagal menerapkan kode referral.","error"),n&&(n.disabled=!1,n.textContent=r||"Terapkan")}}async function loadLoyaltyPoints(e){try{const t=CONFIG.getMainApiUrl(),n=normalizePhoneTo08(e.whatsapp),a=parseInt(e.total_points||e.points||e.poin||0,10)||0;if(!n)return console.warn("\u26A0\uFE0F Invalid phone number for loyalty points lookup"),document.getElementById("loyalty-points").textContent=String(a),void setRewardContextFromAkun(e.whatsapp,a,e.nama);console.log(`\u{1F50D} Loading loyalty points for phone: ${n}`);let o=null;const r=buildSessionQuery(e);if(r){const u=await fetch(`${t}?action=public_user_points${r}&_t=${Date.now()}`);if(u.ok){const p=await u.json();p&&p.success&&(o=parseInt(p.points||0,10)||0)}}if(o!==null)return document.getElementById("loyalty-points").textContent=String(o),void setRewardContextFromAkun(e.whatsapp,o,e.nama);if(!hasPublicSession(e))return document.getElementById("loyalty-points").textContent=String(a),void setRewardContextFromAkun(e.whatsapp,a,e.nama);const s=await fetch(`${t}?sheet=user_points`);if(!s.ok)return console.error("\u274C Failed to fetch loyalty points"),void(document.getElementById("loyalty-points").textContent="0");const l=await s.json();if(console.log("\u{1F4E5} Points data received:",l),l&&typeof l=="object"&&String(l.error||"").toLowerCase()==="unauthorized")return console.warn("\u26A0\uFE0F Unauthorized access to user_points. Falling back to user profile points."),document.getElementById("loyalty-points").textContent=String(a),void setRewardContextFromAkun(e.whatsapp,a,e.nama);let i=Array.isArray(l)?l:l.result||[];if(!Array.isArray(i))return console.warn("\u26A0\uFE0F Unexpected points data format"),void(document.getElementById("loyalty-points").textContent="0");const d=phoneLookupVariants(n);let g=null;for(const u of d)if(g=i.find(p=>normalizePhoneTo08(p.phone||p.whatsapp||"")===normalizePhoneTo08(u)),g){console.log(`\u2705 Found points record for ${u}:`,g);break}if(g){const u=parseInt(g.points||g.poin||0);console.log(`\u2705 User points: ${u}`),document.getElementById("loyalty-points").textContent=u,setRewardContextFromAkun(e.whatsapp,u,e.nama)}else console.log("\u26A0\uFE0F No points record found for user"),document.getElementById("loyalty-points").textContent=String(a),setRewardContextFromAkun(e.whatsapp,a,e.nama)}catch(t){console.error("\u274C Error loading loyalty points:",t);const n=parseInt(e.total_points||e.points||e.poin||0,10)||0;document.getElementById("loyalty-points").textContent=String(n),setRewardContextFromAkun(e.whatsapp,n,e.nama)}}async function loadOrderHistory(e){const t=document.getElementById("order-loading"),n=document.getElementById("order-empty"),a=document.getElementById("order-list"),o=document.getElementById("total-accepted-spend");t.classList.remove("hidden"),n.classList.add("hidden"),a.innerHTML="",o&&(o.textContent="Rp 0");try{const r=CONFIG.getMainApiUrl();let s=[];const l=buildSessionQuery(e);if(l){const i=await fetch(`${r}?action=public_user_orders${l}&_t=${Date.now()}`);if(i.ok){const d=await i.json();d&&d.success&&Array.isArray(d.orders)&&(s=d.orders)}}if(!s.length){let i=await fetch(`${r}?sheet=orders`);if(!i.ok)throw new Error("Gagal memuat riwayat pesanan");let d=await i.json();s=Array.isArray(d)?d.filter(g=>normalizePhoneTo08(g.phone)===normalizePhoneTo08(e.whatsapp)):[]}if(t.classList.add("hidden"),!s||s.length===0||(s=s.filter(i=>normalizePhoneTo08(i.phone||i.whatsapp||"")===normalizePhoneTo08(e.whatsapp)),s.length===0))return void n.classList.remove("hidden");if(s.sort((i,d)=>{const g=new Date(i.tanggal_pesanan||i.timestamp||0);return new Date(d.tanggal_pesanan||d.timestamp||0)-g}),window.allOrders=s,window.currentPage=1,window.ordersPerPage=10,o){const i=new Set(["terima","diterima","selesai"]),d=s.reduce((g,u)=>{const p=String(u.status||"").toLowerCase().trim();return i.has(p)?g+parseCurrencyValue(u.total||u.total_bayar||0):g},0);o.textContent=formatCurrency(d)}displayOrderPage(1)}catch(r){console.error("Error loading order history:",r),t.classList.add("hidden"),a.innerHTML=`
        ...
        <div class="border-t border-gray-100 pt-2 space-y-1.5">
            <div class="text-sm">
                ...
                <p class="font-semibold text-gray-800 lg:hidden">${escapeHtml(r)}</p>
                <p class="font-semibold text-gray-800 hidden lg:block">${escapeHtml(o)}</p>
            </div>
...
Anda akan diberikan akun dengan nomor WhatsApp dan PIN untuk login.`)}function showRegister(){document.getElementById("login-section").classList.add("hidden"),document.getElementById("register-section").classList.remove("hidden"),document.getElementById("forgot-pin-section").classList.add("hidden"),document.getElementById("dashboard-section").classList.add("hidden"),updateRegisterReferralInfo()}function showForgotPIN(){document.getElementById("login-section").classList.add("hidden"),document.getElementById("register-section").classList.add("hidden"),document.getElementById("forgot-pin-section").classList.remove("hidden"),document.getElementById("dashboard-section").classList.add("hidden")}function openEditProfile(){const e=getLoggedInUser();e&&(document.getElementById("edit-name").value=e.nama,document.getElementById("edit-old-pin").value="",document.getElementById("edit-new-pin").value="",document.getElementById("edit-confirm-pin").value="",document.getElementById("edit-error").classList.add("hidden"),document.getElementById("edit-profile-modal").classList.remove("hidden"))}function closeEditProfile(){document.getElementById("edit-profile-modal").classList.add("hidden")}function showOrderDetailModal(e){document.getElementById("tracking-order-id").textContent=e.id||"N/A";const t=e.tanggal||e.tanggal_pesanan||e.timestamp||e.date||e.created_at;document.getElementById("tracking-order-date").textContent=formatDate(t);const n=normalizeStatus(e.status||"Menunggu"),a=getStatusBadge(n),o=document.getElementById("tracking-status-badge");o&&(o.innerHTML=a),document.getElementById("tracking-products").textContent=e.produk||e.items||e.product_name||"N/A",document.getElementById("tracking-total").textContent=formatCurrency(e.total||e.total_bayar||0);const r=createAnimatedTimeline(n);document.getElementById("tracking-timeline").innerHTML=r,document.getElementById("order-tracking-modal").classList.remove("hidden")}function createAnimatedTimeline(e){const t=[{name:"Menunggu",gif:"wait.gif",key:"Menunggu"},{name:"Diproses",gif:"grocery-basket.gif",key:"Diproses"},{name:"Dikirim",gif:"grocery.gif",key:"Dikirim"},{name:"Diterima",gif:"shipping.gif",key:"Diterima"}];let n=t.findIndex(a=>a.key===e);return e==="Dibatalkan"?`
        ...
        <div class="flex items-center justify-between gap-2 py-2">
            ${t.map((a,o)=>{const r=o<=n,s=o===n,l=o===t.length-1,i=`./assets/images/${a.gif}`;return`
                    <div class="flex flex-col items-center flex-1">
                        <div class="${r?"bg-green-50":"bg-gray-100"} w-12 h-12 rounded-full flex items-center justify-center ${s?"ring-2 ring-green-500 ring-offset-2":""}">
                            <img src="${sanitizeUrl(i,"./assets/images/wait.gif")}" alt="${a.name}" class="w-8 h-8 object-contain ${r?"":"opacity-40"}" data-fallback-src="${i}">
                        </div>
                        <p class="text-[10px] font-semibold mt-2 text-center ${r?"text-gray-800":"text-gray-400"}">${a.name}</p>
                        ${s?'<p class="text-[8px] text-green-600 font-bold mt-0.5">\u25CF Saat ini</p>':'<p class="text-[8px] text-transparent mt-0.5">\u25CF</p>'}
                    </div>
                    ${l?"":`
                        <div class="flex-shrink-0 w-8 h-0.5 ${r&&o<n?"bg-green-500":"bg-gray-300"} transition-all duration-500 mb-6"></div>
                    `}
                `}).join("")}
        </div>
    `)}function closeOrderTracking(){document.getElementById("order-tracking-modal").classList.add("hidden")}document.addEventListener("DOMContentLoaded",()=>{pendingReferralCodeFromUrl=getReferralCodeFromUrl(),updateRegisterReferralInfo();const e=getLoggedInUser(),t=document.getElementById("referral-input");t&&t.addEventListener("keydown",n=>{n.key==="Enter"&&(n.preventDefault(),applyReferralCode())}),e?showDashboard(e):showLogin()}),document.getElementById("login-form").addEventListener("submit",async e=>{e.preventDefault();const t=document.getElementById("login-whatsapp").value.trim(),n=document.getElementById("login-pin").value.trim(),a=document.getElementById("login-btn"),o=document.getElementById("login-error");if(document.getElementById("login-error-text"),!t||!n)return void showError("Mohon lengkapi semua field");const r=normalizePhoneTo08(t);if(r)if(r.length<10||r.length>13)showError("Panjang nomor tidak valid");else if(n.length===6){a.disabled=!0,a.innerHTML=`
        <svg class="animate-spin h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
        </svg>
        <span>Memproses...</span>
    `,o.classList.add("hidden");try{const s=CONFIG.getMainApiUrl(),l="&_t="+Date.now();let i=null;const d=`${s}?action=public_login&phone=${encodeURIComponent(r)}&pin=${encodeURIComponent(n)}${l}`,g=await fetch(d);if(g.ok){const u=await g.json();if(u&&u.success&&u.user)i=u.user;else{const p=String(u&&u.error?u.error:"").toLowerCase();if(p==="login_failed")return showError("Nomor WhatsApp atau PIN salah"),void resetLoginButton();if(p==="account_inactive")return showError("Akun Anda tidak aktif. Hubungi admin."),void resetLoginButton();if(p==="rate_limited")return showError(u&&u.message||"Terlalu banyak percobaan login, coba lagi."),void resetLoginButton();if(p!=="invalid sheet"&&p!=="invalid action")return showError(u&&u.message||"Login gagal. Silakan coba lagi."),void resetLoginButton()}}if(!i){const u=phoneLookupVariants(r),p=await fetch(`${s}?sheet=users${l}`);if(!p.ok)return showError("Gagal menghubungi server. Silakan coba lagi."),void resetLoginButton();const c=await p.json();if(isUnauthorizedApiResponse(c))return showError("Layanan login membutuhkan endpoint public_login. Hubungi admin."),void resetLoginButton();const m=parseSheetResponse(c);for(const h of u){const f=m.find(y=>normalizePhoneTo08(y.whatsapp||y.phone||"")===normalizePhoneTo08(h));if(f){i=f;break}}if(!i)return showError("Nomor WhatsApp tidak terdaftar"),void resetLoginButton();if((i.pin||"").toString()!==n)return showError("PIN salah. Silakan coba lagi."),void resetLoginButton()}if(i.status&&i.status.toLowerCase()!=="aktif")return showError("Akun Anda tidak aktif. Hubungi admin."),void resetLoginButton();i.whatsapp=normalizePhoneTo08(i.whatsapp||i.phone||r),saveLoggedInUser(i),showDashboard(i),document.getElementById("dashboard-section").scrollIntoView({behavior:"smooth",block:"start"})}catch(s){console.error("Login error:",s),showError("Terjadi kesalahan. Silakan coba lagi."),resetLoginButton()}}else showError("PIN harus 6 digit");else showError("Gunakan format 08xxxxxxxxxx")}),document.getElementById("register-form").addEventListener("submit",async e=>{e.preventDefault();const t=document.getElementById("register-name").value.trim(),n=document.getElementById("register-whatsapp").value.trim(),a=document.getElementById("register-pin").value.trim(),o=document.getElementById("register-pin-confirm").value.trim(),r=pendingReferralCodeFromUrl,s=document.getElementById("register-error"),l=document.getElementById("register-error-text"),i=document.getElementById("register-success"),d=document.getElementById("register-btn");s.classList.add("hidden"),i.classList.add("hidden");const g=t.replace(/\s/g,"");if(g.length<4)return l.textContent="Masukkan Nama Lengkap",void s.classList.remove("hidden");const u=g.toLowerCase(),p=[/^(.)\1{3,}$/,/^(.{2})\1{2,}$/,/^(.{3})\1{2,}$/,/^([a-z])([a-z])\1\2{2,}$/];for(const f of p)if(f.test(u))return l.textContent="Masukkan Nama Lengkap",void s.classList.remove("hidden");const c=normalizePhoneTo08(n);if(!c)return l.textContent="Gunakan format 08xxxxxxxxxx",void s.classList.remove("hidden");const m=c.replace(/[^0-9]/g,"");if(m.length<10||m.length>13)return l.textContent="Nomor WhatsApp tidak valid",void s.classList.remove("hidden");const h=[/^(\d)\1{9,}$/,/^08(\d)\1{8,}$/,/^(\d{2})\1{4,}$/,/^(\d{3})\1{3,}$/];for(const f of h)if(f.test(m))return l.textContent="Nomor WhatsApp tidak valid",void s.classList.remove("hidden");if(a.length!==6||!/^\d{6}$/.test(a))return l.textContent="PIN harus 6 digit angka",void s.classList.remove("hidden");if(a!==o)return l.textContent="PIN tidak cocok",void s.classList.remove("hidden");d.disabled=!0,d.innerHTML='<svg class="animate-spin h-5 w-5 mx-auto" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>';try{const f=CONFIG.getMainApiUrl(),y=phoneLookupVariants(c);let x=[];const w="&_t="+Date.now();console.log("\u{1F50D} Checking phone formats:",y);try{const v=await fetch(`${f}?sheet=users${w}`);if(v.ok){const I=await v.json();x=parseSheetResponse(I).filter(B=>{const S=normalizePhoneTo08(B.whatsapp||B.phone||"");return y.some($=>normalizePhoneTo08($)===S)}),x&&x.length>0&&console.log("\u{1F4CA} Found existing user:",x)}else console.warn(`API error: ${v.status}`)}catch(v){console.warn("Check failed:",v.message)}if(console.log("\u{1F4CA} Final check result for",n,":",x),x&&x.length>0)return l.textContent="Nomor WhatsApp sudah terdaftar",s.classList.remove("hidden"),d.disabled=!1,void(d.innerHTML='<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z"></path></svg> Daftar');const k=`USR-${Date.now().toString().slice(-6)}`,C=new Date().toISOString().split("T")[0],M=new Date().toLocaleString("id-ID",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",second:"2-digit"}),L=await GASActions.create("users",{id:k,nama:t,whatsapp:c,pin:a,tanggal_daftar:C,status:"aktif",total_points:0,created_at:M,referred_by:"",referred_by_phone:"",referral_count:0,referral_points_total:0,kode_referral:""});if(!L.created||L.created<1)throw new Error("Gagal mendaftar");let b="";if(r)try{const v=await GASActions.post({action:"attach_referral",sheet:"users",data:{referee_phone:c,ref_code:r}});b=v&&v.success?" Kode referral berhasil diterapkan.":" Akun dibuat, namun kode referral tidak valid."}catch(v){console.warn("Referral attach failed after registration:",v),b=" Akun dibuat, namun kode referral gagal diproses."}i.classList.remove("hidden");const E=i.querySelector("span");E&&(E.textContent="Pendaftaran berhasil! Silakan login."+b),document.getElementById("register-form").reset(),setTimeout(()=>{showLogin()},2e3)}catch(f){console.error("Registration error:",f),l.textContent="Terjadi kesalahan. Silakan coba lagi.",s.classList.remove("hidden")}finally{d.disabled=!1,d.innerHTML='<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z"></path></svg> Daftar'}}),document.getElementById("forgot-pin-form").addEventListener("submit",async e=>{e.preventDefault();const t=document.getElementById("forgot-whatsapp").value.trim(),n=document.getElementById("forgot-error"),a=document.getElementById("forgot-error-text"),o=document.getElementById("forgot-btn"),r=normalizePhoneTo08(t);if(!r)return n.classList.remove("hidden"),void(a.textContent="Gunakan format 08xxxxxxxxxx");n.classList.add("hidden"),o.disabled=!0,o.innerHTML='<svg class="animate-spin h-5 w-5 mx-auto" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>';try{const s=CONFIG.getMainApiUrl(),l=await fetch(`${s}?sheet=users`),i=parseSheetResponse(await l.json()).filter(g=>normalizePhoneTo08(g.whatsapp||g.phone||"")===r);if(!i||i.length===0)return a.textContent="Nomor WhatsApp tidak terdaftar",n.classList.remove("hidden"),o.disabled=!1,void(o.innerHTML='<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg> Kirim Kode Verifikasi');alert(`Kode verifikasi telah dikirim ke WhatsApp ${r}.

Untuk sementara, hubungi admin untuk reset PIN.`);const d=r.replace(/^0/,"62");window.open(`https://wa.me/628993370200?text=Halo, saya ingin reset PIN akun saya. Nomor WhatsApp: ${d}`,"_blank"),setTimeout(()=>{showLogin()},1e3)}catch(s){console.error("Forgot PIN error:",s),a.textContent="Terjadi kesalahan. Silakan coba lagi.",n.classList.remove("hidden")}finally{o.disabled=!1,o.innerHTML='<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg> Kirim Kode Verifikasi'}}),document.getElementById("edit-profile-form").addEventListener("submit",async e=>{e.preventDefault();const t=getLoggedInUser();if(!t)return;const n=document.getElementById("edit-name").value.trim(),a=document.getElementById("edit-old-pin").value.trim(),o=document.getElementById("edit-new-pin").value.trim(),r=document.getElementById("edit-confirm-pin").value.trim(),s=document.getElementById("edit-error"),l=document.getElementById("edit-error-text"),i=document.getElementById("edit-save-btn");if(s.classList.add("hidden"),n.replace(/\s/g,"").length<4)return l.textContent="Masukkan Nama Lengkap",void s.classList.remove("hidden");if(a||o||r){if(!a||!o||!r)return l.textContent="Lengkapi semua field PIN",void s.classList.remove("hidden");if(o.length!==6||!/^\d{6}$/.test(o))return l.textContent="PIN baru harus 6 digit angka",void s.classList.remove("hidden");if(o!==r)return l.textContent="PIN baru tidak cocok",void s.classList.remove("hidden")}i.disabled=!0,i.textContent="Menyimpan...";try{const d=CONFIG.getMainApiUrl(),g=await fetch(`${d}?sheet=users&id=${t.id}`),u=await g.json();if(!u||u.length===0)throw new Error("User not found");const p=u[0];if(a&&p.pin!==a)return l.textContent="PIN lama salah",s.classList.remove("hidden"),i.disabled=!1,void(i.textContent="Simpan");const c={nama:n};o&&(c.pin=o);const m=await GASActions.update("users",t.id,c);if(!m.affected||m.affected<1)throw new Error("Failed to update");t.nama=n,saveLoggedInUser(t),document.getElementById("user-name").textContent=n,closeEditProfile(),alert("Profil berhasil diperbarui!")}catch(d){console.error("Edit profile error:",d),l.textContent="Terjadi kesalahan. Silakan coba lagi.",s.classList.remove("hidden")}finally{i.disabled=!1,i.textContent="Simpan"}});const originalCreateOrderCard=createOrderCard;function switchRewardTab(e){const t=document.getElementById("tab-exchange"),n=document.getElementById("tab-history"),a=document.getElementById("exchange-content"),o=document.getElementById("history-content");e==="exchange"?(t.classList.add("text-amber-600","border-amber-600"),t.classList.remove("text-gray-400","border-transparent"),n.classList.add("text-gray-400","border-transparent"),n.classList.remove("text-amber-600","border-amber-600"),a.classList.remove("hidden"),o.classList.add("hidden"),fetchRewardItemsForAkun()):e==="history"&&(n.classList.add("text-amber-600","border-amber-600"),n.classList.remove("text-gray-400","border-transparent"),t.classList.add("text-gray-400","border-transparent"),t.classList.remove("text-amber-600","border-amber-600"),o.classList.remove("hidden"),a.classList.add("hidden"),loadClaimsHistory())}async function loadClaimsHistory(){const e=getLoggedInUser();if(!e)return;const t=document.getElementById("claims-loading"),n=document.getElementById("claims-empty"),a=document.getElementById("claims-list");t.classList.remove("hidden"),n.classList.add("hidden"),a.innerHTML="";try{const o=CONFIG.getMainApiUrl(),r=phoneLookupVariants(e.whatsapp).map(i=>`phone=${encodeURIComponent(i)}`).join("&"),s=await fetch(`${o}?sheet=claims&${r}`);if(!s.ok)throw new Error("Failed to fetch claims");const l=parseSheetResponse(await s.json());if(t.classList.add("hidden"),!l||l.length===0)return void n.classList.remove("hidden");l.sort((i,d)=>{const g=new Date(i.tanggal||i.date||0);return new Date(d.tanggal||d.date||0)-g}),l.forEach(i=>{const d=createClaimCard(i);a.appendChild(d)})}catch(o){console.error("Error loading claims history:",o),t.classList.add("hidden"),n.classList.remove("hidden")}}function createClaimCard(e){const t=document.createElement("div");t.className="border border-gray-200 rounded-xl p-4 hover:shadow-md transition";const n=(e.status||"pending").toLowerCase(),a={pending:"bg-yellow-100 text-yellow-700",approved:"bg-green-100 text-green-700",completed:"bg-blue-100 text-blue-700",rejected:"bg-red-100 text-red-700"}[n]||"bg-gray-100 text-gray-700",o={pending:"Menunggu",approved:"Disetujui",completed:"Selesai",rejected:"Ditolak"}[n]||e.status||"Menunggu",r=new Date(e.tanggal||e.date).toLocaleDateString("id-ID",{day:"numeric",month:"long",year:"numeric"});return t.innerHTML=`
        <div class="flex justify-between items-start mb-3">
            <div class="flex-1">
                <h5 class="font-bold text-gray-800">${escapeHtml(e.hadiah||e.reward||"Reward")}</h5>
                <p class="text-xs text-gray-500 mt-1">${r}</p>
            </div>
            <span class="${a} text-xs font-bold px-3 py-1 rounded-full">${escapeHtml(o)}</span>
        </div>
    ...
    `,t}function closeLoyaltyModal(){document.getElementById("loyalty-modal").classList.add("hidden"),switchRewardTab("exchange")}async function openRewardModal(){const e=getLoggedInUser();if(!e)return;await loadModalPoints();const t=document.getElementById("loyalty-modal-points"),n=t?parseInt(t.textContent||"0"):0;setRewardContextFromAkun(e.whatsapp,n,e.nama),document.getElementById("loyalty-modal").classList.remove("hidden"),switchRewardTab("exchange"),fetchRewardItemsForAkun()}async function loadModalPoints(){const e=getLoggedInUser();if(!e)return;const t=parseInt(e.total_points||e.points||e.poin||0,10)||0;try{const n=CONFIG.getMainApiUrl(),a=buildSessionQuery(e);if(a){const l=await fetch(`${n}?action=public_user_points${a}&_t=${Date.now()}`);if(l.ok){const i=await l.json();if(i&&i.success)return void(document.getElementById("loyalty-modal-points").textContent=String(parseInt(i.points||0,10)||0))}}if(!hasPublicSession(e))return void(document.getElementById("loyalty-modal-points").textContent=String(t));const o=await fetch(`${n}?sheet=user_points`);if(!o.ok)return void(document.getElementById("loyalty-modal-points").textContent=String(t));const r=await o.json();if(r&&typeof r=="object"&&String(r.error||"").toLowerCase()==="unauthorized")return console.warn("\u26A0\uFE0F Unauthorized access to user_points (modal). Falling back to user profile points."),void(document.getElementById("loyalty-modal-points").textContent=String(t));const s=Array.isArray(r)?r.filter(l=>normalizePhoneTo08(l.phone)===normalizePhoneTo08(e.whatsapp)):[];if(s&&s.length>0){const l=s[0],i=parseInt(l.points||l.poin||0);document.getElementById("loyalty-modal-points").textContent=i}else document.getElementById("loyalty-modal-points").textContent=String(t)}catch(n){console.error("Error loading modal points:",n),document.getElementById("loyalty-modal-points").textContent=String(t)}}createOrderCard=function(e){const t=originalCreateOrderCard(e);return t.style.cursor="pointer",t.addEventListener("click",()=>{showOrderDetailModal(e)}),t};var escapeHtml=window.FrontendSanitize&&window.FrontendSanitize.escapeHtml||(e=>{const t=document.createElement("div");return t.textContent=e,t.innerHTML}),sanitizeUrl=window.FrontendSanitize&&window.FrontendSanitize.sanitizeUrl||(e=>String(e||"")),ensureImageFallbackHandler=window.FrontendSanitize&&window.FrontendSanitize.ensureImageFallbackHandler||(()=>{});function hideRewardLoaders(){const e=document.getElementById("reward-items-loading"),t=document.getElementById("rewards-loading");e&&e.classList.add("hidden"),t&&t.classList.add("hidden")}async function fetchRewardItemsForAkun(){const e=document.getElementById("reward-items-loading"),t=document.getElementById("reward-items-empty"),n=document.getElementById("reward-items-list");if(n){e&&e.classList.remove("hidden"),t&&t.classList.add("hidden"),n.innerHTML="";try{const a=CONFIG.getMainApiUrl(),o=await fetch(`${a}?sheet=tukar_poin`);if(!o.ok)throw new Error("Failed to fetch reward items");const r=await o.json();renderRewardItemsListAkun(parseSheetResponse(r))}catch(a){console.error("Error fetching reward items:",a),t&&t.classList.add("hidden"),n.innerHTML=`
            <div class="text-center py-6 bg-red-50 rounded-2xl border-2 border-dashed border-red-200">
                <p class="text-xs text-red-600 font-semibold">Gagal memuat hadiah. Silakan coba lagi nanti.</p>
            </div>
        `}finally{hideRewardLoaders()}}}function renderRewardItemsListAkun(e){const t=document.getElementById("reward-items-empty"),n=document.getElementById("reward-items-list");if(n){if(!e||e.length===0)return t&&t.classList.remove("hidden"),void(n.innerHTML="");t&&t.classList.add("hidden"),n.innerHTML=e.map(a=>{const o=escapeHtml((a.id||"").toString()),r=escapeHtml((a.nama||a.judul||"Hadiah").toString()),s=parseInt(a.poin||a.Poin||0),l=(a.gambar||"https://via.placeholder.com/80?text=Reward").toString();return`
            <div class="border-2 border-gray-200 rounded-xl p-4 hover:border-amber-500 transition">
                <div class="flex gap-3 mb-3">
                    <!-- Left: Image -->
                    <div class="w-16 h-16 bg-gray-100 rounded-lg overflow-hidden flex-shrink-0">
                        <img src="${sanitizeUrl(l,"https://via.placeholder.com/80?text=Reward")}" alt="${r}" class="w-full h-full object-cover" data-fallback-src="https://via.placeholder.com/80?text=Reward">
                    </div>
                    ...
                    <div class="flex-shrink-0">
                        <span class="bg-amber-100 text-amber-700 text-xs font-bold px-3 py-1 rounded-full whitespace-nowrap">${s} Poin</span>
                    </div>
                </div>
