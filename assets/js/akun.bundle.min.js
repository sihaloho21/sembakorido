window.FrontendSanitize={escapeHtml:function(e){return String(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")},sanitizeUrl:function(e,t=""){if(!e)return t;const n=String(e).trim();return/^(https?:\/\/|\/(?!\/)|\.\/|\.\.\/|#)/i.test(n)?n:t},ensureImageFallbackHandler:function(){window.__imageFallbackHandlerAdded||(window.__imageFallbackHandlerAdded=!0,document.addEventListener("error",e=>{const t=e.target;if(!t||t.tagName!=="IMG")return;const n=t.getAttribute("data-fallback-src"),a=t.getAttribute("data-fallback-action");n&&t.src!==n?t.src=n:a==="hide"&&(t.style.display="none")},!0))}},window.sanitizeUrl||(window.sanitizeUrl=window.FrontendSanitize.sanitizeUrl),window.ensureImageFallbackHandler||(window.ensureImageFallbackHandler=window.FrontendSanitize.ensureImageFallbackHandler),window.AdminSanitize||(window.AdminSanitize=window.FrontendSanitize);var escapeHtml=window.FrontendSanitize&&window.FrontendSanitize.escapeHtml||(e=>String(e||"")),sanitizeUrl=window.FrontendSanitize&&window.FrontendSanitize.sanitizeUrl||(e=>String(e||"")),ensureImageFallbackHandler=window.FrontendSanitize&&window.FrontendSanitize.ensureImageFallbackHandler||(()=>{});function getApiUrl(){const e=CONFIG.getMainApiUrl();return console.log("Using API URL:",e),e}ensureImageFallbackHandler();let API_URL=getApiUrl();function refreshApiUrl(){const e=API_URL;API_URL=getApiUrl(),console.log("Old API URL:",e),console.log("New API URL:",API_URL),e!==API_URL&&(console.log("\u{1F504} API URL changed, clearing cache and reloading..."),typeof ApiService<"u"&&ApiService.clearCache()),fetchProducts()}let cart=JSON.parse(localStorage.getItem("sembako_cart"))||[],allProducts=[],currentCategory="Semua",currentPage=1;const itemsPerPage=12;let filteredProducts=[],storeClosed=CONFIG.isStoreClosed(),selectedVariation=null,currentModalProduct=null;function normalizePhoneNumber(e){if(!e)return null;let t=String(e).replace(/[^0-9]/g,"");return t.startsWith("62")&&(t="0"+t.substring(2)),t.startsWith("8")&&!t.startsWith("08")&&(t="0"+t),!t.startsWith("08")||t.length<10||t.length>13?null:t}function createSlug(e){return e?e.toLowerCase().replace(/[^\w\s-]/g,"").trim().replace(/[-\s]+/g,"-"):""}function ensureProductId(e,t){const n=e.id||e.sku||e.slug||createSlug(e.nama)||"product";return e.id||e.sku?String(n):`${n}-${t}`}function findProductById(e){return e&&allProducts.find(t=>String(t.productId)===String(e))||null}async function fetchProducts(){try{const e=await ApiService.get("?sheet=products",{cacheDuration:3e5});console.log("Products received:",e),allProducts=e.map((t,n)=>{const a=parseInt(t.harga)||0,r=typeof calculateGajianPrice=="function"?calculateGajianPrice(a):{price:a,daysLeft:0,markupPercent:0};let s=t.kategori||"Bahan Pokok";t.kategori||(a>=15e4?s="Paket Lengkap":a>=5e4&&(s="Paket Hemat"));let o=[];if(t.variasi)try{o=JSON.parse(t.variasi)}catch(i){console.error("Error parsing variations for product:",t.id,i)}const d=t.slug||createSlug(t.nama),l=ensureProductId({...t,slug:d},n);return{...t,slug:d,productId:l,harga:a,hargaCoret:parseInt(t.harga_coret)||0,hargaGajian:r.price,stok:parseInt(t.stok)||0,category:s,deskripsi:t.deskripsi&&t.deskripsi.trim()!==""?t.deskripsi:"Kualitas Terjamin, Stok Selalu Baru, Harga Kompetitif",variations:o}}),renderCategoryFilters(),filterProducts(),updateCartUI(),checkStoreStatus(),startNotificationLoop()}catch(e){console.error("Error fetching products:",e);const t=document.getElementById("product-grid");t&&(t.innerHTML='<p class="text-center col-span-full text-red-500">Gagal memuat produk. Silakan coba lagi nanti.</p>')}}function renderCategoryFilters(){const e=document.getElementById("category-filters");if(!e||!allProducts||allProducts.length===0)return;console.log("\u{1F3F7}\uFE0F Rendering category filters...");const t=new Set;allProducts.forEach(r=>{r.category&&r.category.trim()!==""&&t.add(r.category.trim())});const n=Array.from(t).sort();console.log("\u{1F4CA} Categories found:",n);let a='<button type="button" data-action="set-category" data-category="Semua" class="filter-btn active snap-start flex-shrink-0 px-6 py-2 rounded-full border-2 border-green-500 bg-green-50 text-green-700 text-sm font-bold transition hover:border-green-600 hover:bg-green-100">Semua</button>';n.forEach(r=>{const s=escapeHtml(r);a+=`<button type="button" data-action="set-category" data-category="${s}" class="filter-btn snap-start flex-shrink-0 px-6 py-2 rounded-full border-2 border-gray-300 bg-white text-gray-700 text-sm font-bold transition hover:border-green-500 hover:bg-green-50">${s}</button>`}),e.innerHTML=a,console.log("\u2705 Category filters rendered:",n.length,"categories"),initCategoryCarouselScroll()}function initCategoryCarouselScroll(){const e=document.getElementById("category-filters");if(!e)return;e.addEventListener("wheel",r=>{r.deltaY!==0&&(r.preventDefault(),e.scrollLeft+=r.deltaY)},{passive:!1});let t,n,a=!1;e.addEventListener("mousedown",r=>{r.target.classList.contains("filter-btn")||(a=!0,e.style.cursor="grabbing",t=r.pageX-e.offsetLeft,n=e.scrollLeft)}),e.addEventListener("mouseleave",()=>{a=!1,e.style.cursor="grab"}),e.addEventListener("mouseup",()=>{a=!1,e.style.cursor="grab"}),e.addEventListener("mousemove",r=>{if(!a)return;r.preventDefault();const s=2*(r.pageX-e.offsetLeft-t);e.scrollLeft=n-s}),e.style.cursor="grab",updateCategoryArrowsVisibility(),e.addEventListener("scroll",updateCategoryArrowsVisibility)}function scrollCategoryCarousel(e){const t=document.getElementById("category-filters");t&&(e==="left"?t.scrollLeft-=300:e==="right"&&(t.scrollLeft+=300))}function updateCategoryArrowsVisibility(){const e=document.getElementById("category-filters"),t=document.getElementById("category-scroll-left"),n=document.getElementById("category-scroll-right");if(!e||!t||!n)return;const a=e.scrollLeft<=10,r=e.scrollLeft+e.clientWidth>=e.scrollWidth-10;a?t.classList.add("opacity-0","pointer-events-none"):t.classList.remove("opacity-0","pointer-events-none"),r?n.classList.add("opacity-0","pointer-events-none"):n.classList.remove("opacity-0","pointer-events-none")}function findProductBySlug(e){return e&&allProducts&&allProducts.length!==0?allProducts.find(t=>t.slug===e):null}function renderProducts(e){const t=document.getElementById("product-grid");if(!t)return;if(e.length===0)return t.innerHTML='<p class="text-center col-span-full text-gray-500 py-10">Tidak ada produk yang ditemukan.</p>',void(document.getElementById("pagination-container").innerHTML="");const n=Math.ceil(e.length/12);currentPage>n&&(currentPage=n||1);const a=12*(currentPage-1),r=a+12,s=e.slice(a,r);t.innerHTML="",s.forEach(o=>{let d="";d=o.stok>10?'<span class="bg-green-100 text-green-700 text-[10px] px-2 py-0.5 rounded-full font-bold">Stok Tersedia</span>':o.stok>5?`<span class="bg-yellow-100 text-yellow-700 text-[10px] px-2 py-0.5 rounded-full font-bold">Stok Menipis (${o.stok})</span>`:o.stok>0?`<span class="bg-orange-100 text-orange-700 text-[10px] px-2 py-0.5 rounded-full font-bold">Hanya sisa ${o.stok}</span>`:'<span class="bg-red-100 text-red-700 text-[10px] px-2 py-0.5 rounded-full font-bold">Stok Habis</span>';const l=(o.gambar?o.gambar.split(","):[])[0]||"https://placehold.co/300x200?text=Produk",i=sanitizeUrl(l,"https://placehold.co/300x200?text=Produk"),u=calculateRewardPoints(o.harga,o.nama);let p="",m=!1;if(o.grosir)try{const y=JSON.parse(o.grosir);Array.isArray(y)&&y.length>0&&(m=!0,p=`
            ...
            <div class="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition duration-300 relative" data-product-id="${h}">
                ...
                <div class="lazy-image-wrapper">
                    ...
                    <img src="${i}" alt="${escapeHtml(o.nama)}" data-action="show-detail" data-product-id="${h}" class="w-full h-48 object-cover cursor-pointer hover:opacity-90 transition-opacity ${o.stok===0?"grayscale opacity-60":""}" loading="lazy" data-fallback-src="https://placehold.co/300x200?text=Produk" onload="this.classList.add('loaded'); this.previousElementSibling.style.display='none';">
                </div>
                <div class="p-6">
                    <div class="flex justify-between items-start mb-2">
                        <h4 class="text-lg font-bold text-gray-800">${escapeHtml(o.nama)}</h4>
                        ${d}
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-6">
                        <div class="bg-green-50 p-3 rounded-lg">
                            ...
                            <div class="flex flex-col">
                                ${c}
                                <p class="text-lg font-bold text-green-700">Rp ${o.harga.toLocaleString("id-ID")}</p>
                            </div>
                        </div>
                        <div class="bg-blue-50 p-3 rounded-lg">
                            ...
                            <div class="flex flex-col">
                                ...
                                <p class="text-lg font-bold text-blue-700">Rp ${o.hargaGajian.toLocaleString("id-ID")}</p>
                            </div>
                        </div>
                    </div>
                    ${p}
                    ...
                    `:`
                    <button data-action="add-to-cart" data-product-id="${h}" ${o.stok===0?"disabled":""} class="w-full bg-green-600 hover:bg-green-700 disabled:bg-gray-300 text-white font-bold py-3 rounded-xl transition flex items-center justify-center gap-2 mb-3">
    ...
    `).join(""),a.classList.remove("hidden")}function closeSearchSuggestions(){[document.getElementById("search-suggestions"),document.getElementById("search-suggestions-header")].forEach(e=>{e&&(e.classList.add("hidden"),e.innerHTML="")})}function getSearchSuggestions(e,t=6){const n=tokenize(e);if(n.length===0)return[];const a=allProducts.map(s=>{const o=[...tokenize(s.nama),...tokenize(s.deskripsi||"")];let d=0;return n.forEach(l=>{o.forEach(i=>{i===l?d=Math.max(d,3):i.includes(l)?d=Math.max(d,2):l.length>2&&i.length>2&&levenshtein(i,l)<=1&&(d=Math.max(d,1))})}),{name:s.nama,score:d}}).filter(s=>s.score>0),r=new Map;return a.sort((s,o)=>o.score-s.score).forEach(s=>{r.has(s.name)||r.set(s.name,s)}),Array.from(r.values()).slice(0,t).map(s=>s.name)}function levenshtein(e,t){const n=e.length,a=t.length;if(n===0)return a;if(a===0)return n;const r=Array.from({length:n+1},()=>new Array(a+1).fill(0));for(let s=0;s<=n;s+=1)r[s][0]=s;for(let s=0;s<=a;s+=1)r[0][s]=s;for(let s=1;s<=n;s+=1)for(let o=1;o<=a;o+=1){const d=e[s-1]===t[o-1]?0:1;r[s][o]=Math.min(r[s-1][o]+1,r[s][o-1]+1,r[s-1][o-1]+d)}return r[n][a]}function renderPagination(e){const t=document.getElementById("pagination-container");if(!t)return;const n=Math.ceil(e/12);if(n<=1)return void(t.innerHTML="");let a="";a+=`
    ...
    `,n.appendChild(a),document.body.appendChild(n);const s=a.querySelector('[data-action="dismiss-notification"]');if(s&&s.addEventListener("click",()=>n.remove()),!document.getElementById("success-notification-styles")){const o=document.createElement("style");o.id="success-notification-styles",o.textContent=`
            @keyframes fadeIn {
                from { opacity: 0; }
                to { opacity: 1; }
            }
        ...
        `,document.head.appendChild(o)}n.addEventListener("click",o=>{o.target===n&&(n.style.animation="fadeOut 0.3s ease-out",setTimeout(()=>n.remove(),300))})}function saveCart(){localStorage.setItem("sembako_cart",JSON.stringify(cart))}function updateCartUI(){const e=cart.reduce((s,o)=>s+o.qty,0),t=document.getElementById("cart-count");t&&(e>0?(t.innerText=e,t.classList.remove("hidden")):t.classList.add("hidden"));const n=document.getElementById("cart-items"),a=document.getElementById("cart-footer"),r=document.getElementById("cart-empty");if(n)if(cart.length===0)n.innerHTML="",a&&a.classList.add("hidden"),r&&r.classList.remove("hidden");else{r&&r.classList.add("hidden"),a&&a.classList.remove("hidden");let s=0;n.innerHTML=cart.map((i,u)=>{const p=calculateTieredPrice(i.harga,i.qty,i.grosir),m=p<i.harga,c=p*i.qty;s+=c;let g=(i.gambar?i.gambar.split(","):[])[0]||"https://placehold.co/100x100?text=Produk";return i.selectedVariation&&i.selectedVariation.gambar&&(g=i.selectedVariation.gambar),`
                <div class="flex items-center gap-4 bg-gray-50 p-3 rounded-xl">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="flex flex-col">
                            ${m?`<span class="text-[10px] text-gray-400 line-through">Rp ${i.harga.toLocaleString("id-ID")}</span>`:""}
                            <p class="text-green-600 font-bold text-xs">Rp ${p.toLocaleString("id-ID")} ${m?'<span class="bg-green-100 text-green-700 text-[8px] px-1 rounded ml-1">Grosir</span>':""}</p>
                        </div>
            ...
            `}).join("");const o=document.getElementById("cart-total");o&&(o.innerText=`Rp ${s.toLocaleString("id-ID")}`);const d=document.getElementById("order-summary-total");d&&(d.innerText=`Rp ${s.toLocaleString("id-ID")}`);const l=document.getElementById("sticky-order-total");l&&(l.innerText=`Rp ${s.toLocaleString("id-ID")}`)}}function updateQty(e,t){cart[e].qty+=t,cart[e].qty<1&&cart.splice(e,1),saveCart(),updateCartUI()}function removeItem(e){cart.splice(e,1),saveCart(),updateCartUI()}function openCartModal(){const e=document.getElementById("cart-modal");e&&(e.classList.remove("hidden"),document.body.classList.add("modal-active"))}function closeCartModal(){const e=document.getElementById("cart-modal");e&&(e.classList.add("hidden"),document.body.classList.remove("modal-active"))}function updateModalQty(e){const t=document.getElementById("modal-qty");if(!t)return;let n=parseInt(t.value)||1;n+=e,n<1&&(n=1),t.value=n,t.oninput({target:t})}function showDetail(e){closeWishlistModal(),console.log("showDetail called for product:",e.nama);const t=document.getElementById("detail-modal");if(!t)return;currentModalProduct=e,t.dataset.productId=e.productId||"",selectedVariation=null;const n=document.getElementById("modal-qty");n&&(n.value=1);const a=document.getElementById("modal-product-name"),r=document.getElementById("modal-product-image"),s=(document.getElementById("modal-cash-price"),document.getElementById("modal-gajian-price"),document.getElementById("modal-price-date")),o=document.getElementById("modal-items-list"),d=document.getElementById("modal-badges"),l=(document.getElementById("savings-highlight"),document.getElementById("savings-amount"),document.getElementById("modal-variation-container"));a&&(a.innerText=e.nama),n&&(n.oninput=u=>{const p=parseInt(u.target.value)||1,m=selectedVariation?selectedVariation.harga:e.harga,c=selectedVariation?selectedVariation.grosir:e.grosir,g=selectedVariation?selectedVariation.harga_coret||0:e.hargaCoret||0;typeof updateTieredPricingUI=="function"&&updateTieredPricingUI({...e,harga:m,grosir:c},p);const h=typeof calculateTieredPrice=="function"?calculateTieredPrice(m,p,c):m;updateModalPrices(h*p,(typeof calculateGajianPrice=="function"?calculateGajianPrice(h):{price:h}).price*p,g*p)}),l?e.variations&&e.variations.length>0?(l.classList.remove("hidden"),document.getElementById("modal-variation-list").innerHTML=e.variations.map((u,p)=>`
                    ...
                    <p class="text-xs font-bold text-gray-800">${escapeHtml(u.nama)}</p>
                    <p class="text-[10px] text-green-600 font-bold">Rp ${u.harga.toLocaleString("id-ID")}</p>
                    ${u.stok<=0?'<p class="text-[8px] text-red-500 font-bold">Stok Habis</p>':""}
                </button>
            `).join(""),selectVariation(e.variations[0],0)):(l.classList.add("hidden"),updateModalPrices(e.harga,e.hargaGajian,e.hargaCoret)):updateModalPrices(e.harga,e.hargaGajian,e.hargaCoret),s&&(s.innerText=`Harga Per Tgl ${new Date().toLocaleDateString("id-ID",{day:"2-digit",month:"2-digit",year:"numeric"}).replace(/\//g,"-")}`),d&&(d.innerHTML=`
        ...
        `);const i=e.gambar?e.gambar.split(","):[];if(typeof initializeSlider=="function"?initializeSlider(i):r&&(r.src=i.length>0?i[0]:"https://placehold.co/300x200?text=Produk",r.onerror=function(){this.src="https://placehold.co/300x200?text=Produk"}),o){const u=String(e.deskripsi||"").replace(/\r\n?/g,`
`).replace(/\\r\\n/g,`
`).replace(/\\n/g,`
`).split(`
`).map(m=>m.trim()).filter(m=>m!==""&&!/^isi paket\s*:\s*$/i.test(m)).map(m=>m.replace(/^[\-•]\s*/,"").trim()).filter(m=>m!==""),p=["\u{1F35C}","\u{1F372}","\u{1F4E6}","\u2615","\u{1F35A}","\u{1F373}","\u{1F9C2}"];o.innerHTML=u.map((m,c)=>`
            <div class="flex items-center gap-4 bg-gray-50/50 p-3 rounded-xl border border-gray-100/50">
                <span class="text-xl">${p[c%p.length]}</span>
                <span class="text-sm font-medium text-gray-700">\u2022${escapeHtml(m)}</span>
            </div>
        `).join("")}typeof updateTieredPricingUI=="function"&&updateTieredPricingUI(e,1),t.classList.remove("hidden"),document.body.classList.add("modal-active")}function selectVariation(e,t){if(selectedVariation=e,document.querySelectorAll(".variation-btn").forEach((a,r)=>{r===t?(a.classList.add("border-green-500","bg-green-50"),a.classList.remove("border-gray-200")):(a.classList.remove("border-green-500","bg-green-50"),a.classList.add("border-gray-200"))}),e.gambar&&typeof initializeSlider=="function"){const a=e.gambar.split(",");initializeSlider(a)}const n=document.getElementById("modal-qty");if(n)n.oninput({target:n});else{const a=typeof calculateGajianPrice=="function"?calculateGajianPrice(e.harga):{price:e.harga,daysLeft:0,markupPercent:0};updateModalPrices(e.harga,a.price,e.harga_coret||0)}}function updateModalPrices(e,t,n){const a=document.getElementById("modal-cash-price"),r=document.getElementById("modal-gajian-price"),s=document.getElementById("savings-highlight"),o=document.getElementById("savings-amount");a&&(a.innerText=`Rp ${e.toLocaleString("id-ID")}`),r&&(r.innerText=`Rp ${t.toLocaleString("id-ID")}`),n>e?(s&&s.classList.remove("hidden"),o&&(o.innerText=`Rp ${(n-e).toLocaleString("id-ID")}`)):s&&s.classList.add("hidden")}function closeDetailModal(){const e=document.getElementById("detail-modal");e&&(e.classList.add("hidden"),document.body.classList.remove("modal-active")),selectedVariation=null}function directOrder(e){storeClosed?showStoreWarning(()=>{proceedDirectOrder(e)}):proceedDirectOrder(e)}function proceedDirectOrder(e){if(e.variations&&e.variations.length>0&&!selectedVariation)return void showDetail(e);const t={...e};if(selectedVariation){t.selectedVariation=selectedVariation,t.harga=selectedVariation.harga,t.sku=selectedVariation.sku,t.stok=selectedVariation.stok;const r=typeof calculateGajianPrice=="function"?calculateGajianPrice(selectedVariation.harga):{price:selectedVariation.harga,daysLeft:0,markupPercent:0};t.hargaGajian=r.price}const n=document.getElementById("modal-qty"),a=n&&parseInt(n.value)||1;cart=[{...t,qty:a}],saveCart(),updateCartUI(),openOrderModal(),selectedVariation=null}function directOrderFromModal(){const e=document.getElementById("detail-modal"),t=findProductById(e?e.dataset.productId:null);t&&(directOrder(t),closeDetailModal())}function checkStoreStatus(){storeClosed=CONFIG.isStoreClosed();const e=document.getElementById("store-closed-banner"),t=document.getElementById("main-header");storeClosed?(e&&e.classList.remove("hidden"),t&&(t.style.top="36px"),sessionStorage.getItem("store_closed_modal_shown")||setTimeout(()=>{document.getElementById("store-closed-modal").classList.remove("hidden"),sessionStorage.setItem("store_closed_modal_shown","true")},1e3)):(e&&e.classList.add("hidden"),t&&(t.style.top="0"))}function closeStoreClosedModal(){document.getElementById("store-closed-modal").classList.add("hidden")}function showStoreWarning(e){const t=document.getElementById("store-warning-modal"),n=document.getElementById("confirm-store-warning");t.classList.remove("hidden"),n.onclick=()=>{t.classList.add("hidden"),e&&e()}}function closeStoreWarningModal(){document.getElementById("store-warning-modal").classList.add("hidden")}function openOrderModal(){if(cart.length===0)return;closeCartModal(),prefillCustomerInfo();const e=document.getElementById("order-summary"),t=document.querySelector('input[name="pay-method"]:checked'),n=t&&t.value==="Bayar Gajian";if(e){let r=0;e.innerHTML=cart.map(s=>{const o=n?s.hargaGajian:s.harga,d=calculateTieredPrice(o,s.qty,s.grosir),l=d<o,i=calculateRewardPoints(s.harga,s.nama)*s.qty;return r+=i,`
                <div class="flex justify-between items-center py-1">
                    ...
                    <div class="flex flex-col items-end">
                        ${l?`<span class="text-[10px] text-gray-400 line-through">Rp ${(o*s.qty).toLocaleString("id-ID")}</span>`:""}
    ...
    `,document.body.appendChild(t),setTimeout(()=>t.remove(),5e3)}function toggleLocationField(){const e=document.querySelector('input[name="ship-method"]:checked'),t=document.getElementById("location-field"),n=document.getElementById("delivery-location-ui"),a=document.getElementById("pickup-location-ui");e&&(e.value==="Antar Nikomas"?(t.classList.add("hidden"),n.classList.add("hidden"),a.classList.add("hidden")):e.value==="Antar Kerumah"?(t.classList.remove("hidden"),n.classList.remove("hidden"),a.classList.add("hidden")):e.value==="Ambil Ditempat"&&(t.classList.remove("hidden"),n.classList.add("hidden"),a.classList.remove("hidden"))),updateOrderTotal()}function getCurrentLocation(){const e=document.getElementById("get-location-btn"),t=document.getElementById("location-link");navigator.geolocation?(e.disabled=!0,e.innerHTML="<span>\u231B Mengambil Lokasi...</span>",navigator.geolocation.getCurrentPosition(n=>{const a=`https://www.google.com/maps?q=${n.coords.latitude},${n.coords.longitude}`;t.value=a,e.classList.remove("bg-white","text-blue-700","border-blue-200"),e.classList.add("bg-green-600","text-white","border-green-600"),e.innerHTML="<span>\u2705 Lokasi Berhasil Dibagikan</span>",alert("Lokasi berhasil diambil!")},n=>{e.disabled=!1,e.innerHTML="<span>\u{1F4CD} Bagikan Lokasi Saya</span>";let a="Gagal mengambil lokasi.";n.code===1&&(a="Mohon izinkan akses lokasi untuk fitur ini."),alert(a)},{enableHighAccuracy:!0,timeout:5e3,maximumAge:0})):alert("Geolocation tidak didukung oleh browser Anda")}function updateOrderTotal(){const e=document.querySelector('input[name="pay-method"]:checked'),t=document.querySelector('input[name="ship-method"]:checked'),n=e&&e.value==="Bayar Gajian",a=t&&t.value==="Antar Nikomas";let r=0;cart.forEach(p=>{const m=n?p.hargaGajian:p.harga,c=calculateTieredPrice(m,p.qty,p.grosir);r+=c*p.qty});const s=a?2e3:0,o=r+s,d=document.getElementById("sticky-order-total"),l=document.getElementById("order-summary-total");d&&(d.innerText=`Rp ${o.toLocaleString("id-ID")}`),l&&(l.innerText=`Rp ${o.toLocaleString("id-ID")}`);const i=document.getElementById("order-subtotal"),u=document.getElementById("order-shipping");i&&(i.innerText=`Rp ${r.toLocaleString("id-ID")}`),u&&(u.innerText=`Rp ${s.toLocaleString("id-ID")}`),updateOrderCTAState()}function isOrderFormValid(){const e=document.getElementById("customer-name"),t=document.getElementById("customer-phone"),n=document.querySelector('input[name="pay-method"]:checked'),a=document.querySelector('input[name="ship-method"]:checked');if(!e||!t)return!1;const r=e.value.trim(),s=r.replace(/\s/g,"");if(!r||s.length<4)return!1;const o=t.value.trim();return o?!(normalizePhone(o).replace(/[^0-9]/g,"").length<10)&&!!(n&&a):!1}function updateOrderCTAState(){const e=document.getElementById("send-order-btn")||document.querySelector('[data-action="send-wa"]');if(!e)return;const t=isOrderFormValid();e.disabled=!t,e.setAttribute("aria-disabled",String(!t))}function normalizePhone(e){if(!e)return"";let t=e.toString().replace(/[^0-9]/g,"");return t.startsWith("62")?t="0"+t.slice(2):t.startsWith("8")?t="0"+t:t.startsWith("0")||(t="0"+t),t.startsWith("0")&&!t.startsWith("08")&&t.length,t}function generateOrderId(){const e="0123456789";let t="";for(let n=0;n<6;n++)t+=e.charAt(Math.floor(10*Math.random()));return`ORD-${t}`}async function logOrderToGAS(e){const t={id:e.id||generateOrderId(),pelanggan:e.pelanggan||"",produk:e.produk||"",qty:e.qty||0,total:e.total||0,status:e.status||"Pending",tanggal:e.tanggal||new Date().toLocaleString("id-ID"),phone:e.phone||"",poin:e.poin||0,point_processed:e.point_processed||"No"};console.log("\u{1F4DD} Logging order to GAS:",t);try{const n=await GASActions.create("orders",t);return console.log("\u2705 Order logged successfully:",n),n}catch(n){throw console.error("\u274C Error logging order to GAS:",n),n}}async function sendToWA(){const e=event?.target||document.querySelector('[data-action="send-wa"]');if(e&&e.disabled)return;const t=document.getElementById("customer-name").value,n=document.getElementById("customer-phone").value,a=document.querySelector('input[name="pay-method"]:checked')?.value,r=document.querySelector('input[name="ship-method"]:checked')?.value;if(!t||t.trim().length===0)return void alert("Masukkan Nama Lengkap");const s=t.replace(/\s/g,"");if(s.length<4)return void alert("Masukkan Nama Lengkap");const o=s.toLowerCase(),d=[/^(.)\1{3,}$/,/^(.{2})\1{2,}$/,/^(.{3})\1{2,}$/,/^([a-z])([a-z])\1\2{2,}$/];for(const b of d)if(b.test(o))return void alert("Masukkan Nama Lengkap");if(!n||n.trim().length===0)return void alert("Nomor WhatsApp tidak valid");const l=n.replace(/[^0-9]/g,"");if(l.length<10)return void alert("Nomor WhatsApp tidak valid");const i=[/^(\d)\1{9,}$/,/^08(\d)\1{8,}$/,/^(\d{2})\1{4,}$/,/^(\d{3})\1{3,}$/];for(const b of i)if(b.test(l))return void alert("Nomor WhatsApp tidak valid");if(!a)return void alert("Pilih metode pembayaran terlebih dahulu");if(!r)return void alert("Pilih metode pengiriman terlebih dahulu");e&&(e.disabled=!0,e.innerHTML=`
            <svg class="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
            </svg>
            <span>Memproses Pesanan...</span>
        `,e.classList.add("opacity-75","cursor-not-allowed"));let u="";u=r==="Antar Nikomas"?"Antar Nikomas (Area PT Nikomas Gemilang)":r==="Antar Kerumah"?"Antar Kerumah (Area Serang & sekitarnya)":"Ambil Ditempat (Kp. Baru, Kec. Kibin)";const p=a==="Bayar Gajian";let m=0,c=0,g="",h="";cart.forEach((b,E)=>{const w=p?b.hargaGajian:b.harga,I=calculateTieredPrice(w,b.qty,b.grosir),B=I<w,$=I*b.qty;m+=$,c+=b.qty;const S=b.selectedVariation?` (${b.selectedVariation.nama})`:"",A=B?` (Harga Grosir: Rp ${I.toLocaleString("id-ID")}/unit)`:"";g+=`${E+1}. ${b.nama}${S} x${b.qty}${A} = Rp ${$.toLocaleString("id-ID")}
`,h+=`${b.nama}${S} (x${b.qty}) | `});const f=r==="Antar Nikomas"?2e3:0;m+=f;const y=CONFIG.getRewardConfig().pointValue||1e4,x=Math.floor(m/y),v=generateOrderId(),k=document.getElementById("location-link")?.value||"",C=`*PESANAN BARU - GOSEMBAKO*
...
Mohon segera diproses ya, terima kasih!`,M=`https://wa.me/628993370200?text=${encodeURIComponent(C)}`,L={id:v,pelanggan:t,produk:h.slice(0,-3),qty:c,total:m,status:"Pending",tanggal:new Date().toLocaleString("id-ID"),phone:normalizePhone(n),poin:x,point_processed:"No"};try{await logOrderToGAS(L),console.log("\u2705 Order logged to spreadsheet successfully"),cart=[],saveCart(),updateCartUI(),closeOrderModal(),showSuccessNotification(v,M)}catch(b){console.error("\u274C Error logging order:",b),alert("Gagal menyimpan pesanan. Silakan coba lagi atau hubungi admin.")}finally{e&&setTimeout(()=>{e.disabled=!1,e.innerHTML=`
        ...
        `}finally{hideGlobalRewardLoader()}}function renderRewardItems(e){const t=document.getElementById("reward-items-list");t&&(e&&e.length!==0?t.innerHTML=e.map(n=>{const a=n.id||"",r=n.nama||n.judul||"Hadiah",s=n.poin||0,o=n.gambar||"https://via.placeholder.com/100?text=Reward",d=sanitizeUrl(o,"https://via.placeholder.com/100?text=Reward"),l=n.deskripsi||"",i=escapeHtml(r);return`
        ...
        `)}function closeRewardModal(){const e=document.getElementById("reward-modal");e&&(e.classList.add("hidden"),document.body.classList.remove("modal-active"))}function checkUserPoints(){const e=document.getElementById("reward-phone"),t=e?e.value.trim():"";if(!t)return void alert("Mohon masukkan nomor WhatsApp.");const n=normalizePhone(t),a=event.target,r=a?a.innerText:"Cek Poin";a&&a.tagName==="BUTTON"&&(a.innerText="Mencari...",a.disabled=!0),ApiService.get("?sheet=user_points",{cache:!1}).then(s=>{const o=s.find(i=>normalizePhone(i.phone||i.whatsapp||"")===n),d=document.getElementById("points-display"),l=document.querySelector("#points-display h4");if(o){const i=(o.points||o.poin||"0").toString().replace(",","."),u=parseFloat(i)||0;l.innerHTML=`${escapeHtml(u.toFixed(1))} <span class="text-sm font-bold">Poin</span>`,sessionStorage.setItem("user_points",u),sessionStorage.setItem("reward_phone",n),sessionStorage.setItem("reward_customer_name",o.nama||o.pelanggan||n),showToast(`Ditemukan ${u.toFixed(1)} poin untuk nomor ini!`)}else l.innerHTML=`${escapeHtml("0.0")} <span class="text-sm font-bold">Poin</span>`,sessionStorage.setItem("user_points",0),sessionStorage.setItem("reward_phone",n),sessionStorage.setItem("reward_customer_name",n),showToast("Nomor tidak ditemukan atau belum memiliki poin.");d.classList.remove("hidden")}).catch(s=>{console.error("Error checking points:",s),alert("Gagal mengecek poin. Silakan coba lagi.")}).finally(()=>{a&&a.tagName==="BUTTON"&&(a.innerText=r,a.disabled=!1)})}async function claimReward(e){const t=sessionStorage.getItem("reward_phone");if(t)try{const n=await ApiService.get(`?sheet=tukar_poin&action=search&id=${e}`,{cache:!1});if(!n||n.length===0)return void alert("Data hadiah tidak ditemukan.");const a=n[0],r=parseFloat(a.poin)||0,s=a.nama||a.judul||"Hadiah",o=parseFloat(sessionStorage.getItem("user_points"))||0,d=`Tukar ${r} poin Anda dengan "${s}"?
Saldo poin saat ini: ${o.toFixed(1)}`;if(!confirm(d))return;let l=prompt("Silakan masukkan nama Anda untuk klaim ini:",sessionStorage.getItem("reward_customer_name")||"");if(l===null)return;l=l.trim()||"Pelanggan",showToast("Sedang memproses penukaran...");const i=`RW-${Date.now()}-${Math.floor(1e5*Math.random())}`,u=await GASActions.post({action:"claim_reward",sheet:"tukar_poin",data:{reward_id:e,phone:t,customer_name:l,request_id:i}}),p=u.claim_id||"CLM-"+Date.now().toString().slice(-6),m=parseFloat(u.points_used||r)||0,c=parseFloat(u.balance_after),g=Number.isNaN(c)?Math.max(0,o-m):c;sessionStorage.setItem("user_points",g);const h=document.querySelector("#points-display h4");h&&(h.innerHTML=`${escapeHtml(g.toFixed(1))} <span class="text-sm font-bold">Poin</span>`);const f=`*KLAIM REWARD POIN BERHASIL*
...
Mohon segera diproses. Terima kasih!`,y=`https://wa.me/628993370200?text=${encodeURIComponent(f)}`;showToast("Penukaran poin berhasil!"),setTimeout(()=>{window.open(y,"_blank")},1500)}catch(n){console.error("Error in claimReward:",n);const a=buildClaimRewardErrorMessage(n);alert(a)}else alert("Mohon cek poin Anda terlebih dahulu.")}document.addEventListener("DOMContentLoaded",()=>{fetchProducts(),fetchTukarPoin();const e=document.getElementById("product-grid");e&&e.addEventListener("click",c=>{const g=c.target.closest("[data-action]");if(!g||g.disabled)return;const h=g.getAttribute("data-action"),f=g.getAttribute("data-product-id")||g.closest("[data-product-id]")?.getAttribute("data-product-id"),y=findProductById(f);switch(h){case"toggle-wishlist":f&&toggleWishlist(f);break;case"show-detail":y&&showDetail(y);break;case"add-to-cart":y&&addToCart(y,c);break;case"direct-order":y&&directOrder(y);break;case"share-product":y&&shareProduct(y.nama)}});const t=document.getElementById("category-filters");t&&t.addEventListener("click",c=>{const g=c.target.closest('[data-action="set-category"]');if(!g)return;const h=g.getAttribute("data-category");h&&setCategory(h)});const n=document.getElementById("pagination-container");n&&n.addEventListener("click",c=>{const g=c.target.closest('[data-action="change-page"]');if(!g||g.disabled)return;const h=parseInt(g.getAttribute("data-page"),10);Number.isNaN(h)||changePage(h)});const a=document.getElementById("modal-add-cart");a&&a.addEventListener("click",c=>{const g=document.getElementById("detail-modal"),h=findProductById(g?g.dataset.productId:null);if(h){const f=document.getElementById("modal-qty");addToCart(h,c,f?parseInt(f.value):1),closeDetailModal()}});const r=document.getElementById("order-modal");r&&(r.querySelectorAll('input[name="ship-method"], input[name="pay-method"], #customer-name, #customer-phone').forEach(c=>{const g=c.type==="radio"?"change":"input";c.addEventListener(g,updateOrderCTAState)}),updateOrderCTAState());const s=document.getElementById("detail-modal");s&&s.addEventListener("click",c=>{const g=c.target.closest('[data-action="select-variation"]');if(!g||g.disabled)return;const h=parseInt(g.getAttribute("data-index"),10);if(!currentModalProduct||!currentModalProduct.variations||Number.isNaN(h))return;const f=currentModalProduct.variations[h];f&&selectVariation(f,h)});const o=document.getElementById("wishlist-items-container");o&&o.addEventListener("click",c=>{const g=c.target.closest("[data-action]");if(!g||g.disabled)return;const h=g.getAttribute("data-action"),f=g.getAttribute("data-product-id"),y=findProductById(f);h==="wishlist-toggle"&&f?toggleWishlist(f):h==="wishlist-buy"&&y&&showDetail(y)});const d=document.getElementById("cart-items");d&&d.addEventListener("click",c=>{const g=c.target.closest("[data-action]");if(!g||g.disabled)return;const h=g.getAttribute("data-action"),f=parseInt(g.getAttribute("data-index"),10),y=parseInt(g.getAttribute("data-delta"),10);h!=="update-cart-qty"||Number.isNaN(f)||Number.isNaN(y)||updateQty(f,y),h!=="remove-cart-item"||Number.isNaN(f)||removeItem(f)});const l=document.getElementById("search-suggestions");l&&l.addEventListener("click",c=>{const g=c.target.closest('[data-action="search-suggestion"]');if(!g)return;const h=g.getAttribute("data-value")||"",f=document.getElementById("search-input");f&&(f.value=h,filterProducts()),closeSearchSuggestions()});const i=document.getElementById("search-suggestions-header");i&&i.addEventListener("click",c=>{const g=c.target.closest('[data-action="search-suggestion"]');if(!g)return;const h=g.getAttribute("data-value")||"",f=document.getElementById("search-input-header"),y=document.getElementById("search-input");f&&(f.value=h),y&&(y.value=h),filterProducts(),closeSearchSuggestions()});const u=document.getElementById("search-input");u&&u.addEventListener("keydown",c=>{c.key==="Escape"&&closeSearchSuggestions(),c.key==="Enter"&&(closeSearchSuggestions(),c.preventDefault(),u.blur())});const p=document.getElementById("search-input-header");p&&p.addEventListener("keydown",c=>{c.key==="Escape"&&closeSearchSuggestions(),c.key==="Enter"&&(closeSearchSuggestions(),c.preventDefault(),p.blur())}),document.addEventListener("click",c=>{c.target.closest("#search-input")||c.target.closest("#search-suggestions")||c.target.closest("#search-input-header")||c.target.closest("#search-suggestions-header")||closeSearchSuggestions()});const m=document.getElementById("search-input-header");m&&(m.addEventListener("input",c=>{const g=c.target.value||"",h=document.getElementById("search-input");h&&(h.value=g),filterProducts()}),m.addEventListener("focus",()=>{const c=document.getElementById("main-header");c&&c.classList.add("header-search-active")}),m.addEventListener("blur",()=>{const c=document.getElementById("main-header");c&&setTimeout(()=>c.classList.remove("header-search-active"),120)}))});let pendingRewardData={id:null,nama:null,poin:null,gambar:null,deskripsi:null};async function showConfirmTukarModal(e){const t=parseFloat(sessionStorage.getItem("user_points"))||0;if(sessionStorage.getItem("reward_phone"))try{const n=await ApiService.get(`?sheet=tukar_poin&action=search&id=${e}`,{cache:!1});if(!n||n.length===0)return void showToast("Data hadiah tidak ditemukan.");const a=n[0],r=parseFloat(a.poin)||0,s=a.nama||a.judul||"Hadiah";if(t<r)return void showToast(`Poin Anda tidak cukup. Dibutuhkan ${r} poin, saldo Anda ${t.toFixed(1)} poin.`);pendingRewardData={id:e,nama:s,poin:r,gambar:a.gambar||"",deskripsi:a.deskripsi||""},document.getElementById("confirm-reward-name").textContent=s,document.getElementById("confirm-reward-points").textContent=r,document.getElementById("confirm-remaining-points").textContent=(t-r).toFixed(1),document.getElementById("confirm-tukar-modal").classList.remove("hidden"),document.body.classList.add("modal-active")}catch(n){console.error("Error showing confirm modal:",n),showToast("Terjadi kesalahan saat memproses permintaan Anda.")}else showToast("Mohon cek poin Anda terlebih dahulu.")}function cancelTukarModal(){document.getElementById("confirm-tukar-modal").classList.add("hidden"),document.body.classList.remove("modal-active"),pendingRewardData={id:null,nama:null,poin:null,gambar:null,deskripsi:null}}function proceedToNameInput(){document.getElementById("confirm-tukar-modal").classList.add("hidden"),document.getElementById("name-input-modal").classList.remove("hidden"),setTimeout(()=>{document.getElementById("claim-name-input").focus()},100)}function backToConfirmModal(){document.getElementById("name-input-modal").classList.add("hidden"),document.getElementById("confirm-tukar-modal").classList.remove("hidden"),document.getElementById("claim-name-input").value=""}async function submitNameAndClaim(){const e=document.getElementById("claim-name-input").value.trim();e?e.length<3?showToast("Nama harus minimal 3 karakter."):(document.getElementById("name-input-modal").classList.add("hidden"),document.body.classList.remove("modal-active"),await processClaimReward(pendingRewardData.id,e)):showToast("Mohon masukkan nama Anda terlebih dahulu.")}async function processClaimReward(e,t){const n=sessionStorage.getItem("reward_phone");try{const a=await ApiService.get(`?sheet=tukar_poin&action=search&id=${e}`,{cache:!1});if(!a||a.length===0)return void showToast("Data hadiah tidak ditemukan.");const r=a[0],s=parseFloat(r.poin)||0,o=r.nama||r.judul||"Hadiah",d=parseFloat(sessionStorage.getItem("user_points"))||0;showToast("Sedang memproses penukaran...");const l=`RW-${Date.now()}-${Math.floor(1e5*Math.random())}`,i=await GASActions.post({action:"claim_reward",sheet:"tukar_poin",data:{reward_id:e,phone:n,customer_name:t,request_id:l}}),u=i.claim_id||"CLM-"+Date.now().toString().slice(-6),p=parseFloat(i.points_used||s)||0,m=parseFloat(i.balance_after),c=Number.isNaN(m)?Math.max(0,d-p):m;sessionStorage.setItem("user_points",c);const g=document.querySelector("#points-display h4");g&&(g.innerHTML=`${escapeHtml(c.toFixed(1))} <span class="text-sm font-bold">Poin</span>`);const h=`*KLAIM REWARD POIN BERHASIL*
...
Mohon segera diproses. Terima kasih!`,f=`https://wa.me/628993370200?text=${encodeURIComponent(h)}`;window.claimWhatsAppUrl=f,pendingRewardData={id:null,nama:null,poin:null,gambar:null,deskripsi:null},document.getElementById("name-input-modal").classList.add("hidden"),document.getElementById("confirm-tukar-modal").classList.add("hidden"),setTimeout(()=>{showClaimSuccessModal(u)},300)}catch(a){console.error("Error processing claim:",a),showToast(buildClaimRewardErrorMessage(a))}}function buildClaimRewardErrorMessage(e){const t=String(e&&e.message||e||"").toLowerCase();return t.includes("reward_stock_empty")?"Stok reward sedang habis.":t.includes("reward_daily_quota_reached")?"Quota harian reward sudah habis.":t.includes("points_insufficient")?"Poin Anda tidak cukup untuk reward ini.":t.includes("user_not_found")?"Data poin pengguna tidak ditemukan.":t.includes("rate_limited")?"Terlalu banyak percobaan klaim, coba lagi sebentar.":"Gagal memproses penukaran. Silakan coba lagi."}const WISHLIST_KEY="gos_wishlist";function getWishlist(){const e=localStorage.getItem(WISHLIST_KEY);return e?JSON.parse(e):[]}function saveWishlist(e){localStorage.setItem(WISHLIST_KEY,JSON.stringify(e)),updateWishlistCount()}function toggleWishlist(e){let t=getWishlist();const n=t.indexOf(e);n>-1?(t.splice(n,1),showToast("Produk dihapus dari Wishlist.")):(t.push(e),showToast("Produk ditambahkan ke Wishlist!")),saveWishlist(t),updateProductWishlistIcon(e);const a=document.getElementById("wishlist-modal");a&&!a.classList.contains("hidden")&&renderWishlistItems()}function isProductInWishlist(e){return getWishlist().includes(e)}function updateWishlistCount(){const e=getWishlist().length,t=document.getElementById("wishlist-count");t&&(t.textContent=e,e>0?t.classList.remove("hidden"):t.classList.add("hidden"))}function updateProductWishlistIcon(e){const t=document.getElementById(`wishlist-btn-${e}`);if(!t)return;const n=isProductInWishlist(e)?'<svg class="w-5 h-5 text-red-500 fill-current" viewBox="0 0 24 24"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg>':'<svg class="w-5 h-5 text-gray-400 hover:text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/></svg>';t.innerHTML=n}function openWishlistModal(){const e=document.getElementById("wishlist-modal");e&&(e.classList.remove("hidden"),document.body.classList.add("modal-active"),renderWishlistItems())}function closeWishlistModal(){const e=document.getElementById("wishlist-modal");e&&(e.classList.add("hidden"),document.body.classList.remove("modal-active"))}async function renderWishlistItems(){const e=getWishlist(),t=document.getElementById("wishlist-items-container");if(console.log("\u{1F50D} Rendering wishlist items:",{wishlistIds:e,containerFound:!!t}),!t)return;if(e.length===0)return void(t.innerHTML=`
        ...
        `);if(!allProducts||allProducts.length===0){console.log("\u{1F4E6} Fetching products for wishlist...");try{await fetchProducts(),console.log("\u2705 Products fetched:",allProducts.length)}catch(r){return console.error("\u274C Failed to fetch products:",r),void(t.innerHTML='<p class="text-center text-red-500 py-4">Gagal memuat produk. Silakan refresh halaman.</p>')}}const n=new Set(e.map(r=>String(r).trim()));console.log("\u{1F50D} Wishlist Set:",Array.from(n));const a=allProducts.filter(r=>{const s=String(r.productId||"").trim(),o=n.has(s);return o&&console.log("\u2705 Found wishlist product:",s,r.nama),o});console.log("\u{1F4CA} Wishlist products found:",a.length),a.length!==0?(t.innerHTML=a.map(r=>{const s=r.productId,o=parseFloat(r.harga_tunai||r.harga||0),d=r.gambar?String(r.gambar).split(",")[0].trim():"";return`
            <div class="flex items-center justify-between p-4 border-b border-gray-100 hover:bg-gray-50 transition">
                <div class="flex items-center gap-3 flex-1">
                    <img src="${sanitizeUrl(d,"https://placehold.co/100x100?text=Produk")}" alt="${escapeHtml(r.nama)}" class="w-16 h-16 object-cover rounded-lg shadow-sm" data-fallback-src="https://placehold.co/100x100?text=Produk">
                    <div class="flex-1">
    ...
    `}function selectSidebarCategory(e){setCategory(e),closeCategorySidebar();const t=document.getElementById("products");t&&t.scrollIntoView({behavior:"smooth",block:"start"})}function updateMobileCartCount(){const e=document.getElementById("mobile-cart-count"),t=document.getElementById("cart-count");if(e&&t){const n=cart.length;n>0?(e.textContent=n,e.classList.remove("hidden")):e.classList.add("hidden")}}document.addEventListener("DOMContentLoaded",updateWishlistCount),window.addEventListener("hashchange",handleDeepLink,!1),document.addEventListener("DOMContentLoaded",()=>{setTimeout(handleDeepLink,1e3)});const originalUpdateCartUI=updateCartUI;updateCartUI=function(){typeof originalUpdateCartUI=="function"&&originalUpdateCartUI(),updateMobileCartCount()},document.addEventListener("DOMContentLoaded",function(){updateMobileCartCount()});const normalizePhoneTo08=e=>{const t=String(e??"").replace(/[^0-9]/g,"");if(!t)return"";let n=t;return n.startsWith("62")&&(n=n.slice(2)),n.startsWith("0")&&(n=n.slice(1)),n.startsWith("8")?"0"+n:""},phoneLookupVariants=e=>{const t=normalizePhoneTo08(e);if(!t)return[];const n=t.slice(1);return[t,`+62${n}`,`62${n}`,n]},displayPhone=e=>normalizePhoneTo08(e)||e||"",parseSheetResponse=e=>Array.isArray(e)?e:e&&Array.isArray(e.result)?e.result:e&&e.result?Array.isArray(e.result)?e.result:[e.result]:[],isUnauthorizedApiResponse=e=>!!(e&&typeof e=="object"&&!Array.isArray(e)&&String(e.error||"").toLowerCase()==="unauthorized"),buildSessionQuery=e=>{const t=String(e&&e.session_token||"").trim();return t?`&session_token=${encodeURIComponent(t)}`:""},hasPublicSession=e=>String(e&&e.session_token||"").trim()!=="";let referralProfileCache=null,pendingReferralCodeFromUrl="",referralConfigCache={rewardReferrerPoints:20,rewardRefereePoints:10,minFirstOrder:5e4};function toReferralCodeValue(e){return String(e||"").trim().toUpperCase()}function getReferralCodeFromUrl(){try{const e=new URLSearchParams(window.location.search||"");return toReferralCodeValue(e.get("ref")||e.get("referral")||e.get("kode_referral")||"")}catch(e){return console.warn("Failed parsing referral code from URL:",e),""}}function buildReferralShareUrl(e){const t=toReferralCodeValue(e);if(!t||t==="-")return"";const n=new URL(`${window.location.origin}${window.location.pathname}`);return n.searchParams.set("ref",t),n.toString()}function updateRegisterReferralInfo(){const e=document.getElementById("register-referral-info"),t=document.getElementById("register-referral-info-text");!e||!t||(pendingReferralCodeFromUrl?(t.textContent=`Referral dari link aktif: ${pendingReferralCodeFromUrl}`,e.classList.remove("hidden")):e.classList.add("hidden"))}function buildReferralShareMessage(e,t){const n=toReferralCodeValue(e);if(!n||n==="-"||!t)return"";const a=parseInt(referralConfigCache.rewardReferrerPoints||0,10)||0,r=parseInt(referralConfigCache.rewardRefereePoints||0,10)||0,s=parseInt(referralConfigCache.minFirstOrder||0,10)||0,o=s>0?formatCurrency(s):"sesuai ketentuan program";return["Halo, yuk daftar GoSembako pakai link referral saya.","",`Link: ${t}`,`Kode Referral: ${n}`,"",`Bonus pengguna baru: ${r} poin`,`Bonus pengajak: ${a} poin`,`Bonus aktif setelah pesanan pertama selesai (minimal ${o}).`,"","Terima kasih."].join(`
`)}function updateReferralShareUI(e){const t=toReferralCodeValue(e&&e.kode_referral||referralProfileCache&&referralProfileCache.kode_referral||getLoggedInUser()&&getLoggedInUser().kode_referral||""),n=buildReferralShareUrl(t),a=document.getElementById("referral-code-display"),r=document.getElementById("referral-share-message"),s=document.getElementById("referral-reward-info");if(a&&(a.value=n||"-",a.dataset.referralCode=t||""),r&&(r.textContent=n?buildReferralShareMessage(t,n):"Link referral akan muncul setelah kode referral tersedia."),s){const o=parseInt(referralConfigCache.rewardReferrerPoints||0,10)||0,d=parseInt(referralConfigCache.rewardRefereePoints||0,10)||0,l=parseInt(referralConfigCache.minFirstOrder||0,10)||0;s.textContent=`Pengajak: ${o} poin, Pengguna baru: ${d} poin (setelah pesanan pertama selesai${l>0?`, minimal ${formatCurrency(l)}`:""}).`}}function getCurrentReferralCode(){const e=document.getElementById("referral-code-display");return toReferralCodeValue(e?.dataset?.referralCode||"")||toReferralCodeValue(getLoggedInUser()?.kode_referral||"")}async function loadPublicReferralConfig(){try{const e=CONFIG.getMainApiUrl(),t=await fetch(`${e}?action=public_referral_config&_t=${Date.now()}`);if(!t.ok)return;const n=await t.json();if(!n||n.success!==!0)return;referralConfigCache={rewardReferrerPoints:parseInt(n.reward_referrer_points||referralConfigCache.rewardReferrerPoints,10)||referralConfigCache.rewardReferrerPoints,rewardRefereePoints:parseInt(n.reward_referee_points||referralConfigCache.rewardRefereePoints,10)||referralConfigCache.rewardRefereePoints,minFirstOrder:parseInt(n.min_first_order||referralConfigCache.minFirstOrder,10)||referralConfigCache.minFirstOrder}}catch(e){console.warn("Failed to load public referral config:",e)}}function setReferralStatus(e,t){const n=document.getElementById("referral-status");n&&(n.textContent=e||"",n.className="text-xs",t==="success"?n.classList.add("text-green-600"):t==="error"?n.classList.add("text-red-600"):t==="warning"?n.classList.add("text-amber-600"):n.classList.add("text-gray-500"))}function generateReferralCode(e,t,n){const a=String(e||"USER").toUpperCase().replace(/[^A-Z0-9]/g,"").slice(0,6)||"USER",r=normalizePhoneTo08(t).replace(/\D/g,"").slice(-4)||Math.floor(1e3+Math.random()*9e3).toString();let s=`${a}${r}`.slice(0,24);if(!n.has(s))return s;for(let o=1;o<=2e3;o+=1){const d=`${a}${r}${o}`.slice(0,24);if(!n.has(d))return d}return`${a}${Date.now().toString().slice(-6)}`.slice(0,24)}async function fetchUsersList(){const e=CONFIG.getMainApiUrl(),t=await fetch(`${e}?sheet=users&_t=${Date.now()}`);if(!t.ok)throw new Error("Gagal memuat data users");return parseSheetResponse(await t.json())}async function ensureUserReferralCode(e,t){const n=t.find(o=>String(o.id)===String(e.id))||t.find(o=>normalizePhoneTo08(o.whatsapp||o.phone||"")===normalizePhoneTo08(e.whatsapp));if(!n)return{user:{id:e.id||"",nama:e.nama||"User",whatsapp:normalizePhoneTo08(e.whatsapp||e.phone||""),kode_referral:"",referred_by:"",referral_count:0,referral_points_total:0},code:"",missingProfile:!0};const a=toReferralCodeValue(n.kode_referral);if(a)return{user:n,code:a};const r=new Set(t.map(o=>toReferralCodeValue(o.kode_referral)).filter(Boolean)),s=generateReferralCode(n.nama,n.whatsapp||n.phone,r);return await GASActions.update("users",n.id,{kode_referral:s}),n.kode_referral=s,{user:n,code:s}}function applyReferralDataToUI(e){const t=document.getElementById("referral-code-display"),n=document.getElementById("referral-input"),a=document.getElementById("apply-referral-btn"),r=document.getElementById("referral-count"),s=document.getElementById("referral-points-total");t&&(t.value="-"),r&&(r.textContent=String(parseInt(e.referral_count||0,10)||0)),s&&(s.textContent=String(parseInt(e.referral_points_total||0,10)||0));const o=toReferralCodeValue(e.referred_by)!=="";n&&(n.value=o?toReferralCodeValue(e.referred_by):"",n.disabled=o),a&&(a.disabled=o),o?setReferralStatus(`Referral aktif dengan kode: ${toReferralCodeValue(e.referred_by)}`,"success"):setReferralStatus("Kode referral hanya bisa dipakai satu kali.","info"),updateReferralShareUI(e)}function buildReferralProfileFallback(e){return{id:e&&e.id?e.id:"",nama:e&&e.nama?e.nama:"User",whatsapp:normalizePhoneTo08(e&&(e.whatsapp||e.phone)||""),kode_referral:toReferralCodeValue(e&&e.kode_referral?e.kode_referral:""),referred_by:toReferralCodeValue(e&&e.referred_by?e.referred_by:""),referral_count:parseInt(e&&e.referral_count||0,10)||0,referral_points_total:parseInt(e&&e.referral_points_total||0,10)||0}}function getReferralStatusBadge(e){const t=String(e||"").toLowerCase();return t==="approved"?'<span class="px-2 py-0.5 rounded-full bg-green-100 text-green-700 font-bold">Approved</span>':t==="rejected"||t==="void"?'<span class="px-2 py-0.5 rounded-full bg-red-100 text-red-700 font-bold">Rejected</span>':'<span class="px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 font-bold">Pending</span>'}async function loadReferralHistory(e){const t=document.getElementById("referral-history-list"),n=document.getElementById("referral-history-badge");if(!(!t||!n))try{const a=CONFIG.getMainApiUrl(),r=normalizePhoneTo08(e.whatsapp);let s=[],o=!1;const d=buildSessionQuery(e);if(d){const i=await fetch(`${a}?action=public_referral_history${d}&_t=${Date.now()}`);if(i.ok){const u=await i.json();u&&u.success&&Array.isArray(u.referrals)&&(s=u.referrals,o=!0)}}if(!s.length&&!o){const i=await fetch(`${a}?sheet=referrals&phone=${encodeURIComponent(r)}&_t=${Date.now()}`);if(!i.ok)throw new Error("Gagal memuat riwayat referral");s=parseSheetResponse(await i.json())}const l=s.filter(i=>{const u=normalizePhoneTo08(i.referrer_phone||""),p=normalizePhoneTo08(i.referee_phone||"");return u===r||p===r}).sort((i,u)=>new Date(u.created_at||0).getTime()-new Date(i.created_at||0).getTime()).slice(0,5);if(n.textContent=`${l.length} data`,l.length===0){t.innerHTML='<p class="text-gray-400">Belum ada riwayat referral.</p>';return}t.innerHTML=l.map(i=>{const u=normalizePhoneTo08(i.referrer_phone||"")===r?"Anda mengajak":"Anda diajak",p=normalizePhoneTo08(u==="Anda mengajak"?i.referee_phone||"":i.referrer_phone||""),m=u==="Anda mengajak"?parseInt(i.reward_referrer_points||0,10)||0:parseInt(i.reward_referee_points||0,10)||0;return`
                <div class="border border-gray-200 rounded-lg p-2 bg-gray-50">
                    <div class="flex items-center justify-between mb-1">
                        <span class="font-bold text-gray-700">${escapeHtml(u)}</span>
                        ${getReferralStatusBadge(i.status)}
                    </div>
                    <p class="text-gray-600">No: ${escapeHtml(p||"-")}</p>
                    <p class="text-green-700 font-semibold">Reward: ${escapeHtml(String(m))} poin</p>
                </div>
            `}).join("")}catch(a){console.error("Error loading referral history:",a),n.textContent="error",t.innerHTML='<p class="text-red-500">Gagal memuat riwayat referral.</p>'}}async function fetchPublicReferralProfile(e){if(!hasPublicSession(e))return null;const t=buildSessionQuery(e);if(!t)return null;try{const n=CONFIG.getMainApiUrl(),a=await fetch(`${n}?action=public_user_profile${t}&_t=${Date.now()}`);if(!a.ok)return null;const r=await a.json();return!r||r.success!==!0||!r.user?null:r.user}catch(n){return console.warn("Failed to load public referral profile:",n),null}}async function loadReferralData(e){try{const t=await fetchPublicReferralProfile(e);if(t){referralProfileCache=t,applyReferralDataToUI(t),await loadReferralHistory({...e,session_token:String(e.session_token||t.session_token||"").trim()});return}const n=await fetchUsersList(),a=await ensureUserReferralCode(e,n);if(referralProfileCache=a.user,applyReferralDataToUI(a.user),a.missingProfile){applyReferralDataToUI(buildReferralProfileFallback(e)),setReferralStatus("Kode referral hanya bisa dipakai satu kali.","info"),await loadReferralHistory(e);return}await loadReferralHistory(e)}catch(t){console.error("Error loading referral data:",t),applyReferralDataToUI(buildReferralProfileFallback(e)),setReferralStatus("Kode referral hanya bisa dipakai satu kali.","info");try{await loadReferralHistory(e)}catch(n){console.warn("Failed loading referral history after fallback:",n)}}}document.addEventListener("DOMContentLoaded",()=>{pendingReferralCodeFromUrl=getReferralCodeFromUrl(),updateRegisterReferralInfo(),loadPublicReferralConfig().then(()=>{referralProfileCache&&updateReferralShareUI(referralProfileCache)});const e=getLoggedInUser(),t=document.getElementById("referral-input");t&&t.addEventListener("keydown",n=>{n.key==="Enter"&&(n.preventDefault(),applyReferralCode())}),e?showDashboard(e):showLogin()});function showLogin(){document.getElementById("login-section").classList.remove("hidden"),document.getElementById("register-section").classList.add("hidden"),document.getElementById("forgot-pin-section").classList.add("hidden"),document.getElementById("dashboard-section").classList.add("hidden")}function showDashboard(e){document.getElementById("login-section").classList.add("hidden"),document.getElementById("register-section").classList.add("hidden"),document.getElementById("forgot-pin-section").classList.add("hidden"),document.getElementById("dashboard-section").classList.remove("hidden"),document.getElementById("user-name").textContent=e.nama,document.getElementById("user-whatsapp").textContent=displayPhone(e.whatsapp),loadLoyaltyPoints(e),loadReferralData(e),loadOrderHistory(e)}document.getElementById("login-form").addEventListener("submit",async e=>{e.preventDefault();const t=document.getElementById("login-whatsapp").value.trim(),n=document.getElementById("login-pin").value.trim(),a=document.getElementById("login-btn"),r=document.getElementById("login-error"),s=document.getElementById("login-error-text");if(!t||!n){showError("Mohon lengkapi semua field");return}const o=normalizePhoneTo08(t);if(!o){showError("Gunakan format 08xxxxxxxxxx");return}if(o.length<10||o.length>13){showError("Panjang nomor tidak valid");return}if(n.length!==6){showError("PIN harus 6 digit");return}a.disabled=!0,a.innerHTML=`
        <svg class="animate-spin h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
        </svg>
        <span>Memproses...</span>
    `,r.classList.add("hidden");try{const d=CONFIG.getMainApiUrl(),l="&_t="+Date.now();let i=null;const u=`${d}?action=public_login&phone=${encodeURIComponent(o)}&pin=${encodeURIComponent(n)}${l}`,p=await fetch(u);if(p.ok){const m=await p.json();if(m&&m.success&&m.user)i=m.user;else{const c=String(m&&m.error?m.error:"").toLowerCase();if(c==="login_failed"){showError("Nomor WhatsApp atau PIN salah"),resetLoginButton();return}if(c==="account_inactive"){showError("Akun Anda tidak aktif. Hubungi admin."),resetLoginButton();return}if(c==="rate_limited"){showError(m&&m.message||"Terlalu banyak percobaan login, coba lagi."),resetLoginButton();return}if(!(c==="invalid sheet"||c==="invalid action")){showError(m&&m.message||"Login gagal. Silakan coba lagi."),resetLoginButton();return}}}if(!i){const m=phoneLookupVariants(o),c=await fetch(`${d}?sheet=users${l}`);if(!c.ok){showError("Gagal menghubungi server. Silakan coba lagi."),resetLoginButton();return}const g=await c.json();if(isUnauthorizedApiResponse(g)){showError("Layanan login membutuhkan endpoint public_login. Hubungi admin."),resetLoginButton();return}const h=parseSheetResponse(g);for(const f of m){const y=h.find(x=>normalizePhoneTo08(x.whatsapp||x.phone||"")===normalizePhoneTo08(f));if(y){i=y;break}}if(!i){showError("Nomor WhatsApp tidak terdaftar"),resetLoginButton();return}if((i.pin||"").toString()!==n){showError("PIN salah. Silakan coba lagi."),resetLoginButton();return}}if(i.status&&i.status.toLowerCase()!=="aktif"){showError("Akun Anda tidak aktif. Hubungi admin."),resetLoginButton();return}i.whatsapp=normalizePhoneTo08(i.whatsapp||i.phone||o),saveLoggedInUser(i),showDashboard(i),document.getElementById("dashboard-section").scrollIntoView({behavior:"smooth",block:"start"})}catch(d){console.error("Login error:",d),showError("Terjadi kesalahan. Silakan coba lagi."),resetLoginButton()}});function showError(e){const t=document.getElementById("login-error"),n=document.getElementById("login-error-text");n.textContent=e,t.classList.remove("hidden")}function resetLoginButton(){const e=document.getElementById("login-btn");e.disabled=!1,e.innerHTML=`
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1"></path>
        </svg>
        Masuk
    `}function saveLoggedInUser(e){const t=normalizePhoneTo08(e.whatsapp||e.phone||"");localStorage.setItem("gosembako_user",JSON.stringify({id:e.id,nama:e.nama,whatsapp:t||e.whatsapp,tanggal_daftar:e.tanggal_daftar,status:e.status||"",total_points:parseInt(e.total_points||e.points||e.poin||0,10)||0,kode_referral:toReferralCodeValue(e.kode_referral||""),referred_by:toReferralCodeValue(e.referred_by||""),referral_count:parseInt(e.referral_count||0,10)||0,referral_points_total:parseInt(e.referral_points_total||0,10)||0,session_token:String(e.session_token||"").trim()}))}function getLoggedInUser(){const e=localStorage.getItem("gosembako_user");if(!e)return null;const t=JSON.parse(e);return t.whatsapp=normalizePhoneTo08(t.whatsapp||t.phone||"")||t.whatsapp,t.session_token=String(t.session_token||"").trim(),t}function logout(){confirm("Apakah Anda yakin ingin keluar?")&&(localStorage.removeItem("gosembako_user"),window.location.reload())}async function copyReferralCode(){const e=getCurrentReferralCode();if(!e||e==="-"){setReferralStatus("Link referral belum tersedia.","warning");return}const t=buildReferralShareUrl(e);if(!t){setReferralStatus("Link referral tidak tersedia.","warning");return}try{if(navigator.clipboard&&navigator.clipboard.writeText)await navigator.clipboard.writeText(t);else{const n=document.createElement("textarea");n.value=t,document.body.appendChild(n),n.select(),document.execCommand("copy"),document.body.removeChild(n)}setReferralStatus("Link referral berhasil disalin.","success")}catch(n){console.error("Copy referral failed:",n),setReferralStatus("Gagal menyalin link referral.","error")}}function shareReferralWhatsApp(){const e=getCurrentReferralCode(),t=buildReferralShareUrl(e);if(!t){setReferralStatus("Link referral belum tersedia.","warning");return}const n=buildReferralShareMessage(e,t),a=`https://wa.me/?text=${encodeURIComponent(n)}`;window.open(a,"_blank","noopener"),setReferralStatus("Link referral siap dibagikan ke WhatsApp.","success")}async function applyReferralCode(){const e=getLoggedInUser();if(!e){setReferralStatus("Silakan login ulang.","error");return}const t=document.getElementById("referral-input"),n=document.getElementById("apply-referral-btn"),a=toReferralCodeValue(t?t.value:"");if(!a){setReferralStatus("Masukkan kode referral terlebih dahulu.","warning");return}const r=getCurrentReferralCode();if(r&&a===r){setReferralStatus("Kode referral sendiri tidak bisa dipakai.","error");return}if(t&&t.disabled){setReferralStatus("Kode referral sudah pernah diterapkan.","warning");return}const s=n?n.textContent:"";try{n&&(n.disabled=!0,n.textContent="Memproses...");const o=await GASActions.post({action:"attach_referral",sheet:"users",data:{referee_phone:normalizePhoneTo08(e.whatsapp),ref_code:a}});if(!o||o.success===!1){const d=o&&(o.message||o.error||o.error_code)||"Gagal menerapkan referral";setReferralStatus(d,"error"),n&&(n.disabled=!1,n.textContent=s||"Terapkan");return}t&&(t.value=a,t.disabled=!0),n&&(n.disabled=!0,n.textContent="Terapkan"),setReferralStatus("Kode referral berhasil diterapkan.","success"),await loadReferralData(e)}catch(o){console.error("applyReferralCode error:",o),setReferralStatus(o.message||"Gagal menerapkan kode referral.","error"),n&&(n.disabled=!1,n.textContent=s||"Terapkan")}}async function loadLoyaltyPoints(e){try{const t=CONFIG.getMainApiUrl(),n=normalizePhoneTo08(e.whatsapp),a=parseInt(e.total_points||e.points||e.poin||0,10)||0;if(!n){console.warn("\u26A0\uFE0F Invalid phone number for loyalty points lookup"),document.getElementById("loyalty-points").textContent=String(a),setRewardContextFromAkun(e.whatsapp,a,e.nama);return}console.log(`\u{1F50D} Loading loyalty points for phone: ${n}`);let r=null;const s=buildSessionQuery(e);if(s){const p=await fetch(`${t}?action=public_user_points${s}&_t=${Date.now()}`);if(p.ok){const m=await p.json();m&&m.success&&(r=parseInt(m.points||0,10)||0)}}if(r!==null){document.getElementById("loyalty-points").textContent=String(r),setRewardContextFromAkun(e.whatsapp,r,e.nama);return}if(!hasPublicSession(e)){document.getElementById("loyalty-points").textContent=String(a),setRewardContextFromAkun(e.whatsapp,a,e.nama);return}const o=await fetch(`${t}?sheet=user_points`);if(!o.ok){console.error("\u274C Failed to fetch loyalty points"),document.getElementById("loyalty-points").textContent="0";return}const d=await o.json();if(console.log("\u{1F4E5} Points data received:",d),d&&typeof d=="object"&&String(d.error||"").toLowerCase()==="unauthorized"){console.warn("\u26A0\uFE0F Unauthorized access to user_points. Falling back to user profile points."),document.getElementById("loyalty-points").textContent=String(a),setRewardContextFromAkun(e.whatsapp,a,e.nama);return}let l=Array.isArray(d)?d:d.result||[];if(!Array.isArray(l)){console.warn("\u26A0\uFE0F Unexpected points data format"),document.getElementById("loyalty-points").textContent="0";return}const i=phoneLookupVariants(n);let u=null;for(const p of i)if(u=l.find(m=>normalizePhoneTo08(m.phone||m.whatsapp||"")===normalizePhoneTo08(p)),u){console.log(`\u2705 Found points record for ${p}:`,u);break}if(u){const p=parseInt(u.points||u.poin||0);console.log(`\u2705 User points: ${p}`),document.getElementById("loyalty-points").textContent=p,setRewardContextFromAkun(e.whatsapp,p,e.nama)}else console.log("\u26A0\uFE0F No points record found for user"),document.getElementById("loyalty-points").textContent=String(a),setRewardContextFromAkun(e.whatsapp,a,e.nama)}catch(t){console.error("\u274C Error loading loyalty points:",t);const n=parseInt(e.total_points||e.points||e.poin||0,10)||0;document.getElementById("loyalty-points").textContent=String(n),setRewardContextFromAkun(e.whatsapp,n,e.nama)}}async function loadOrderHistory(e){const t=document.getElementById("order-loading"),n=document.getElementById("order-empty"),a=document.getElementById("order-list"),r=document.getElementById("total-accepted-spend");t.classList.remove("hidden"),n.classList.add("hidden"),a.innerHTML="",r&&(r.textContent="Rp 0");try{const s=CONFIG.getMainApiUrl();let o=[];const d=buildSessionQuery(e);if(d){const l=await fetch(`${s}?action=public_user_orders${d}&_t=${Date.now()}`);if(l.ok){const i=await l.json();i&&i.success&&Array.isArray(i.orders)&&(o=i.orders)}}if(!o.length){let l=await fetch(`${s}?sheet=orders`);if(!l.ok)throw new Error("Gagal memuat riwayat pesanan");let i=await l.json();o=Array.isArray(i)?i.filter(u=>normalizePhoneTo08(u.phone)===normalizePhoneTo08(e.whatsapp)):[]}if(t.classList.add("hidden"),!o||o.length===0){n.classList.remove("hidden");return}if(o=o.filter(l=>{const i=normalizePhoneTo08(l.phone||l.whatsapp||""),u=normalizePhoneTo08(e.whatsapp);return i===u}),o.length===0){n.classList.remove("hidden");return}if(o.sort((l,i)=>{const u=new Date(l.tanggal_pesanan||l.timestamp||0);return new Date(i.tanggal_pesanan||i.timestamp||0)-u}),window.allOrders=o,window.currentPage=1,window.ordersPerPage=10,r){const l=new Set(["terima","diterima","selesai"]),i=o.reduce((u,p)=>{const m=String(p.status||"").toLowerCase().trim();return l.has(m)?u+parseCurrencyValue(p.total||p.total_bayar||0):u},0);r.textContent=formatCurrency(i)}displayOrderPage(1)}catch(s){console.error("Error loading order history:",s),t.classList.add("hidden"),a.innerHTML=`
    ...
    `,t.addEventListener("click",i=>{i.target.tagName!=="BUTTON"&&i.target.tagName!=="A"&&showOrderDetailModal(e)}),t}function getStatusBadge(e){const t=normalizeStatus(e),n={Menunggu:{bg:"bg-yellow-100",text:"text-yellow-700",label:"Menunggu"},Diproses:{bg:"bg-blue-100",text:"text-blue-700",label:"Diproses"},Dikirim:{bg:"bg-purple-100",text:"text-purple-700",label:"Dikirim"},Diterima:{bg:"bg-green-100",text:"text-green-700",label:"Diterima"},Dibatalkan:{bg:"bg-red-100",text:"text-red-700",label:"Dibatalkan"}},a=n[t]||n.Menunggu;return`<span class="${a.bg} ${a.text} text-xs font-bold px-3 py-1 rounded-full">${a.label}</span>`}function normalizeStatus(e){if(!e)return"Menunggu";const t=e.toLowerCase().trim();return{menunggu:"Menunggu",pending:"Menunggu",diproses:"Diproses",proses:"Diproses",processing:"Diproses",dikirim:"Dikirim",kirim:"Dikirim",shipped:"Dikirim",diterima:"Diterima",terima:"Diterima",selesai:"Diterima",completed:"Diterima",dibatalkan:"Dibatalkan",batal:"Dibatalkan",cancelled:"Dibatalkan",canceled:"Dibatalkan"}[t]||e}function formatDate(e){if(!e||e==="N/A"||e==="")return"N/A";let t;if(typeof e=="number")t=new Date(e);else if(typeof e=="string"){let a=e.split(",")[0].trim();if(t=new Date(e),isNaN(t.getTime())&&a.includes("/")){const r=a.split("/");if(r.length===3){const s=parseInt(r[0]),o=parseInt(r[1]),d=parseInt(r[2]);s<=31&&o<=12&&d>1900&&(t=new Date(d,o-1,s))}}}else t=new Date(e);if(isNaN(t.getTime()))return"N/A";const n={year:"numeric",month:"long",day:"numeric"};return t.toLocaleDateString("id-ID",n)}function truncateText(e,t){return e?e.length<=t?e:e.substring(0,t)+"...":"N/A"}function formatCurrency(e){const t=parseInt(e)||0;return new Intl.NumberFormat("id-ID",{style:"currency",currency:"IDR",minimumFractionDigits:0}).format(t)}function showRegisterInfo(){alert(`Untuk mendaftar, silakan hubungi admin GoSembako melalui WhatsApp.

Anda akan diberikan akun dengan nomor WhatsApp dan PIN untuk login.`)}function showRegister(){document.getElementById("login-section").classList.add("hidden"),document.getElementById("register-section").classList.remove("hidden"),document.getElementById("forgot-pin-section").classList.add("hidden"),document.getElementById("dashboard-section").classList.add("hidden"),updateRegisterReferralInfo()}function showForgotPIN(){document.getElementById("login-section").classList.add("hidden"),document.getElementById("register-section").classList.add("hidden"),document.getElementById("forgot-pin-section").classList.remove("hidden"),document.getElementById("dashboard-section").classList.add("hidden")}document.getElementById("register-form").addEventListener("submit",async e=>{e.preventDefault();const t=document.getElementById("register-name").value.trim(),n=document.getElementById("register-whatsapp").value.trim(),a=document.getElementById("register-pin").value.trim(),r=document.getElementById("register-pin-confirm").value.trim(),s=pendingReferralCodeFromUrl,o=document.getElementById("register-error"),d=document.getElementById("register-error-text"),l=document.getElementById("register-success"),i=document.getElementById("register-btn");o.classList.add("hidden"),l.classList.add("hidden");const u=t.replace(/\s/g,"");if(u.length<4){d.textContent="Masukkan Nama Lengkap",o.classList.remove("hidden");return}const p=u.toLowerCase(),m=[/^(.)\1{3,}$/,/^(.{2})\1{2,}$/,/^(.{3})\1{2,}$/,/^([a-z])([a-z])\1\2{2,}$/];for(const f of m)if(f.test(p)){d.textContent="Masukkan Nama Lengkap",o.classList.remove("hidden");return}const c=normalizePhoneTo08(n);if(!c){d.textContent="Gunakan format 08xxxxxxxxxx",o.classList.remove("hidden");return}const g=c.replace(/[^0-9]/g,"");if(g.length<10||g.length>13){d.textContent="Nomor WhatsApp tidak valid",o.classList.remove("hidden");return}const h=[/^(\d)\1{9,}$/,/^08(\d)\1{8,}$/,/^(\d{2})\1{4,}$/,/^(\d{3})\1{3,}$/];for(const f of h)if(f.test(g)){d.textContent="Nomor WhatsApp tidak valid",o.classList.remove("hidden");return}if(a.length!==6||!/^\d{6}$/.test(a)){d.textContent="PIN harus 6 digit angka",o.classList.remove("hidden");return}if(a!==r){d.textContent="PIN tidak cocok",o.classList.remove("hidden");return}i.disabled=!0,i.innerHTML='<svg class="animate-spin h-5 w-5 mx-auto" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>';try{const f=CONFIG.getMainApiUrl(),y=phoneLookupVariants(c);let x=[];const v="&_t="+Date.now();console.log("\u{1F50D} Checking phone formats:",y);try{const w=await fetch(`${f}?sheet=users${v}`);if(!w.ok)console.warn(`API error: ${w.status}`);else{const I=await w.json();x=parseSheetResponse(I).filter(B=>{const $=normalizePhoneTo08(B.whatsapp||B.phone||"");return y.some(S=>normalizePhoneTo08(S)===$)}),x&&x.length>0&&console.log("\u{1F4CA} Found existing user:",x)}}catch(w){console.warn("Check failed:",w.message)}if(console.log("\u{1F4CA} Final check result for",n,":",x),x&&x.length>0){d.textContent="Nomor WhatsApp sudah terdaftar",o.classList.remove("hidden"),i.disabled=!1,i.innerHTML='<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z"></path></svg> Daftar';return}const k=`USR-${Date.now().toString().slice(-6)}`,C=new Date().toISOString().split("T")[0],M=new Date().toLocaleString("id-ID",{year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",second:"2-digit"}),L=await GASActions.create("users",{id:k,nama:t,whatsapp:c,pin:a,tanggal_daftar:C,status:"aktif",total_points:0,created_at:M,referred_by:"",referred_by_phone:"",referral_count:0,referral_points_total:0,kode_referral:""});if(!L.created||L.created<1)throw new Error("Gagal mendaftar");let b="";if(s)try{const w=await GASActions.post({action:"attach_referral",sheet:"users",data:{referee_phone:c,ref_code:s}});w&&w.success?b=" Kode referral berhasil diterapkan.":b=" Akun dibuat, namun kode referral tidak valid."}catch(w){console.warn("Referral attach failed after registration:",w),b=" Akun dibuat, namun kode referral gagal diproses."}l.classList.remove("hidden");const E=l.querySelector("span");E&&(E.textContent="Pendaftaran berhasil! Silakan login."+b),document.getElementById("register-form").reset(),setTimeout(()=>{showLogin()},2e3)}catch(f){console.error("Registration error:",f),d.textContent="Terjadi kesalahan. Silakan coba lagi.",o.classList.remove("hidden")}finally{i.disabled=!1,i.innerHTML='<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z"></path></svg> Daftar'}}),document.getElementById("forgot-pin-form").addEventListener("submit",async e=>{e.preventDefault();const t=document.getElementById("forgot-whatsapp").value.trim(),n=document.getElementById("forgot-error"),a=document.getElementById("forgot-error-text"),r=document.getElementById("forgot-btn"),s=normalizePhoneTo08(t);if(!s){n.classList.remove("hidden"),a.textContent="Gunakan format 08xxxxxxxxxx";return}n.classList.add("hidden"),r.disabled=!0,r.innerHTML='<svg class="animate-spin h-5 w-5 mx-auto" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>';try{const o=CONFIG.getMainApiUrl(),d=await fetch(`${o}?sheet=users`),l=parseSheetResponse(await d.json()).filter(u=>normalizePhoneTo08(u.whatsapp||u.phone||"")===s);if(!l||l.length===0){a.textContent="Nomor WhatsApp tidak terdaftar",n.classList.remove("hidden"),r.disabled=!1,r.innerHTML='<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg> Kirim Kode Verifikasi';return}alert(`Kode verifikasi telah dikirim ke WhatsApp ${s}.

Untuk sementara, hubungi admin untuk reset PIN.`);const i=s.replace(/^0/,"62");window.open(`https://wa.me/628993370200?text=Halo, saya ingin reset PIN akun saya. Nomor WhatsApp: ${i}`,"_blank"),setTimeout(()=>{showLogin()},1e3)}catch(o){console.error("Forgot PIN error:",o),a.textContent="Terjadi kesalahan. Silakan coba lagi.",n.classList.remove("hidden")}finally{r.disabled=!1,r.innerHTML='<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg> Kirim Kode Verifikasi'}});function openEditProfile(){const e=getLoggedInUser();e&&(document.getElementById("edit-name").value=e.nama,document.getElementById("edit-old-pin").value="",document.getElementById("edit-new-pin").value="",document.getElementById("edit-confirm-pin").value="",document.getElementById("edit-error").classList.add("hidden"),document.getElementById("edit-profile-modal").classList.remove("hidden"))}function closeEditProfile(){document.getElementById("edit-profile-modal").classList.add("hidden")}document.getElementById("edit-profile-form").addEventListener("submit",async e=>{e.preventDefault();const t=getLoggedInUser();if(!t)return;const n=document.getElementById("edit-name").value.trim(),a=document.getElementById("edit-old-pin").value.trim(),r=document.getElementById("edit-new-pin").value.trim(),s=document.getElementById("edit-confirm-pin").value.trim(),o=document.getElementById("edit-error"),d=document.getElementById("edit-error-text"),l=document.getElementById("edit-save-btn");if(o.classList.add("hidden"),n.replace(/\s/g,"").length<4){d.textContent="Masukkan Nama Lengkap",o.classList.remove("hidden");return}if(a||r||s){if(!a||!r||!s){d.textContent="Lengkapi semua field PIN",o.classList.remove("hidden");return}if(r.length!==6||!/^\d{6}$/.test(r)){d.textContent="PIN baru harus 6 digit angka",o.classList.remove("hidden");return}if(r!==s){d.textContent="PIN baru tidak cocok",o.classList.remove("hidden");return}}l.disabled=!0,l.textContent="Menyimpan...";try{const i=CONFIG.getMainApiUrl(),u=await(await fetch(`${i}?sheet=users&id=${t.id}`)).json();if(!u||u.length===0)throw new Error("User not found");const p=u[0];if(a&&p.pin!==a){d.textContent="PIN lama salah",o.classList.remove("hidden"),l.disabled=!1,l.textContent="Simpan";return}const m={nama:n};r&&(m.pin=r);const c=await GASActions.update("users",t.id,m);if(!c.affected||c.affected<1)throw new Error("Failed to update");t.nama=n,saveLoggedInUser(t),document.getElementById("user-name").textContent=n,closeEditProfile(),alert("Profil berhasil diperbarui!")}catch(i){console.error("Edit profile error:",i),d.textContent="Terjadi kesalahan. Silakan coba lagi.",o.classList.remove("hidden")}finally{l.disabled=!1,l.textContent="Simpan"}});function showOrderDetailModal(e){document.getElementById("tracking-order-id").textContent=e.id||"N/A";const t=e.tanggal||e.tanggal_pesanan||e.timestamp||e.date||e.created_at;document.getElementById("tracking-order-date").textContent=formatDate(t);const n=normalizeStatus(e.status||"Menunggu"),a=getStatusBadge(n),r=document.getElementById("tracking-status-badge");r&&(r.innerHTML=a),document.getElementById("tracking-products").textContent=e.produk||e.items||e.product_name||"N/A",document.getElementById("tracking-total").textContent=formatCurrency(e.total||e.total_bayar||0);const s=createAnimatedTimeline(n);document.getElementById("tracking-timeline").innerHTML=s,document.getElementById("order-tracking-modal").classList.remove("hidden")}function createAnimatedTimeline(e){const t=[{name:"Menunggu",gif:"wait.gif",key:"Menunggu"},{name:"Diproses",gif:"grocery-basket.gif",key:"Diproses"},{name:"Dikirim",gif:"grocery.gif",key:"Dikirim"},{name:"Diterima",gif:"shipping.gif",key:"Diterima"}];let n=t.findIndex(a=>a.key===e);return e==="Dibatalkan"?`
        ...
        <div class="flex items-center justify-between gap-2 py-2">
            ${t.map((a,r)=>{const s=r<=n,o=r===n,d=r===t.length-1,l=`./assets/images/${a.gif}`,i=sanitizeUrl(l,"./assets/images/wait.gif");return`
                    <div class="flex flex-col items-center flex-1">
                        <div class="${s?"bg-green-50":"bg-gray-100"} w-12 h-12 rounded-full flex items-center justify-center ${o?"ring-2 ring-green-500 ring-offset-2":""}">
                            <img src="${i}" alt="${a.name}" class="w-8 h-8 object-contain ${s?"":"opacity-40"}" data-fallback-src="${l}">
                        </div>
    ...
    `,t}function closeLoyaltyModal(){document.getElementById("loyalty-modal").classList.add("hidden"),switchRewardTab("exchange")}async function openRewardModal(){const e=getLoggedInUser();if(!e)return;await loadModalPoints();const t=document.getElementById("loyalty-modal-points"),n=t?parseInt(t.textContent||"0"):0;setRewardContextFromAkun(e.whatsapp,n,e.nama),document.getElementById("loyalty-modal").classList.remove("hidden"),switchRewardTab("exchange"),fetchRewardItemsForAkun()}async function loadModalPoints(){const e=getLoggedInUser();if(!e)return;const t=parseInt(e.total_points||e.points||e.poin||0,10)||0;try{const n=CONFIG.getMainApiUrl(),a=buildSessionQuery(e);if(a){const d=await fetch(`${n}?action=public_user_points${a}&_t=${Date.now()}`);if(d.ok){const l=await d.json();if(l&&l.success){document.getElementById("loyalty-modal-points").textContent=String(parseInt(l.points||0,10)||0);return}}}if(!hasPublicSession(e)){document.getElementById("loyalty-modal-points").textContent=String(t);return}const r=await fetch(`${n}?sheet=user_points`);if(!r.ok){document.getElementById("loyalty-modal-points").textContent=String(t);return}const s=await r.json();if(s&&typeof s=="object"&&String(s.error||"").toLowerCase()==="unauthorized"){console.warn("\u26A0\uFE0F Unauthorized access to user_points (modal). Falling back to user profile points."),document.getElementById("loyalty-modal-points").textContent=String(t);return}const o=Array.isArray(s)?s.filter(d=>normalizePhoneTo08(d.phone)===normalizePhoneTo08(e.whatsapp)):[];if(o&&o.length>0){const d=o[0],l=parseInt(d.points||d.poin||0);document.getElementById("loyalty-modal-points").textContent=l}else document.getElementById("loyalty-modal-points").textContent=String(t)}catch(n){console.error("Error loading modal points:",n),document.getElementById("loyalty-modal-points").textContent=String(t)}}var escapeHtml=window.FrontendSanitize&&window.FrontendSanitize.escapeHtml||(e=>{const t=document.createElement("div");return t.textContent=e,t.innerHTML}),sanitizeUrl=window.FrontendSanitize&&window.FrontendSanitize.sanitizeUrl||(e=>String(e||"")),ensureImageFallbackHandler=window.FrontendSanitize&&window.FrontendSanitize.ensureImageFallbackHandler||(()=>{});ensureImageFallbackHandler();function hideRewardLoaders(){const e=document.getElementById("reward-items-loading"),t=document.getElementById("rewards-loading");e&&e.classList.add("hidden"),t&&t.classList.add("hidden")}async function fetchRewardItemsForAkun(){const e=document.getElementById("reward-items-loading"),t=document.getElementById("reward-items-empty"),n=document.getElementById("reward-items-list");if(n){e&&e.classList.remove("hidden"),t&&t.classList.add("hidden"),n.innerHTML="";try{const a=CONFIG.getMainApiUrl(),r=await fetch(`${a}?sheet=tukar_poin`);if(!r.ok)throw new Error("Failed to fetch reward items");const s=await r.json(),o=parseSheetResponse(s);renderRewardItemsListAkun(o)}catch(a){console.error("Error fetching reward items:",a),t&&t.classList.add("hidden"),n.innerHTML=`
            <div class="text-center py-6 bg-red-50 rounded-2xl border-2 border-dashed border-red-200">
                <p class="text-xs text-red-600 font-semibold">Gagal memuat hadiah. Silakan coba lagi nanti.</p>
            </div>
        `}finally{hideRewardLoaders()}}}function renderRewardItemsListAkun(e){const t=document.getElementById("reward-items-empty"),n=document.getElementById("reward-items-list");if(n){if(!e||e.length===0){t&&t.classList.remove("hidden"),n.innerHTML="";return}t&&t.classList.add("hidden"),n.innerHTML=e.map(a=>{const r=escapeHtml((a.id||"").toString()),s=escapeHtml((a.nama||a.judul||"Hadiah").toString()),o=parseInt(a.poin||a.Poin||0),d=(a.gambar||"https://via.placeholder.com/80?text=Reward").toString(),l=sanitizeUrl(d,"https://via.placeholder.com/80?text=Reward"),i=escapeHtml((a.deskripsi||"").toString());return`
            <div class="border-2 border-gray-200 rounded-xl p-4 hover:border-amber-500 transition">
                <div class="flex gap-3 mb-3">
                    <!-- Left: Image -->
                    <div class="w-16 h-16 bg-gray-100 rounded-lg overflow-hidden flex-shrink-0">
                        <img src="${l}" alt="${s}" class="w-full h-full object-cover" data-fallback-src="https://via.placeholder.com/80?text=Reward">
                    </div>
                    ...
                    <div class="flex-shrink-0">
                        <span class="bg-amber-100 text-amber-700 text-xs font-bold px-3 py-1 rounded-full whitespace-nowrap">${o} Poin</span>
                    </div>
                </div>
