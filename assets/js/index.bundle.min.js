...
`).filter(c=>c.trim()!==""),u=["\u{1F35C}","\u{1F372}","\u{1F4E6}","\u2615","\u{1F35A}","\u{1F373}","\u{1F9C2}"];r.innerHTML=m.map((c,g)=>`
            <div class="flex items-center gap-4 bg-gray-50/50 p-3 rounded-xl border border-gray-100/50">
                <span class="text-xl">${u[g%u.length]}</span>
...
Saldo poin saat ini: ${n.toFixed(1)}`;if(!confirm(d))return;let l=prompt("Silakan masukkan nama Anda untuk klaim ini:",sessionStorage.getItem("reward_customer_name")||"");if(l===null)return;l=l.trim()||"Pelanggan",showToast("Sedang memproses penukaran...");const i=await fetch(`${API_URL}?sheet=user_points`),m=(await i.json()).find(v=>v.phone===e);if(!m||!m.id)return void alert("Data pengguna tidak ditemukan.");const u=n-s;await GASActions.update("user_points",m.id,{points:u,last_updated:new Date().toLocaleString("id-ID")});const c="CLM-"+Date.now().toString().slice(-6);await ApiService.post("?sheet=claims",{data:[{id:c,phone:e,nama:l,hadiah:r,poin:s,status:"Menunggu",tanggal:new Date().toLocaleString("id-ID")}]}),sessionStorage.setItem("user_points",u);const g=document.querySelector("#points-display h4");g&&(g.innerHTML=`${escapeHtml(u.toFixed(1))} <span class="text-sm font-bold">Poin</span>`);const p=`*KLAIM REWARD POIN BERHASIL*
