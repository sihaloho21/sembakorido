...
function renderProducts(products) {
    ...
    paginatedProducts.forEach(p => {
        ...
        if (p.hargaCoret > p.harga) {
            ...
            hargaCoretHtml = `
                <div class="flex min-h-[16px] items-center gap-1 mb-1 md:min-h-[20px]">
                    <span class="text-[9px] leading-none text-gray-500 line-through md:text-xs">Rp ${p.hargaCoret.toLocaleString('id-ID')}</span>
        ...
        cardsHtml += `
	            <div class="bg-white rounded-lg md:rounded-xl shadow-md md:shadow-lg hover:shadow-xl transition duration-300 relative${hiddenCardClass}" data-product-id="${productId}" aria-disabled="${isHiddenProd ? 'true' : 'false'}">
                    ...
                    ${grosirGridHtml}
                    ${!isHiddenProd ? `
                    <div class="flex items-center justify-end px-2 pt-1 pb-2 md:px-3 md:pb-3">
                        ${hasVariations ? `
                        ...
                        ` : `
                        <div class="product-card-cart-slot" data-product-cart-slot data-product-id="${productId}" data-cart-mode="${inlineCartState.mode}" data-cart-qty="${inlineCartState.quantity}">
...
function populateCartShippingInfo() {
    ...
    if (addresses.length > 0 && addresses[selectedIndex]) {
        ...
        if (address && addresses.length === 0) {
            const initialAddr = {
                label: 'Home',
                name: name === '-' ? 'Penerima' : name,
                detail: address,
                note: note
            };
            saveStoredAddresses([initialAddr]);
...
function handleDeleteAddress() {
    ...
    // Sesuaikan selected_address_index
...
function initAddressForm() {
    ...
    if (addressForm) {
        addressForm.onsubmit = (e) => {
            ...
            saveStoredAddresses(addresses);
            selectAddress(index === -1 ? addresses.length - 1 : index);
...
// Call this in the main initialization block or at the end of script.js
initAddressForm();
...
function getSelectedDeliveryAddress() {
    ...
    if (selectedAddress && selectedAddress.detail) {
        return {
            label: String(selectedAddress.label || 'Alamat pengiriman').trim(),
            recipient: String(selectedAddress.name || 'Penerima').trim(),
            detail: String(selectedAddress.detail || '').trim(),
            note: String(selectedAddress.note || '').trim()
        };
    }

    const user = (typeof getStoredLoggedInUser === 'function') ? getStoredLoggedInUser() : null;
    return {
        label: 'Alamat pengiriman',
        recipient: String((user && (user.name || user.full_name || user.fullname || user.nama)) || localStorage.getItem('cart_recipient_name') || 'Penerima').trim(),
        detail: String((user && (user.alamat || user.address || user.delivery_address || user.alamat_pengiriman)) || localStorage.getItem('cart_delivery_address') || '').trim(),
        note: String((user && (user.catatan_alamat || user.address_note)) || localStorage.getItem('cart_address_note') || '').trim()
    };
}

function closeDeliveryAddressConfirmation() {
    const modal = document.getElementById('delivery-address-confirmation-modal');
    if (!modal) return;
    modal.classList.add('hidden');
    const visibleModals = Array.from(document.querySelectorAll('[data-modal]')).filter((item) => !item.classList.contains('hidden'));
    if (visibleModals.length === 0) document.body.classList.remove('modal-active');
}

function openDeliveryAddressConfirmation(address) {
    const modal = document.getElementById('delivery-address-confirmation-modal');
    if (!modal) return;

    const labelEl = document.getElementById('delivery-address-confirm-label');
    const recipientEl = document.getElementById('delivery-address-confirm-recipient');
    const detailEl = document.getElementById('delivery-address-confirm-detail');
    const noteEl = document.getElementById('delivery-address-confirm-note');
    if (labelEl) labelEl.textContent = address.label || 'Alamat pengiriman';
    if (recipientEl) recipientEl.textContent = address.recipient || 'Penerima';
    if (detailEl) detailEl.textContent = address.detail || 'Belum ada alamat pengiriman';
    if (noteEl) {
        noteEl.textContent = address.note || '';
        noteEl.classList.toggle('hidden', !address.note);
    }
    ...
    if (!modal.dataset.backdropInit) {
        modal.addEventListener('click', (event) => {
            if (event.target === modal) closeDeliveryAddressConfirmation();
        });
        modal.dataset.backdropInit = 'true';
    }
    window.setTimeout(() => document.getElementById('confirm-delivery-address-btn')?.focus(), 0);
}

function openCartAddressEditorFromConfirmation() {
    closeDeliveryAddressConfirmation();
    closeOrderModal();
    openCartModal();
    window.setTimeout(() => openLocationPickerModal(), 0);
}

function confirmDeliveryAddressAndSend() {
    closeDeliveryAddressConfirmation();
    sendToWA(true);
}

function getOrderSubmitButton() {
    return document.getElementById('send-order-btn') || document.querySelector('[data-action="send-wa"]');
}

function setOrderSubmitLoading(isLoading) {
    const sendButton = getOrderSubmitButton();
    if (!sendButton) return;

    if (!sendButton.dataset.idleMarkup) sendButton.dataset.idleMarkup = sendButton.innerHTML;
    sendButton.disabled = Boolean(isLoading);
    sendButton.setAttribute('aria-busy', isLoading ? 'true' : 'false');
    sendButton.classList.toggle('is-submitting', Boolean(isLoading));
    sendButton.classList.toggle('opacity-75', Boolean(isLoading));
    sendButton.classList.toggle('cursor-not-allowed', Boolean(isLoading));
    if (isLoading) {
        sendButton.innerHTML = '<span class="order-submit-spinner" aria-hidden="true"></span><span>Memproses pesanan...</span>';
    } else {
        sendButton.innerHTML = sendButton.dataset.idleMarkup;
    }
}
...
let paymentSummaryRefreshTimer = null;

function getPaymentSummaryMethodKey(paymentMethod) {
    const normalizedMethod = String(paymentMethod || '').trim().toLowerCase();
    if (normalizedMethod === 'qris') return 'qris';
    if (normalizedMethod === 'bayar gajian') return 'gajian';
    if (normalizedMethod === 'paylater') return 'paylater';
    if (normalizedMethod === 'cash / transfer' || normalizedMethod === 'tunai/cod') return 'cash';
    return 'default';
}

function animatePaymentSummary(paymentMethod) {
    const summaryCard = document.querySelector('#order-modal .payment-summary-card');
    if (!summaryCard) return;

    summaryCard.dataset.paymentMethod = getPaymentSummaryMethodKey(paymentMethod);

    if (window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
        return;
    }

    if (paymentSummaryRefreshTimer) {
        window.clearTimeout(paymentSummaryRefreshTimer);
    }

    summaryCard.classList.remove('is-summary-refresh');
    void summaryCard.offsetWidth;
    summaryCard.classList.add('is-summary-refresh');

    paymentSummaryRefreshTimer = window.setTimeout(() => {
        summaryCard.classList.remove('is-summary-refresh');
        paymentSummaryRefreshTimer = null;
    }, 340);
}

function renderPaymentSummary(snapshot, paymentMethod) {
    ...
    animatePaymentSummary(paymentMethod);
}
...
function updateOrderTotal() {
    ...
    renderPaymentSummary(snapshot, payMethod);
...
async function sendToWA(skipDeliveryAddressConfirmation = false) {
    const sendButton = getOrderSubmitButton();
    ...
    const payMethod = getSelectedPayMethodValue();
    const shipMethod = getSelectedShipMethodValue();
    const deliveryAddress = shipMethod === 'Antar Kerumah' ? getSelectedDeliveryAddress() : null;

    if (shipMethod === 'Antar Kerumah' && !skipDeliveryAddressConfirmation) {
        if (!deliveryAddress || !deliveryAddress.detail) {
            showToast('Silakan lengkapi alamat pengiriman terlebih dahulu.');
            closeOrderModal();
            openCartModal();
            window.setTimeout(() => openLocationPickerModal(), 0);
            return;
        }
        openDeliveryAddressConfirmation(deliveryAddress);
        return;
    }
    ...
    const message = `*PESANAN BARU - GOSEMBAKO*\n\n` +
        ...
        `${deliveryAddress && deliveryAddress.detail ? `*Alamat Pengiriman:* ${deliveryAddress.label}, ${deliveryAddress.recipient}\n${deliveryAddress.detail}${deliveryAddress.note ? `\nCatatan: ${deliveryAddress.note}` : ''}\n` : ''}\n` +
