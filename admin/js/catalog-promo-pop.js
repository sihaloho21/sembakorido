    ...
    function renderPreview() {
        ...
        const renderPositionedItem = (item) => {
            ...
            return `<article class="${itemClass}" data-position-surface="${productId}" style="${isRetail ? tileSizeStyle(item.id) + tileAnchorStyle(item.id) : ''}">
                ...
                <span class="flyer-item-badge-shelf"><span class="${badgeClass}">${escapeHtml(brochureBadgeLabel(item))}</span></span>
                <div class="${elementClass('media', 'flyer-retail-media')}" ${draggableAttrs('image', 'gambar produk')}>${item.image ? `<img src="${escapeHtml(safeHttpUrl(item.image))}" alt="" loading="eager">` : 'POP'}${elementHandles('image', 'gambar produk')}</div>
